  ...
  ensureDirectories();
       ...
       // =========================== НАСТРОЙКА CORS ============================
const corsOptions = {
  origin: ["http://151.241.228.247:39906", "http://151.241.228.247:39905"],
  credentials: true,
      ...
      console.error('Ошибка автоматического обновления статусов:', error);
        ...
        app.post('/resend-confirmation', async (req, res) => {
          try {
            ...
            // ВРЕМЕННО: Автоматически подтверждаем пользователя
            await db.execute(
              'UPDATE users SET is_confirmed = true, confirmation_code = NULL WHERE email = ?',
              [email]
            );

            console.log('⚠️ ВНИМАНИЕ: Email автоматически подтвержден из-за проблем с SMTP:', email);
            ...
            res.status(200).json({ 
              message: 'Email автоматически подтвержден. Вы можете войти в систему.',
              email: email,
              user: userData
            });
