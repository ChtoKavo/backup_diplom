...
const AdminPanel = ({ currentUser, onBack }) => {
  ...
  const loadData = async () => {
    try {
      ...
      switch (activeTab) {
        case 'users':
          const usersResponse = await fetch('http://151.247.196.66:5001/admin/users');
          if (!usersResponse.ok) {
        ...
        case 'posts':
          const postsResponse = await fetch('http://151.247.196.66:5001/admin/posts-simple');
          if (!postsResponse.ok) {
            const fallbackResponse = await fetch('http://151.247.196.66:5001/admin/posts?limit=100');
        ...
        case 'statistics':
          const statsResponse = await fetch('http://151.247.196.66:5001/admin/statistics');
          if (!statsResponse.ok) {
  ...
  const handleDeleteUser = async (userId) => {
    ...
    try {
      const response = await fetch(`http://151.247.196.66:5001/admin/users/${userId}?current_user_id=${currentUser.user_id}`, {
        method: 'DELETE',
        headers: {
  ...
  const handleDeletePost = async (postId) => {
    ...
    try {
      const response = await fetch(`http://151.247.196.66:5001/admin/posts/${postId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        }
  ...
  const handleToggleUserStatus = async (userId, currentStatus) => {
    try {
      const response = await fetch(`http://151.247.196.66:5001/admin/users/${userId}/status`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
