  const express = require('express');
  const mysql = require('mysql2');
  const bcrypt = require('bcryptjs');
  const jwt = require('jsonwebtoken');
  const multer = require('multer');
  const path = require('path');
  const fs = require('fs');
  const cors = require('cors');
  const http = require('http');
  const socketIo = require('socket.io');
  const { Expo } = require('expo-server-sdk');
  require('dotenv').config();

  const app = express();
  const server = http.createServer(app);
  const io = socketIo(server, {
    cors: {
      origin: "*",
      methods: ["GET", "POST"]
    },
    transports: ['websocket', 'polling'],
    allowEIO3: true
  });

  // ⭐ НОВОЕ: Инициализация Expo SDK для push-уведомлений
  const expo = new Expo();

  // ========== ФАЙЛ ДЛЯ СОХРАНЕНИЯ ТОКЕНОВ ==========
  const TOKENS_FILE = path.join(__dirname, 'push_tokens.json');

  // Функция загрузки токенов из файла
  const loadTokensFromFile = () => {
    try {
      if (fs.existsSync(TOKENS_FILE)) {
        const data = fs.readFileSync(TOKENS_FILE, 'utf8');
        return JSON.parse(data);
      }
    } catch (error) {
      console.error('❌ Ошибка загрузки токенов:', error.message);
    }
    return {};
  };

  // Функция сохранения токенов в файл
  const saveTokensToFile = (tokens) => {
    try {
      fs.writeFileSync(TOKENS_FILE, JSON.stringify(tokens, null, 2));
    } catch (error) {
      console.error('❌ Ошибка сохранения токенов:', error.message);
    }
  };

  // Загружаем токены при запуске
  let userPushTokens = loadTokensFromFile();
  console.log(`✅ Загружено ${Object.keys(userPushTokens).length} push-токенов из файла`);

  app.use(cors({
    origin: '*',
    methods: ['GET', 'POST', 'PUT', 'DELETE', 'OPTIONS'],
    allowedHeaders: ['Content-Type', 'Authorization'],
    credentials: true
  }));

  // ВАЖНО: Инициализируем req.body по умолчанию
  app.use((req, res, next) => {
    req.body = req.body || {};
    next();
  });

  app.use(express.json({ limit: '50mb', strict: false }));
  app.use(express.urlencoded({ limit: '50mb', extended: true }));

  // ===== MIDDLEWARE ДЛЯ ОБРАБОТКИ ОШИБОК =====

  // Логирование всех входящих запросов
  app.use((req, res, next) => {
    const timestamp = new Date().toISOString();
    console.log(`[${timestamp}] ${req.method} ${req.path}`);
    
    // Логируем только первые 200 символов тела для экономии логов
    if (req.body && Object.keys(req.body).length > 0) {
      console.log('  Body:', JSON.stringify(req.body).slice(0, 200));
    }
    next();
  });

  // Обработчик ошибок JSON парсинга от express.json()
  app.use((err, req, res, next) => {
    // Проверяем, это ошибка парсинга JSON
    if (err instanceof SyntaxError && err.status === 400 && 'body' in err) {
      // Игнорируем ошибку "null is not valid JSON" - это нормально для запросов без body
      if (err.message.includes('null')) {
        console.log('⚠️ Null body received (normal for requests without body)');
        req.body = {};
        return next();
      }
      
      console.error('❌ JSON Parse Error:', err.message);
      console.error('   Request path:', req.path);
      console.error('   Content-Type:', req.headers['content-type']);
      console.error('   Request body:', req.body);
      
      return res.status(400).json({ 
        error: 'Invalid JSON in request body',
        message: err.message,
        path: req.path
      });
    }
    
    // Если это не ошибка JSON, пропускаем дальше
    next(err);
  });

  // ===== КОНЕЦ MIDDLEWARE =====

  // Тестовый эндпоинт
  app.get('/', (req, res) => {
    res.json({ message: 'Сервер работает!', port: process.env.PORT || 3001 });
  });

  // MySQL пул соединений
  const db = mysql.createPool({
    host: process.env.DB_HOST,
    user: process.env.DB_USER,
    password: process.env.DB_PASSWORD,
    database: process.env.DB_NAME,
    multipleStatements: true,
    connectionLimit: 10,
    acquireTimeout: 60000,
    timeout: 60000,
    reconnect: true
  });

  // Проверка подключения к MySQL
  db.getConnection((err, connection) => {
    if (err) {
      console.error('Ошибка подключения к MySQL:', err);
      return;
    }
    console.log('Подключено к MySQL');
    connection.release();
    
    // Создание таблиц
    db.query(`
      CREATE TABLE IF NOT EXISTS users (
        id INT AUTO_INCREMENT PRIMARY KEY,
        username VARCHAR(50) UNIQUE NOT NULL,
        email VARCHAR(100) UNIQUE NOT NULL,
        password VARCHAR(255) NOT NULL,
        phone VARCHAR(20),
        avatar TEXT,
        bio TEXT,
        status VARCHAR(100) DEFAULT 'Онлайн',
        is_online BOOLEAN DEFAULT FALSE,
        is_banned BOOLEAN DEFAULT FALSE,
        ban_reason TEXT DEFAULT NULL,
        banned_at TIMESTAMP NULL,
        banned_by INT NULL,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (banned_by) REFERENCES users(id) ON DELETE SET NULL
      )
    `);
    
    // ⭐ Таблица для кодов верификации по SMS
    db.query(`
      CREATE TABLE IF NOT EXISTS verification_codes (
        id INT AUTO_INCREMENT PRIMARY KEY,
        email VARCHAR(100) NOT NULL,
        phone VARCHAR(20) NOT NULL,
        code VARCHAR(6) NOT NULL,
        session_token VARCHAR(255) NOT NULL,
        attempts INT DEFAULT 0,
        max_attempts INT DEFAULT 5,
        is_verified BOOLEAN DEFAULT FALSE,
        expires_at TIMESTAMP NOT NULL,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        UNIQUE KEY unique_email_phone (email, phone),
        INDEX idx_email (email),
        INDEX idx_phone (phone),
        INDEX idx_expires_at (expires_at)
      )
    `);

    db.query(`
      CREATE TABLE IF NOT EXISTS messages (
        id INT AUTO_INCREMENT PRIMARY KEY,
        sender_id INT NOT NULL,
        receiver_id INT NOT NULL,
        message TEXT NOT NULL,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (sender_id) REFERENCES users(id),
        FOREIGN KEY (receiver_id) REFERENCES users(id)
      )
    `);
    
    db.query(`
      CREATE TABLE IF NOT EXISTS posts (
        id INT AUTO_INCREMENT PRIMARY KEY,
        user_id INT NOT NULL,
        content TEXT NOT NULL,
        image TEXT,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (user_id) REFERENCES users(id)
      )
    `);
    
    db.query(`
      CREATE TABLE IF NOT EXISTS likes (
        id INT AUTO_INCREMENT PRIMARY KEY,
        user_id INT NOT NULL,
        post_id INT NOT NULL,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (user_id) REFERENCES users(id),
        FOREIGN KEY (post_id) REFERENCES posts(id),
        UNIQUE KEY unique_like (user_id, post_id)
      )
    `);
    
    db.query(`
      CREATE TABLE IF NOT EXISTS comments (
        id INT AUTO_INCREMENT PRIMARY KEY,
        user_id INT NOT NULL,
        post_id INT NOT NULL,
        comment TEXT NOT NULL,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (user_id) REFERENCES users(id),
        FOREIGN KEY (post_id) REFERENCES posts(id)
      )
    `);

    // Таблица для хештегов
    db.query(`
      CREATE TABLE IF NOT EXISTS hashtags (
        id INT AUTO_INCREMENT PRIMARY KEY,
        name VARCHAR(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci NOT NULL UNIQUE,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        INDEX idx_name (name)
      )
    `);

    // Таблица для связи постов и хештегов
    db.query(`
      CREATE TABLE IF NOT EXISTS post_hashtags (
        id INT AUTO_INCREMENT PRIMARY KEY,
        post_id INT NOT NULL,
        hashtag_id INT NOT NULL,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (post_id) REFERENCES posts(id) ON DELETE CASCADE,
        FOREIGN KEY (hashtag_id) REFERENCES hashtags(id) ON DELETE CASCADE,
        UNIQUE KEY unique_post_hashtag (post_id, hashtag_id)
      )
    `);

    db.query(`
      CREATE TABLE IF NOT EXISTS friends (
        id INT AUTO_INCREMENT PRIMARY KEY,
        user_id INT NOT NULL,
        friend_id INT NOT NULL,
        status ENUM('pending', 'accepted') DEFAULT 'pending',
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (user_id) REFERENCES users(id),
        FOREIGN KEY (friend_id) REFERENCES users(id),
        UNIQUE KEY unique_friendship (user_id, friend_id)
      )
    `);
    
    db.query(`
      CREATE TABLE IF NOT EXISTS groups (
        id INT AUTO_INCREMENT PRIMARY KEY,
        name VARCHAR(100) NOT NULL,
        description TEXT DEFAULT NULL,
        created_by INT NOT NULL,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (created_by) REFERENCES users(id)
      )
    `);
    
    db.query(`
      CREATE TABLE IF NOT EXISTS group_members (
        id INT AUTO_INCREMENT PRIMARY KEY,
        group_id INT NOT NULL,
        user_id INT NOT NULL,
        role ENUM('admin', 'member') DEFAULT 'member',
        joined_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (group_id) REFERENCES groups(id) ON DELETE CASCADE,
        FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
        UNIQUE KEY unique_membership (group_id, user_id)
      )
    `);
    
    db.query(`
      CREATE TABLE IF NOT EXISTS group_messages (
        id INT AUTO_INCREMENT PRIMARY KEY,
        group_id INT NOT NULL,
        sender_id INT NOT NULL,
        message TEXT NOT NULL,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (group_id) REFERENCES groups(id) ON DELETE CASCADE,
        FOREIGN KEY (sender_id) REFERENCES users(id)
      )
    `);

    // Таблица для отслеживания статуса прочтения личных сообщений
    db.query(`
      CREATE TABLE IF NOT EXISTS message_read_status (
        message_id INT NOT NULL,
        reader_id INT NOT NULL,
        is_read BOOLEAN DEFAULT FALSE,
        read_at TIMESTAMP NULL,
        PRIMARY KEY (message_id, reader_id),
        FOREIGN KEY (message_id) REFERENCES messages(id) ON DELETE CASCADE,
        FOREIGN KEY (reader_id) REFERENCES users(id) ON DELETE CASCADE
      )
    `);

    // Таблица для отслеживания статуса прочтения групповых сообщений
    db.query(`
      CREATE TABLE IF NOT EXISTS group_message_read_status (
        message_id INT NOT NULL,
        reader_id INT NOT NULL,
        is_read BOOLEAN DEFAULT FALSE,
        read_at TIMESTAMP NULL,
        PRIMARY KEY (message_id, reader_id),
        FOREIGN KEY (message_id) REFERENCES group_messages(id) ON DELETE CASCADE,
        FOREIGN KEY (reader_id) REFERENCES users(id) ON DELETE CASCADE
      )
    `);

    // Таблица для закреплённых сообщений
    db.query(`
      CREATE TABLE IF NOT EXISTS pinned_messages (
        id INT AUTO_INCREMENT PRIMARY KEY,
        user_id INT NOT NULL,
        message_id INT NOT NULL,
        chat_type ENUM('personal', 'group') NOT NULL,
        chat_id INT NOT NULL,
        pinned_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
        UNIQUE KEY unique_pin (user_id, message_id, chat_id)
      )
    `);

    // Таблица для push-токенов
    db.query(`
      CREATE TABLE IF NOT EXISTS push_tokens (
        id INT AUTO_INCREMENT PRIMARY KEY,
        user_id INT NOT NULL,
        push_token TEXT NOT NULL,
        device_type VARCHAR(50) DEFAULT 'unknown',
        device_name VARCHAR(100) DEFAULT 'Unknown Device',
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
        UNIQUE KEY unique_user_token (user_id),
        INDEX idx_user_id (user_id),
        INDEX idx_push_token (push_token(255))
      )
    `);

    // Таблица для закреплённых чатов
    db.query(`
      CREATE TABLE IF NOT EXISTS pinned_chats (
        id INT AUTO_INCREMENT PRIMARY KEY,
        user_id INT NOT NULL,
        chat_type ENUM('personal', 'group') NOT NULL,
        chat_id INT NOT NULL,
        pinned_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
        UNIQUE KEY unique_pinned_chat (user_id, chat_type, chat_id)
      )
    `);

    // Таблица для предпочтений пользователя
    db.query(`
      CREATE TABLE IF NOT EXISTS user_preferences (
        id INT AUTO_INCREMENT PRIMARY KEY,
        user_id INT NOT NULL UNIQUE,
        chat_background VARCHAR(255) DEFAULT 'default',
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
      )
    `);

    // Таблица для управления звонками
    db.query(`
      CREATE TABLE IF NOT EXISTS calls (
        id INT AUTO_INCREMENT PRIMARY KEY,
        caller_id INT NOT NULL,
        receiver_id INT NOT NULL,
        call_type ENUM('audio', 'video') NOT NULL,
        status ENUM('pending', 'accepted', 'rejected', 'ended', 'missed') DEFAULT 'pending',
        started_at TIMESTAMP NULL,
        ended_at TIMESTAMP NULL,
        duration INT DEFAULT 0,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (caller_id) REFERENCES users(id) ON DELETE CASCADE,
        FOREIGN KEY (receiver_id) REFERENCES users(id) ON DELETE CASCADE,
        INDEX idx_receiver_id (receiver_id),
        INDEX idx_status (status),
        INDEX idx_created_at (created_at)
      )
    `);

    // Таблица для участников звонков
    db.query(`
      CREATE TABLE IF NOT EXISTS call_participants (
        id INT AUTO_INCREMENT PRIMARY KEY,
        call_id INT NOT NULL,
        user_id INT NOT NULL,
        joined_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        left_at TIMESTAMP NULL,
        is_muted BOOLEAN DEFAULT FALSE,
        is_video_on BOOLEAN DEFAULT TRUE,
        FOREIGN KEY (call_id) REFERENCES calls(id) ON DELETE CASCADE,
        FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
        UNIQUE KEY unique_call_participant (call_id, user_id)
      )
    `);

    // ✅ Таблица для жалоб на посты
    db.query(`
      CREATE TABLE IF NOT EXISTS post_reports (
        id INT AUTO_INCREMENT PRIMARY KEY,
        post_id INT NOT NULL,
        reporter_id INT NOT NULL,
        reason TEXT NOT NULL,
        status ENUM('pending', 'reviewed', 'approved', 'rejected') DEFAULT 'pending',
        reviewed_by INT NULL,
        reviewed_at TIMESTAMP NULL,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (post_id) REFERENCES posts(id) ON DELETE CASCADE,
        FOREIGN KEY (reporter_id) REFERENCES users(id) ON DELETE CASCADE,
        FOREIGN KEY (reviewed_by) REFERENCES users(id) ON DELETE SET NULL,
        UNIQUE KEY unique_report (post_id, reporter_id),
        INDEX idx_status (status),
        INDEX idx_created_at (created_at)
      )
    `);

    // ============================================
    // ⭐ ТАБЛИЦЫ СООБЩЕСТВ (COMMUNITIES)
    // ============================================

    // Таблица категорий сообществ
    db.query(`
      CREATE TABLE IF NOT EXISTS community_categories (
        id INT AUTO_INCREMENT PRIMARY KEY,
        name VARCHAR(50) UNIQUE NOT NULL,
        icon VARCHAR(50),
        description TEXT,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
      )
    `);

    // Основная таблица сообществ
    db.query(`
      CREATE TABLE IF NOT EXISTS communities (
        id INT AUTO_INCREMENT PRIMARY KEY,
        name VARCHAR(100) NOT NULL,
        description TEXT,
        category VARCHAR(50) DEFAULT 'General',
        creator_id INT NOT NULL,
        image LONGTEXT,
        banner_image LONGTEXT,
        rules TEXT,
        is_private BOOLEAN DEFAULT FALSE,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        FOREIGN KEY (creator_id) REFERENCES users(id) ON DELETE CASCADE,
        INDEX idx_category (category),
        INDEX idx_created_at (created_at),
        INDEX idx_is_private (is_private)
      )
    `);

    // Члены сообщества
    db.query(`
      CREATE TABLE IF NOT EXISTS community_members (
        id INT AUTO_INCREMENT PRIMARY KEY,
        community_id INT NOT NULL,
        user_id INT NOT NULL,
        role ENUM('member', 'moderator', 'admin') DEFAULT 'member',
        joined_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (community_id) REFERENCES communities(id) ON DELETE CASCADE,
        FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
        UNIQUE KEY unique_membership (community_id, user_id),
        INDEX idx_community_id (community_id),
        INDEX idx_user_id (user_id)
      )
    `);

    // Статистика сообществ
    db.query(`
      CREATE TABLE IF NOT EXISTS community_stats (
        id INT AUTO_INCREMENT PRIMARY KEY,
        community_id INT UNIQUE NOT NULL,
        members_count INT DEFAULT 0,
        posts_count INT DEFAULT 0,
        followers_count INT DEFAULT 0,
        views_count INT DEFAULT 0,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        FOREIGN KEY (community_id) REFERENCES communities(id) ON DELETE CASCADE
      )
    `);

    // Посты в сообществах
    db.query(`
      CREATE TABLE IF NOT EXISTS community_posts (
        id INT AUTO_INCREMENT PRIMARY KEY,
        community_id INT NOT NULL,
        author_id INT NOT NULL,
        title VARCHAR(255),
        content TEXT NOT NULL,
        image LONGTEXT,
        is_pinned BOOLEAN DEFAULT FALSE,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        FOREIGN KEY (community_id) REFERENCES communities(id) ON DELETE CASCADE,
        FOREIGN KEY (author_id) REFERENCES users(id) ON DELETE CASCADE,
        INDEX idx_community_id (community_id),
        INDEX idx_created_at (created_at),
        INDEX idx_is_pinned (is_pinned)
      )
    `);

    // Лайки на посты сообществ
    db.query(`
      CREATE TABLE IF NOT EXISTS community_post_likes (
        id INT AUTO_INCREMENT PRIMARY KEY,
        post_id INT NOT NULL,
        user_id INT NOT NULL,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (post_id) REFERENCES community_posts(id) ON DELETE CASCADE,
        FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
        UNIQUE KEY unique_like (post_id, user_id),
        INDEX idx_post_id (post_id)
      )
    `);

    // Комментарии на посты сообществ
    db.query(`
      CREATE TABLE IF NOT EXISTS community_post_comments (
        id INT AUTO_INCREMENT PRIMARY KEY,
        post_id INT NOT NULL,
        author_id INT NOT NULL,
        content TEXT NOT NULL,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        FOREIGN KEY (post_id) REFERENCES community_posts(id) ON DELETE CASCADE,
        FOREIGN KEY (author_id) REFERENCES users(id) ON DELETE CASCADE,
        INDEX idx_post_id (post_id)
      )
    `);

    // Подписчики сообществ
    db.query(`
      CREATE TABLE IF NOT EXISTS community_followers (
        id INT AUTO_INCREMENT PRIMARY KEY,
        community_id INT NOT NULL,
        user_id INT NOT NULL,
        followed_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (community_id) REFERENCES communities(id) ON DELETE CASCADE,
        FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
        UNIQUE KEY unique_follow (community_id, user_id),
        INDEX idx_community_id (community_id)
      )
    `);

    // Баны в сообществах
    db.query(`
      CREATE TABLE IF NOT EXISTS community_bans (
        id INT AUTO_INCREMENT PRIMARY KEY,
        community_id INT NOT NULL,
        banned_user_id INT NOT NULL,
        banned_by INT NOT NULL,
        reason TEXT,
        ban_duration_days INT,
        banned_until TIMESTAMP NULL,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (community_id) REFERENCES communities(id) ON DELETE CASCADE,
        FOREIGN KEY (banned_user_id) REFERENCES users(id) ON DELETE CASCADE,
        FOREIGN KEY (banned_by) REFERENCES users(id) ON DELETE CASCADE,
        UNIQUE KEY unique_ban (community_id, banned_user_id),
        INDEX idx_banned_until (banned_until)
      )
    `);

    // Предзаполнение категорий
    db.query(`
      INSERT IGNORE INTO community_categories (name, icon, description) VALUES
      ('General', '📝', 'Общие обсуждения'),
      ('Technology', '💻', 'Технологии и программирование'),
      ('Gaming', '🎮', 'Видеоигры'),
      ('Sports', '⚽', 'Спорт'),
      ('Music', '🎵', 'Музыка'),
      ('Art', '🎨', 'Искусство'),
      ('Books', '📚', 'Книги'),
      ('Movies', '🎬', 'Кино'),
      ('Food', '🍕', 'Еда'),
      ('Travel', '✈️', 'Путешествия'),
      ('Photography', '📷', 'Фотография'),
      ('Fashion', '👗', 'Мода'),
      ('Education', '🎓', 'Образование'),
      ('Business', '💼', 'Бизнес'),
      ('Health', '💪', 'Здоровье')
    `);

    // ============================================
    // 🆘 ТАБЛИЦЫ СИСТЕМЫ ПОДДЕРЖКИ
    // ============================================

    // Таблица категорий поддержки
    db.query(`
      CREATE TABLE IF NOT EXISTS support_categories (
        id INT AUTO_INCREMENT PRIMARY KEY,
        name VARCHAR(50) UNIQUE NOT NULL,
        icon VARCHAR(50),
        description TEXT,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
      )
    `);

    // Таблица тикетов поддержки
    db.query(`
      CREATE TABLE IF NOT EXISTS support_tickets (
        id INT AUTO_INCREMENT PRIMARY KEY,
        user_id INT NOT NULL,
        subject VARCHAR(255) NOT NULL,
        message TEXT NOT NULL,
        category VARCHAR(50) DEFAULT 'general',
        priority ENUM('low', 'medium', 'high', 'critical') DEFAULT 'medium',
        status ENUM('open', 'in_progress', 'resolved', 'closed') DEFAULT 'open',
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
        INDEX idx_user_id (user_id),
        INDEX idx_status (status),
        INDEX idx_priority (priority),
        INDEX idx_category (category),
        INDEX idx_created_at (created_at)
      )
    `);

    // Таблица ответов на тикеты
    db.query(`
      CREATE TABLE IF NOT EXISTS support_replies (
        id INT AUTO_INCREMENT PRIMARY KEY,
        ticket_id INT NOT NULL,
        admin_id INT NOT NULL,
        message TEXT NOT NULL,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (ticket_id) REFERENCES support_tickets(id) ON DELETE CASCADE,
        FOREIGN KEY (admin_id) REFERENCES users(id) ON DELETE CASCADE,
        INDEX idx_ticket_id (ticket_id),
        INDEX idx_admin_id (admin_id)
      )
    `);

    // Таблица статуса прочитанных тикетов
    db.query(`
      CREATE TABLE IF NOT EXISTS support_ticket_read_status (
        id INT AUTO_INCREMENT PRIMARY KEY,
        ticket_id INT NOT NULL,
        user_id INT NOT NULL,
        is_read BOOLEAN DEFAULT FALSE,
        read_at TIMESTAMP NULL,
        FOREIGN KEY (ticket_id) REFERENCES support_tickets(id) ON DELETE CASCADE,
        FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
        UNIQUE KEY unique_read_status (ticket_id, user_id)
      )
    `);

    // Предзаполнение категорий поддержки
    db.query(`
      INSERT IGNORE INTO support_categories (name, icon, description) VALUES
      ('general', '📝', 'Общие вопросы'),
      ('bug', '🐛', 'Сообщить об ошибке'),
      ('feature', '✨', 'Запросить функцию'),
      ('account', '👤', 'Проблемы с аккаунтом'),
      ('payment', '💳', 'Вопросы оплаты'),
      ('technical', '⚙️', 'Технические проблемы'),
      ('abuse', '⚠️', 'Сообщить о нарушении')
    `);
  });

  // ⭐ Функции для Email верификации
  const nodemailer = require('nodemailer');
  const crypto = require('crypto');

  console.log('\n' + '='.repeat(70));
  console.log('🔍 Проверка EMAIL конфигурации:');
  console.log('  EMAIL_USER:', process.env.EMAIL_USER ? '✅ задан' : '❌ НЕ задан');
  console.log('  EMAIL_PASSWORD:', process.env.EMAIL_PASSWORD ? '✅ задан' : '❌ НЕ задан');
  console.log('='.repeat(70) + '\n');

  // Настройка отправки email
  // Поддержка двух режимов:
  // 1. SendGrid - облачный сервис (РЕКОМЕНДУЕТСЯ для хостингов)
  // 2. service: 'gmail' - встроенный сервис Gmail
  // 3. Custom SMTP - для других почтовых сервисов
  
  let transporter;
  
  if (process.env.SENDGRID_API_KEY) {
    // SendGrid configuration (рекомендуется для production)
    console.log('\n📧 EMAIL: Используется SendGrid API');
    const sgTransport = require('nodemailer-sendgrid-transport');
    transporter = nodemailer.createTransport(sgTransport({
      auth: {
        api_key: process.env.SENDGRID_API_KEY
      }
    }));
  } else if (process.env.EMAIL_SMTP_HOST) {
    // Custom SMTP configuration
    console.log('\n📧 EMAIL: Используется Custom SMTP');
    transporter = nodemailer.createTransport({
      host: process.env.EMAIL_SMTP_HOST,
      port: parseInt(process.env.EMAIL_SMTP_PORT || '587'),
      secure: process.env.EMAIL_SMTP_SECURE === 'true', // true для 465, false для 587
      auth: {
        user: process.env.EMAIL_SMTP_USER || process.env.EMAIL_USER,
        pass: process.env.EMAIL_SMTP_PASSWORD || process.env.EMAIL_PASSWORD
      },
      tls: {
        rejectUnauthorized: false
      },
      connectionTimeout: 10000,
      socketTimeout: 10000,
      pool: process.env.EMAIL_SMTP_POOL === 'true' ? {
        maxConnections: 3,
        maxMessages: 20,
        rateDelta: 2000,
        rateLimit: 5
      } : undefined
    });
  } else {
    // Gmail service
    console.log('\n📧 EMAIL: Используется Gmail SMTP');
    // Попробуем оба варианта: с сервисом и с явными хостами
    const gmailConfig = {
      auth: {
        user: process.env.EMAIL_USER,
        pass: process.env.EMAIL_PASSWORD
      },
      tls: {
        rejectUnauthorized: false
      },
      connectionTimeout: 15000,
      socketTimeout: 15000,
      pool: {
        maxConnections: 2,
        maxMessages: 10,
        rateDelta: 1000,
        rateLimit: 5
      }
    };

    // Если указан EMAIL_SERVICE, используем service (например 'gmail')
    if (process.env.EMAIL_SERVICE) {
      gmailConfig.service = process.env.EMAIL_SERVICE;
      console.log(`   Сервис: ${process.env.EMAIL_SERVICE}`);
    } else {
      // Иначе используем явные хосты (gmail smtp)
      gmailConfig.host = 'smtp.gmail.com';
      gmailConfig.port = 465;
      gmailConfig.secure = true; // TLS на 465
      console.log('   Хост: smtp.gmail.com:465 (SSL)');
    }

    transporter = nodemailer.createTransport(gmailConfig);
  }

  // Проверка конфигурации при старте
  console.log('📋 Проверка EMAIL конфигурации...');
  
  // Добавляем таймаут на проверку (15 сек)
  const verifyTimeout = setTimeout(() => {
    console.log('⏱️  EMAIL проверка timeout - используем fallback (сохранение в файл)');
  }, 15000);

  transporter.verify((error, success) => {
    clearTimeout(verifyTimeout);
    if (error) {
      console.log('⚠️  EMAIL КОНФИГУРАЦИЯ:', error.message);
      console.log('   Коды будут сохраняться в файл для тестирования\n');
    } else {
      console.log('✅ EMAIL СЕРВИС ГОТОВ К ОТПРАВКЕ');
      console.log(`   From: ${process.env.EMAIL_USER}\n`);
    }
  });

  function generateVerificationCode() {
    const code = Math.floor(100000 + Math.random() * 900000).toString();
    console.log(`🔐 Сгенерирован код: ${code}`);
    return code;
  }

  function generateSessionToken() {
    const token = crypto.randomBytes(32).toString('hex');
    console.log(`🎫 Сгенерирован session token: ${token.substring(0, 16)}...`);
    return token;
  }

  async function sendVerificationEmail(email, code, username) {
    const fs = require('fs');
    const path = require('path');
    const codesLogFile = path.join(__dirname, 'verification_codes.log');

    try {
      console.log('\n📧 Отправляю email...');
      
      const mailOptions = {
        from: process.env.EMAIL_USER,
        to: email,
        subject: 'Код подтверждения аккаунта',
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 20px; border-radius: 10px 10px 0 0; color: white;">
              <h2 style="margin: 0; text-align: center;">Подтверждение аккаунта</h2>
            </div>
            
            <div style="background: #f8f9fa; padding: 30px; border-radius: 0 0 10px 10px;">
              <p style="color: #333; font-size: 16px;">Привет, <strong>${username}</strong>!</p>
              
              <p style="color: #666; font-size: 14px;">
                Вы создали новый аккаунт. Для завершения регистрации введите код подтверждения:
              </p>
              
              <div style="background: white; border: 2px solid #667eea; border-radius: 8px; padding: 20px; text-align: center; margin: 20px 0;">
                <h1 style="margin: 0; color: #667eea; font-size: 40px; letter-spacing: 5px;">
                  ${code}
                </h1>
              </div>
              
              <p style="color: #999; font-size: 12px; text-align: center;">
                Код действителен 5 минут
              </p>
              
              <hr style="border: none; border-top: 1px solid #ddd; margin: 20px 0;">
              
              <p style="color: #999; font-size: 12px;">
                Если вы не создавали аккаунт, проигнорируйте это письмо.
              </p>
            </div>
          </div>
        `
      };

      console.log('📧 Конфигурация email:');
      console.log(`   from: ${mailOptions.from}`);
      console.log(`   to: ${mailOptions.to}`);
      console.log(`   subject: ${mailOptions.subject}`);

      const info = await transporter.sendMail(mailOptions);
      
      console.log(`✅ Email успешно отправлен на ${email}`);
      console.log(`   Message ID: ${info.messageId}`);
      console.log('');
      return true;
    } catch (error) {
      console.error('❌ Ошибка отправки email:', error.message);
      console.error('   Error Code:', error.code);
      console.error('   Command:', error.command);
      
      // 📝 FALLBACK: Сохраняем код в файл для тестирования
      console.log('📝 Fallback: Сохраняю код в файл для тестирования...');
      try {
        const logEntry = {
          timestamp: new Date().toISOString(),
          email: email,
          username: username,
          code: code,
          note: 'Код сохранён в файл т.к. email не отправился'
        };

        // Добавляем в файл
        const existingData = fs.existsSync(codesLogFile) ? fs.readFileSync(codesLogFile, 'utf8') : '[]';
        const codes = JSON.parse(existingData || '[]');
        codes.push(logEntry);
        fs.writeFileSync(codesLogFile, JSON.stringify(codes, null, 2));

        console.log(`📝 Код сохранён в ${codesLogFile}`);
        console.log(`   Email: ${email}`);
        console.log(`   Код: ${code}`);
        console.log('');
        
        // Не выбрасываем ошибку - регистрация всё равно успешна
        return true;
      } catch (fileError) {
        console.error('❌ Ошибка сохранения в файл:', fileError.message);
        return true; // Всё равно возвращаем true, код в БД сохранён
      }
    }
  }

  // Эндпоинт для получения кода из файла (только для DEV)
  app.get('/api/dev/verification-codes', (req, res) => {
    const fs = require('fs');
    const path = require('path');
    const codesLogFile = path.join(__dirname, 'verification_codes.log');

    try {
      if (!fs.existsSync(codesLogFile)) {
        return res.json({ message: 'Файл кодов ещё не создан', codes: [] });
      }

      const data = fs.readFileSync(codesLogFile, 'utf8');
      const codes = JSON.parse(data || '[]');

      // Показываем только последние 10 кодов
      const recentCodes = codes.slice(-10).map(c => ({
        email: c.email,
        code: c.code,
        time: c.timestamp
      }));

      res.json({
        message: '📝 Последние коды верификации (только для DEV)',
        total: codes.length,
        recent: recentCodes
      });
    } catch (error) {
      res.status(500).json({ error: 'Ошибка чтения файла', details: error.message });
    }
  });

  // Middleware для проверки токена
  function authenticateToken(req, res, next) {
    const authHeader = req.headers['authorization'];
    const token = authHeader && authHeader.split(' ')[1];
    
    if (!token) {
      return res.sendStatus(401);
    }
    
    jwt.verify(token, process.env.JWT_SECRET, (err, user) => {
      if (err) return res.sendStatus(403);
      
      // ✅ НОВОЕ: Проверяем, не забанен ли пользователь
      db.query('SELECT is_banned, ban_reason FROM users WHERE id = ?', [user.id], (err, results) => {
        if (err) return res.sendStatus(500);
        
        if (results.length > 0 && results[0].is_banned) {
          return res.status(403).json({ 
            error: 'Ваш аккаунт заблокирован', 
            reason: results[0].ban_reason || 'Нарушение правил сообщества'
          });
        }
        
        req.user = user;
        next();
      });
    });
  }

  // Middleware для проверки статуса муте
  function checkUserMuted(req, res, next) {
    if (global.mutedUsers) {
      const muteInfo = global.mutedUsers.get(String(req.user.id));
      
      if (muteInfo && new Date() < new Date(muteInfo.muteUntil)) {
        return res.status(403).json({ 
          error: 'Вы заглушены администратором',
          until: muteInfo.muteUntil
        });
      } else if (muteInfo) {
        global.mutedUsers.delete(String(req.user.id));
      }
    }
    next();
  }

  // ========== HELPER FUNCTION: Transform posts data ==========
  // Конвертирует одиночное изображение в массив для совместимости
  const transformPostsData = (posts) => {
    return posts.map(post => {
      let processedPost = { ...post };
      
      // Если images уже есть - пробуем распарсить если это JSON строка
      if (processedPost.images && typeof processedPost.images === 'string') {
        try {
          processedPost.images = JSON.parse(processedPost.images);
          console.log(`✅ Распарсены images для поста ${post.id}`);
        } catch (e) {
          console.warn(`⚠️ Ошибка парсинга images для поста ${post.id}:`, e.message);
          processedPost.images = null;
        }
      }
      
      // КЛЮЧЕВОЕ: Если есть одиночное изображение image, но нет массива images - создаем его
      if (processedPost.image && (!processedPost.images || processedPost.images.length === 0)) {
        processedPost.images = [processedPost.image];
        console.log(`✅ Конвертировано одиночное изображение в массив для поста ${post.id}`);
      }
      
      return processedPost;
    });
  };

  // ========== HELPER FUNCTION: Extract and save hashtags ==========

  const extractAndSaveHashtags = (content, postId) => {
    return new Promise((resolve, reject) => {
      // ✨ ИСПРАВЛЕНО: Regex теперь поддерживает Unicode (Cyrillic, emojis и т.д.)
      // /#[\p{L}\p{N}_-]+/gu - это Unicode Property Escapes
      // \p{L} = любая буква (Latin, Cyrillic, Greek и т.д.)
      // \p{N} = любая цифра
      // _- = подчеркивание и дефис
      const hashtags = (content.match(/#[\p{L}\p{N}_-]+/gu) || []).map(tag => tag.toLowerCase());
      
      console.log(`🏷️ Извлечены хештеги для поста ${postId}:`, hashtags);
      
      if (hashtags.length === 0) {
        resolve([]);
        return;
      }
      
      // Удаляем дубликаты
      const uniqueHashtags = [...new Set(hashtags)];
      
      // Для каждого хештега: создаем/получаем ID и связываем с постом
      const savedHashtags = [];
      let completed = 0;
      
      uniqueHashtags.forEach(hashtag => {
        // Вставляем или получаем хештег
        db.query(
          'INSERT INTO hashtags (name) VALUES (?) ON DUPLICATE KEY UPDATE id=LAST_INSERT_ID(id)',
          [hashtag],
          (err, result) => {
            if (err) {
              console.error('❌ Error saving hashtag:', err);
              completed++;
              if (completed === uniqueHashtags.length) {
                resolve(savedHashtags);
              }
              return;
            }
            
            const hashtagId = result.insertId;
            
            // Создаем связь пост-хештег
            db.query(
              'INSERT INTO post_hashtags (post_id, hashtag_id) VALUES (?, ?) ON DUPLICATE KEY UPDATE id=id',
              [postId, hashtagId],
              (err) => {
                if (err) console.error('❌ Error linking hashtag to post:', err);
                savedHashtags.push(hashtag);
                completed++;
                
                if (completed === uniqueHashtags.length) {
                  console.log(`✅ Сохранено ${savedHashtags.length} уникальных хештегов`);
                  resolve(savedHashtags);
                }
              }
            );
          }
        );
      });
    });
  };

  // ========== CALL TRACKING WITH PUSH NOTIFICATIONS ==========
  // Активные звонки: { call_id: { from_user_id, to_user_id, status, started_at, socket_from } }
  const activeCalls = new Map();

  // Пользователи, присоединенные к Socket.io: { user_id: [socket_id, ...] }
  const userSockets = new Map();

  const getUserSocketIds = (targetUserId) => {
    if (!targetUserId) return [];
    const sockets = userSockets.get(String(targetUserId));
    return Array.isArray(sockets) ? sockets : [];
  };

  const emitToUserSockets = (targetUserId, event, payload) => {
    const sockets = getUserSocketIds(targetUserId);
    sockets.forEach((socketId) => io.to(socketId).emit(event, payload));
    return sockets.length;
  };

  // Хранилище push-токенов пользователей: { user_id: push_token }
  // (Загружается из файла push_tokens.json при запуске)

  // ==================== ФУНКЦИИ PUSH-УВЕДОМЛЕНИЙ ====================

  /**
   * Отправить push-уведомление о входящем звонке
   */
  async function sendCallPushNotification(userId, callerName, callType, callData) {
    try {
      const pushToken = userPushTokens[String(userId)];
      
      if (!pushToken) {
        console.log(`❌ Push-токен не найден для пользователя ${userId}`);
        return false;
      }
      
      // Проверяем валидность Expo токена
      if (!Expo.isExpoPushToken(pushToken)) {
        console.log(`⚠️ Невалидный push-токен: ${pushToken}`);
        delete userPushTokens[String(userId)];
        saveTokensToFile(userPushTokens);
        return false;
      }
      
      // Формируем сообщение
      const message = {
        to: pushToken,
        sound: 'default',
        title: `${callType === 'video' ? '📹' : '📞'} ${callerName}`,
        body: `${callType === 'video' ? 'Видео' : 'Аудио'} звонок`,
        data: {
          type: 'incoming_call',
          callType: callType,
          callerId: callData.from_user_id,
          callerName: callerName,
          callId: callData.call_id,
        },
        badge: 1,
        priority: 'high',
      };

      // Отправляем push через Expo
      const tickets = await expo.sendPushNotificationsAsync([message]);
      console.log(`📬 Push-уведомление отправлено пользователю ${userId}`);
      console.log(`   Заголовок: "${message.title}"`);
      console.log(`   Токен: ${pushToken.slice(0, 40)}...`);
      
      return true;
    } catch (error) {
      console.error('❌ Ошибка при отправке push-уведомления о звонке:', error.message);
      return false;
    }
  }

  /**
   * Отправить push-уведомление о пропущенном звонке
   */
  async function sendMissedCallNotification(userId, callerName, callType) {
    try {
      const pushToken = userPushTokens[String(userId)];
      
      if (!pushToken) return false;
      
      if (!Expo.isExpoPushToken(pushToken)) {
        delete userPushTokens[String(userId)];
        saveTokensToFile(userPushTokens);
        return false;
      }

      const message = {
        to: pushToken,
        sound: 'default',
        title: `📞 Пропущенный ${callType === 'video' ? 'видео' : 'аудио'} звонок`,
        body: `От: ${callerName}`,
        data: {
          type: 'missed_call',
          callType: callType,
          callerName: callerName,
        },
        badge: 1,
      };

      await expo.sendPushNotificationsAsync([message]);
      console.log(`📬 Уведомление о пропущенном звонке отправлено пользователю ${userId}`);
      
      return true;
    } catch (error) {
      console.error('❌ Ошибка при отправке уведомления о пропущенном звонке:', error.message);
      return false;
    }
  }

  // ⭐ ПОМОЩНИК: Получить push-токен из БД (MySQL)
  const getPushTokenFromDB = (userId) => {
    return new Promise((resolve, reject) => {
      db.query(
        'SELECT push_token, device_type, device_name, updated_at FROM push_tokens WHERE user_id = ? ORDER BY updated_at DESC LIMIT 1',
        [userId],
        (err, results) => {
          if (err) {
            console.error(`❌ Ошибка запроса токена из БД для пользователя ${userId}:`, err.message);
            resolve(null);
            return;
          }
          
          if (results && results.length > 0) {
            const { push_token, device_type, device_name, updated_at } = results[0];
            console.log(`✅ Токен найден в БД для ${userId}:`);
            console.log(`   Токен: ${push_token.slice(0, 40)}...`);
            console.log(`   Устройство: ${device_type} (${device_name})`);
            console.log(`   Обновлен: ${updated_at}`);
            resolve(push_token);
          } else {
            console.log(`❌ Токен не найден в БД для пользователя ${userId}`);
            // Дополнительная отладка: проверяем, есть ли вообще записи в таблице
            db.query('SELECT COUNT(*) as count FROM push_tokens', (err, countResult) => {
              if (!err && countResult) {
                console.log(`   📊 Всего токенов в БД: ${countResult[0].count}`);
                // Показываем все user_ids в таблице для отладки
                db.query('SELECT DISTINCT user_id FROM push_tokens LIMIT 10', (err, userIds) => {
                  if (!err && userIds) {
                    const ids = userIds.map(r => r.user_id).join(', ');
                    console.log(`   📋 User IDs с токенами: ${ids}`);
                  }
                });
              }
            });
            resolve(null);
          }
        }
      );
    });
  };

  // ============================================
  // 🔄 ОТСЛЕЖИВАНИЕ АКТИВНЫХ ЧАТОВ
  // ============================================

  // { userId: { chatId, chatType, timestamp } }
  const activeChats = new Map();

  // 🔍 ФУНКЦИЯ для проверки - находится ли пользователь в активном чате?
  function isUserInActiveChat(userId, chatId, chatType) {
    const activeChat = activeChats.get(userId);
    
    if (!activeChat) {
      // Пользователь офлайн или вышел из всех чатов
      return false;
    }
    
    const isInChat = 
      activeChat.chatId === chatId && 
      activeChat.chatType === chatType;
    
    if (isInChat) {
      console.log(`⚠️ Пользователь ${userId} в активном ${chatType}-чате ${chatId} -> PUSH не отправляем`);
    }
    
    return isInChat;
  }

  // Для отладки: функция для просмотра всех активных чатов
  function debugActiveChats() {
    console.log('📊 === АКТИВНЫЕ ЧАТЫ ===');
    for (const [userId, chat] of activeChats) {
      console.log(`  Пользователь ${userId}: ${chat.chatType}-чат ${chat.chatId}`);
    }
    console.log('========================');
  }

  // ==================== SOCKET.IO CONNECTION ==========
  io.on('connection', (socket) => {
    console.log('🔌 Socket подключен:', socket.id);

    let userId = null;

    const attachUserToSocket = (incomingUserId) => {
      if (!incomingUserId) return;
      userId = incomingUserId;
      socket.join(`user_${userId}`);
      console.log(`✅ Socket ${socket.id} присоединился к комнате user_${userId}`);
      
      const userKey = String(userId);
      if (!userSockets.has(userKey)) {
        userSockets.set(userKey, []);
      }
      const sockets = userSockets.get(userKey);
      if (!sockets.includes(socket.id)) {
        sockets.push(socket.id);
        
        // ⭐ ИСПРАВЛЕНО: Обновляем статус в БД
        db.query('UPDATE users SET is_online = 1, last_seen = NOW() WHERE id = ?', [userId], (err) => {
          if (err) console.error('Ошибка обновления статуса:', err);
          else console.log(`✅ Пользователь ${userId} установлен онлайн`);
        });

        // ⭐ ИСПРАВЛЕНО: Отправляем через комнаты друзей
        db.query(`
          SELECT DISTINCT
            CASE 
              WHEN f.user_id = ? THEN f.friend_id 
              ELSE f.user_id 
            END as friend_id
          FROM friends f
          WHERE (f.user_id = ? OR f.friend_id = ?) AND f.status = 'accepted'
        `, [userId, userId, userId], (err, friends) => {
          if (friends && friends.length > 0) {
            friends.forEach(friend => {
              io.to(`user_${friend.friend_id}`).emit('user_status_changed', {
                userId: userId,
                is_online: true,
                timestamp: new Date().toISOString()
              });
            });
          }
        });
      }
    };

    // Попытка авторизовать сразу по данным рукопожатия
    const handshakeUserId = socket.handshake?.auth?.user_id || socket.handshake?.query?.user_id;
    if (handshakeUserId) {
      attachUserToSocket(handshakeUserId);
    }

    // ✅ АУТЕНТИФИКАЦИЯ СОКЕТА
    socket.on('authenticate_socket', (data) => {
      const incomingUserId = data?.user_id;
      console.log(`📍 authenticate_socket вызван для user_id=${incomingUserId}`);
      attachUserToSocket(incomingUserId);

      // Отправляем снимок статусов всех друзей при подключении
      if (incomingUserId) {
        // ⭐ ИСПРАВЛЕНО: Обновляем статус пользователя в БД немедленно
        db.query('UPDATE users SET is_online = 1, last_seen = NOW() WHERE id = ?', [incomingUserId], (err) => {
          if (err) console.error('❌ Ошибка обновления статуса при подключении:', err);
          else console.log(`✅ Статус пользователя ${incomingUserId} установлен в онлайн`);
        });

        db.query(`
          SELECT DISTINCT
            CASE 
              WHEN f.user_id = ? THEN f.friend_id 
              ELSE f.user_id 
            END as user_id,
            u.is_online,
            u.username
          FROM friends f
          JOIN users u ON (f.friend_id = u.id OR f.user_id = u.id)
          WHERE (f.user_id = ? OR f.friend_id = ?) AND f.status = 'accepted'
          AND u.id != ?
        `, [incomingUserId, incomingUserId, incomingUserId, incomingUserId], (err, results) => {
          if (err) {
            console.error(`❌ Ошибка при запросе друзей пользователя ${incomingUserId}:`, err);
            return;
          }
          
          console.log(`📊 Найдено друзей: ${results?.length || 0} для пользователя ${incomingUserId}`);
          
          if (results && results.length > 0) {
            const seen = new Set();
            const uniqueStatuses = results.filter(r => {
              if (seen.has(r.user_id)) return false;
              seen.add(r.user_id);
              return true;
            });

            console.log(`📸 Отправляю снимок статусов ${uniqueStatuses.length} друзей`);
            socket.emit('friends_status_snapshot', uniqueStatuses);
            console.log(`✅ Отправлен снимок статусов ${uniqueStatuses.length} друзей пользователю ${incomingUserId}`);
            
            // ⭐ ИСПРАВЛЕНО: Уведомляем ДРУЗЕЙ что этот пользователь онлайн
            uniqueStatuses.forEach(friend => {
              io.to(`user_${friend.user_id}`).emit('user_status_changed', {
                userId: incomingUserId,
                is_online: true,
                timestamp: new Date().toISOString()
              });
            });
          } else {
            console.log(`ℹ️ У пользователя ${incomingUserId} нет друзей, отправляю пустой массив`);
            socket.emit('friends_status_snapshot', []);
          }
        });
      } else {
        console.warn('⚠️ authenticate_socket: user_id не передан');
      }
    });

    // ✅ ПРИСОЕДИНЕНИЕ К ГРУППОВОЙ КОМНАТЕ
    socket.on('join_group_room', (groupId) => {
      socket.join(`group_${groupId}`);
      console.log(`✅ Socket ${socket.id} присоединился к комнате group_${groupId}`);
      
      // ⭐ ИСПРАВЛЕНО: Отправляем snapshot группы новому пользователю
      if (userId) {
        // Получаем последние N сообщений для snapshot
        db.query(`
          SELECT gm.id, gm.sender_id, gm.message, gm.reply_to, gm.media_type, gm.media_url, 
                 gm.duration, gm.caption, gm.created_at, u.username, u.avatar,
                 GROUP_CONCAT(DISTINCT reader_id SEPARATOR ',') as read_by_users
          FROM group_messages gm
          JOIN users u ON gm.sender_id = u.id
          LEFT JOIN group_message_read_status gmrs ON gm.id = gmrs.message_id
          WHERE gm.group_id = ?
          GROUP BY gm.id
          ORDER BY gm.created_at DESC
          LIMIT 50
        `, [groupId], (err, messages) => {
          if (!err && messages) {
            const messagesWithReadStatus = messages.reverse().map(msg => ({
              id: msg.id,
              group_id: groupId,
              sender_id: msg.sender_id,
              message: msg.message,
              reply_to: msg.reply_to,
              media_type: msg.media_type,
              media_url: msg.media_url,
              duration: msg.duration,
              caption: msg.caption,
              sender_username: msg.username,
              sender_avatar: msg.avatar,
              created_at: msg.created_at,
              read_by: msg.read_by_users ? msg.read_by_users.split(',').map(id => parseInt(id)) : []
            }));
            
            // Отправляем snapshot только подключившемуся клиенту
            socket.emit('group_snapshot', {
              group_id: groupId,
              messages: messagesWithReadStatus,
              timestamp: new Date().toISOString()
            });
            console.log(`📸 Отправлен snapshot ${messagesWithReadStatus.length} сообщений для группы ${groupId}`);
          }
        });

        // Отправляем уведомление о присоединении пользователя
        db.query('SELECT username FROM users WHERE id = ?', [userId], (err, results) => {
          if (!err && results && results.length > 0) {
            io.to(`group_${groupId}`).emit('user_joined_group', {
              user_id: userId,
              username: results[0].username,
              group_id: groupId,
              timestamp: new Date().toISOString()
            });
            console.log(`📢 Отправлено уведомление: пользователь ${results[0].username} присоединился к группе ${groupId}`);
          }
        });
      }
    });

    // ✅ ПОКИДАНИЕ ГРУППОВОЙ КОМНАТЫ
    socket.on('leave_group', (groupId) => {
      socket.leave(`group_${groupId}`);
      console.log(`✅ Socket ${socket.id} покинул комнату group_${groupId}`);
      
      // ⭐ ИСПРАВЛЕНО: Отправляем уведомление об оставлении пользователя
      if (userId) {
        db.query('SELECT username FROM users WHERE id = ?', [userId], (err, results) => {
          if (!err && results && results.length > 0) {
            io.to(`group_${groupId}`).emit('user_left_group', {
              user_id: userId,
              username: results[0].username,
              group_id: groupId,
              timestamp: new Date().toISOString()
            });
            console.log(`📢 Отправлено уведомление: пользователь ${results[0].username} покинул группу ${groupId}`);
          }
        });
      }
    });

    // ✅ ОБРАБОТКА ПЕЧАТАНИЯ
    // ✅ ОБРАБОТЧИК: Личное печатание
    socket.on('user_typing', (data) => {
      if (!userId) {
        console.log('⚠️ user_typing: userId не найден в auth');
        return;
      }

      const { to_user_id, from_user_id, from_user_username, is_typing } = data;

      if (!to_user_id) {
        console.log('⚠️ user_typing: to_user_id не предоставлен');
        return;
      }

      // ✅ ВАЖНО: Используем данные от клиента
      const senderId = from_user_id || userId;
      const senderName = from_user_username || 'Пользователь';

      console.log(`📝 user_typing: ${senderId} печатает для ${to_user_id}. is_typing=${is_typing}`);

      // ✅ Отправляем событие ТОЛЬКО получателю через Room
      io.to(`user_${to_user_id}`).emit('user_typing', {
        from_user_id: senderId,
        from_user_username: senderName,
        to_user_id: to_user_id,
        is_typing: is_typing,
        timestamp: new Date().toISOString()
      });

      console.log(`📤 user_typing отправлено: ${senderName}(${senderId}) → user_${to_user_id}`);
    });

    // ✅ ОБРАБОТЧИК: Групповое печатание
    socket.on('group_user_typing', (data) => {
      if (!userId) {
        console.log('⚠️ group_user_typing: userId не найден в auth');
        return;
      }

      const { group_id, is_typing } = data;

      if (!group_id) {
        console.log('⚠️ group_user_typing: group_id не предоставлен');
        return;
      }

      console.log(`📝 Пользователь ${userId} печатает в группе ${group_id}. is_typing=${is_typing}`);

      // Получаем имя пользователя из БД для полной информации
      db.query(
        'SELECT username FROM users WHERE id = ?',
        [userId],
        (err, results) => {
          if (err) {
            console.error('❌ Error fetching username:', err);
            return;
          }

          const username = results && results.length > 0 ? results[0].username : 'Пользователь';

          // Отправляем событие всей группе с полной информацией
          io.to(`group_${group_id}`).emit('group_user_typing', {
            group_id: group_id,
            user_id: userId,
            username: username,
            is_typing: is_typing
          });

          console.log(`📤 Отправлено group_user_typing: ${username} печатает в группе ${group_id}`);
        }
      );
    });

    // ✅ ОБРАБОТЧИК: Отметить сообщение как прочитанное
    socket.on('mark_message_read', (data) => {
      const { message_id } = data;
      
      if (!userId || !message_id) {
        console.warn('⚠️ mark_message_read: missing userId or message_id');
        return;
      }
      
      console.log(`\n${'='.repeat(70)}`);
      console.log(`📖 mark_message_read: user_id=${userId}, message_id=${message_id}`);
      console.log(`${'='.repeat(70)}`);
      
      // Шаг 1: ОПРЕДЕЛЯЕМ ТИП СООБЩЕНИЯ (личное или групповое)
      // Сначала проверяем групповые сообщения
      db.query(
        'SELECT sender_id, group_id FROM group_messages WHERE id = ?',
        [message_id],
        (err, groupResults) => {
          if (err) {
            console.error('❌ Error checking group message:', err);
            return;
          }
          
          // ЕСЛИ НАЙДЕНО В ГРУППОВЫХ СООБЩЕНИЯХ
          if (groupResults && groupResults.length > 0) {
            const { sender_id, group_id } = groupResults[0];
            console.log(`✅ Найдено ГРУППОВОЕ сообщение: sender_id=${sender_id}, group_id=${group_id}`);
            
            // Шаг 2: Обновляем статус в group_message_read_status
            db.query(
              `INSERT INTO group_message_read_status (message_id, reader_id, is_read, read_at)
              VALUES (?, ?, TRUE, CURRENT_TIMESTAMP)
              ON DUPLICATE KEY UPDATE is_read = TRUE, read_at = CURRENT_TIMESTAMP`,
              [message_id, userId],
              (err) => {
                if (err) {
                  console.error('❌ Error updating group_message_read_status:', err);
                  return;
                }
                
                console.log(`✅ group_message_read_status обновлена для message_id=${message_id}, reader_id=${userId}`);
                
                // Шаг 3: ПОЛУЧАЕМ ВСЕХ ЧИТАТЕЛЕЙ сообщения (ВОТ ГЛАВНОЕ ИСПРАВЛЕНИЕ!)
                db.query(
                  `SELECT GROUP_CONCAT(reader_id) as reader_ids, COUNT(*) as reader_count
                  FROM group_message_read_status
                  WHERE message_id = ? AND is_read = TRUE`,
                  [message_id],
                  (err, readerResults) => {
                    if (err) {
                      console.error('❌ Error getting readers:', err);
                      return;
                    }
                    
                    // Преобразуем в массив
                    const reader_ids = readerResults && readerResults[0].reader_ids 
                      ? readerResults[0].reader_ids.split(',').map(id => parseInt(id))
                      : [userId];
                    
                    console.log(`📋 Читатели сообщения ${message_id}: [${reader_ids.join(', ')}]`);
                    
                    // Шаг 4: ОТПРАВЛЯЕМ УВЕДОМЛЕНИЕ ВСЕЙ ГРУППЕ С МАССИВОМ ВСЕХ ЧИТАТЕЛЕЙ
                    console.log(`📬 Отправляю message_read_status_updated в group_${group_id}`);
                    
                    // ✅ ГЛАВНОЕ: Отправляем МАССИВ всех читателей, а не одного
                    io.to(`group_${group_id}`).emit('message_read_status_updated', {
                      message_id: message_id,
                      is_read: true,
                      read_by: reader_ids,  // ✅ МАССИВ со ВСЕМИ читателями - ВОТ ГЛАВНОЕ!
                      sender_id: sender_id,
                      group_id: group_id,
                      updated_by: userId,
                      reader_count: reader_ids.length
                    });
                    
                    console.log(`✅ Уведомление отправлено в группу. Читателей: ${reader_ids.length}\n`);
                  }
                );
              }
            );
            return;
          }
          
          // ЕСЛИ НЕ НАЙДЕНО В ГРУППОВЫХ - ищем в личных
          console.log(`⏳ Не найдено в групповых, проверяю личные сообщения...`);
          
          db.query(
            'SELECT sender_id, receiver_id FROM messages WHERE id = ?',
            [message_id],
            (err, personalResults) => {
              if (err) {
                console.error('❌ Error checking personal message:', err);
                return;
              }
              
              // ЕСЛИ НАЙДЕНО В ЛИЧНЫХ СООБЩЕНИЯХ
              if (personalResults && personalResults.length > 0) {
                const { sender_id, receiver_id } = personalResults[0];
                console.log(`✅ Найдено ЛИЧНОЕ сообщение: sender_id=${sender_id}, receiver_id=${receiver_id}`);
                
                // Шаг 2: Обновляем статус в message_read_status
                db.query(
                  `INSERT INTO message_read_status (message_id, reader_id, is_read, read_at)
                  VALUES (?, ?, TRUE, CURRENT_TIMESTAMP)
                  ON DUPLICATE KEY UPDATE is_read = TRUE, read_at = CURRENT_TIMESTAMP`,
                  [message_id, userId],
                  (err) => {
                    if (err) {
                      console.error('❌ Error updating message_read_status:', err);
                      return;
                    }
                    
                    console.log(`✅ message_read_status обновлена для message_id=${message_id}, reader_id=${userId}`);
                    
                    // ⭐ ГЛАВНОЕ ИСПРАВЛЕНИЕ: Отправляем обновление обоим пользователям ЧЕРЕЗ SOCKET
                    console.log(`📬 Отправляю message_read_status_updated к user_${sender_id} и user_${receiver_id}`);
                    
                    // Отправляем ОБОИМ пользователям в реальном времени
                    io.to(`user_${sender_id}`).emit('message_read_status_updated', {
                      message_id: message_id,
                      is_read: true,
                      read_by: userId,  // ✅ ID читателя
                      sender_id: sender_id,
                      receiver_id: receiver_id,
                      timestamp: new Date().toISOString()
                    });
                    
                    io.to(`user_${receiver_id}`).emit('message_read_status_updated', {
                      message_id: message_id,
                      is_read: true,
                      read_by: userId,
                      sender_id: sender_id,
                      receiver_id: receiver_id,
                      timestamp: new Date().toISOString()
                    });
                    
                    console.log(`✅ Уведомление отправлено обоим пользователям\n`);
                  }
                );
                return;
              }
              
              // НЕ НАЙДЕНО НИ ГДЕ
              console.warn(`❌ Message ${message_id} не найдено ни в личных, ни в групповых сообщениях\n`);
            }
          );
        }
      );
    });

    // ✅ РЕГИСТРАЦИЯ PUSH-ТОКЕНА
    socket.on('register_push_token', (data) => {
      if (!userId) {
        console.log('⚠️ register_push_token: userId не найден');
        return;
      }
      
      const { pushToken, deviceType = 'unknown', deviceName = 'Unknown Device' } = data || {};
      if (pushToken && Expo.isExpoPushToken(pushToken)) {
        console.log(`🔍 Регистрирую токен для user_id=${userId}: ${pushToken.slice(0, 40)}...`);
        
        db.query(
          `INSERT INTO push_tokens (user_id, push_token, device_type, device_name) 
          VALUES (?, ?, ?, ?)
          ON DUPLICATE KEY UPDATE push_token = ?, device_type = ?, device_name = ?, updated_at = CURRENT_TIMESTAMP`,
          [userId, pushToken, deviceType, deviceName, pushToken, deviceType, deviceName],
          (err) => {
            if (err) {
              console.error(`❌ Ошибка сохранения push-токена для user_id=${userId}:`, err.message);
            } else {
              console.log(`✅ Push-токен зарегистрирован для ${userId} в БД: ${pushToken.slice(0, 40)}...`);
            }
          }
        );
      }
    });

    // ✅ ПОДПИСКА НА СТАТУС ПОЛЬЗОВАТЕЛЯ
    socket.on('subscribe_user_status', (targetUserId) => {
      socket.join(`user_status_${targetUserId}`);
      console.log(`✅ Socket ${socket.id} подписался на статус user_${targetUserId}`);
      
      // ⭐ ИСПРАВЛЕНО: Отправляем текущий статус пользователя
      db.query('SELECT is_online, username FROM users WHERE id = ?', [targetUserId], (err, results) => {
        if (!err && results && results.length > 0) {
          socket.emit('user_status_snapshot', {
            userId: targetUserId,
            is_online: results[0].is_online,
            username: results[0].username,
            timestamp: new Date().toISOString()
          });
        }
      });
    });

    // ✅ ОБРАБОТЧИК: Обновление статуса пользователя
    socket.on('user_status', (data) => {
      const { user_id, is_online } = data;
      
      console.log(`📊 user_status: user_id=${user_id}, is_online=${is_online}`);
      
      // ⭐ ИСПРАВЛЕНО: Отправляем через room user_status_${user_id}
      io.to(`user_status_${user_id}`).emit('user_status_changed', {
        userId: user_id,
        is_online: is_online,
        timestamp: new Date().toISOString()
      });
      
      // Также отправляем друзьям через их личные комнаты
      if (user_id) {
        db.query(`
          SELECT DISTINCT
            CASE 
              WHEN f.user_id = ? THEN f.friend_id 
              ELSE f.user_id 
            END as friend_id
          FROM friends f
          WHERE (f.user_id = ? OR f.friend_id = ?) AND f.status = 'accepted'
        `, [user_id, user_id, user_id], (err, friends) => {
          if (friends && friends.length > 0) {
            friends.forEach(friend => {
              io.to(`user_${friend.friend_id}`).emit('user_status_changed', {
                userId: user_id,
                is_online: is_online,
                timestamp: new Date().toISOString()
              });
            });
          }
        });
      }
      
      console.log(`✅ Отправлено 'user_status_changed' для пользователя ${user_id} → ${is_online ? 'онлайн' : 'офлайн'}`);
    });

    // ========== CALL HANDLERS ==========
    
    // ========== ИНИЦИАЦИЯ ЗВОНКА (15 СЕК ДОЗВОН) ==========
    socket.on('call_initiate', async (data) => {
      const { from_user_id, to_user_id, call_type, from_user } = data;
      const callId = `${from_user_id}_${to_user_id}_${Date.now()}`;
      
      console.log(`\n${'='.repeat(70)}`);
      console.log(`📞 ЗВОНОК ИНИЦИИРОВАН`);
      console.log(`   От: пользователь ${from_user_id} (${from_user?.username || 'Unknown'})`);
      console.log(`   Кому: пользователь ${to_user_id}`);
      console.log(`   Тип: ${call_type}`);
      console.log(`   Call ID: ${callId}`);
      console.log(`   Socket ID отправителя: ${socket.id}`);
      
      // ПРОВЕРКА: есть ли from_user_id?
      if (!from_user_id || !to_user_id) {
        console.error(`❌ ОШИБКА: Отсутствуют from_user_id (${from_user_id}) или to_user_id (${to_user_id})`);
        socket.emit('call_error', {
          message: 'Invalid call data',
          details: 'from_user_id and to_user_id are required'
        });
        return;
      }
      
      // Сохраняем информацию о звонке
      activeCalls.set(callId, {
        call_id: callId,
        from_user_id,
        to_user_id,
        call_type,
        status: 'ringing',
        started_at: new Date(),
        socket_from: socket.id
      });
      
      // Проверяем: онлайн ли получатель?
      const recipientSocketIds = getUserSocketIds(to_user_id);
      
      console.log(`📊 Статус подключений в системе:`);
      console.log(`   Всего пользователей онлайн: ${userSockets.size}`);
      console.log(`   Список: ${Array.from(userSockets.entries())
        .map(([uid, ids]) => `${uid}(${Array.isArray(ids) ? ids.length : 0})`)
        .join(', ') || 'пусто'}`);
      console.log(`   Socket ID получателя ${to_user_id}: ${recipientSocketIds.length > 0 ? recipientSocketIds.join(', ') : 'НЕ НАЙДЕН'}`);
      
      if (recipientSocketIds.length > 0) {
        // ✅ ОНЛАЙН - отправляем WebSocket
        console.log(`✅ НАЙДЕН ОНЛАЙН - отправляем WebSocket сигнал`);
        
        recipientSocketIds.forEach((socketId) => {
          io.to(socketId).emit('call_incoming', {
            call_id: callId,
            from_user_id,
            call_type,
            from_user: {
              id: from_user_id,
              username: from_user?.username || 'Пользователь'
            }
          });
        });
        
        console.log(`✅ Сигнал отправлен на sockets: ${recipientSocketIds.join(', ')}`);

        // 🔔 Дожидаемся ответа 15 секунд
        setTimeout(() => {
          const call = activeCalls.get(callId);
          if (call && call.status === 'ringing') {
            console.log(`⏰ Пропущенный вызов (15 сек): ${callId}`);
            
            // Уведомляем звонящего
            io.to(socket.id).emit('call_missed', {
              call_id: callId,
              to_user_id
            });
            
            activeCalls.delete(callId);
          }
        }, 15000);
        
      } else {
        // ❌ ОФФЛАЙН - ИЩЕМ PUSH-ТОКЕН В БД
        console.log(`❌ ОФФЛАЙН - пусконичатель ${to_user_id} не найден в userSockets`);
        console.log(`⏳ Ищем push-токен в БД...`);
        
        try {
          const pushToken = await getPushTokenFromDB(to_user_id);
          
          if (pushToken) {
            try {
              // Формируем сообщение
              const message = {
                to: pushToken,
                sound: 'default',
                title: `${call_type === 'video' ? '📹' : '📞'} ${from_user?.username || 'Пользователь'}`,
                body: `${call_type === 'video' ? 'Видео' : 'Аудио'} звонок`,
                data: {
                  type: 'incoming_call',
                  callType: call_type,
                  callerId: from_user_id,
                  callerName: from_user?.username || 'Пользователь',
                  callId: callId,
                },
                badge: 1,
                priority: 'high',
              };

              // Отправляем push
              const tickets = await expo.sendPushNotificationsAsync([message]);
              console.log(`📬 Push отправлен!`);
              console.log(`   Заголовок: "${message.title}"`);
              console.log(`   Токен: ${pushToken.slice(0, 40)}...`);
              
              // Уведомляем звонящего что push отправлен
              io.to(socket.id).emit('call_user_offline_push_sent', {
                message: 'Push-уведомление отправлено',
                callId
              });

              // ⏳ ДОЖИДАЕМСЯ ОТВЕТА 15 СЕКУНД (ПОКА ПОЛЬЗОВАТЕЛЬ ОТКРОЕТ УВЕДОМЛЕНИЕ)
              const missedCallTimeout = setTimeout(() => {
                const call = activeCalls.get(callId);
                if (call && call.status === 'ringing') {
                  console.log(`⏰ Пропущенный вызов (15 сек): ${callId}`);
                  
                  // Уведомляем звонящего
                  io.to(socket.id).emit('call_missed', {
                    call_id: callId,
                    to_user_id
                  });
                  
                  activeCalls.delete(callId);
                }
              }, 15000);

              // Если пользователь принял вызов - очищаем таймер
              const handleCallAccepted = (callData) => {
                if (callData.call_id === callId) {
                  clearTimeout(missedCallTimeout);
                  socket.off('call_accepted_internal', handleCallAccepted);
                }
              };
              socket.on('call_accepted_internal', handleCallAccepted);

            } catch (error) {
              console.error(`❌ Ошибка отправки push:`, error.message);
              io.to(socket.id).emit('call_user_offline', {
                message: 'Ошибка отправки push-уведомления'
              });
            }
          } else {
            console.log(`❌ Push-токен не найден для ${to_user_id} в БД`);
            console.log(`📱 Пользователь ${to_user_id} ДОЛЖЕН зарегистрировать push-токен через POST /api/users/push-token`);
            
            io.to(socket.id).emit('call_user_offline', {
              message: `Пользователь ${to_user_id} оффлайн и не зарегистрировал push-токен`
            });
          }
        } catch (error) {
          console.error(`❌ Ошибка при поиске push-токена:`, error.message);
          io.to(socket.id).emit('call_user_offline', {
            message: 'Ошибка при поиске токена'
          });
        }
      }
    });

    // ПРИНЯТИЕ ЗВОНКА
    socket.on('call_accepted', (data) => {
      const { call_id, to_user_id } = data;
      const call = activeCalls.get(call_id);
      
      if (call) {
        call.status = 'connected';
        call.connected_at = new Date();
        
        console.log(`✅ Принят: ${call.from_user_id} ↔ ${to_user_id}\n`);
        
        // Уведомляем звонящего
        io.to(call.socket_from).emit('call_accepted', {
          call_id,
          from_user_id: call.from_user_id,
          to_user_id
        });
        
        // Сигнал для таймера пропущенного звонка
        socket.emit('call_accepted_internal', { call_id });
      }
    });

    // ОТКЛОНЕНИЕ ЗВОНКА
    socket.on('call_rejected', (data) => {
      const { call_id, to_user_id } = data;
      const call = activeCalls.get(call_id);
      
      if (call) {
        console.log(`❌ Отклонен: ${call.from_user_id}\n`);
        
        io.to(call.socket_from).emit('call_rejected', {
          call_id,
          reason: 'user_declined'
        });
        
        activeCalls.delete(call_id);
      }
    });

    // ЗАВЕРШЕНИЕ ЗВОНКА
    socket.on('call_end', (data) => {
      const { to_user_id } = data;
      
      // Находим активный звонок
      let callToEnd = null;
      for (const [callId, call] of activeCalls) {
        if ((call.from_user_id === userId && call.to_user_id === to_user_id) ||
            (call.from_user_id === to_user_id && call.to_user_id === userId)) {
          callToEnd = { callId, call };
          break;
        }
      }
      
      if (callToEnd) {
        const duration = Math.floor((new Date() - callToEnd.call.started_at) / 1000);
        console.log(`📴 Завершен (${duration}сек)\n`);
        
        // Уведомляем другого пользователя
        const otherUserId = callToEnd.call.from_user_id === userId 
          ? callToEnd.call.to_user_id 
          : callToEnd.call.from_user_id;
        
        const otherSocketIds = getUserSocketIds(otherUserId);
        if (otherSocketIds.length > 0) {
          otherSocketIds.forEach((socketId) => {
            io.to(socketId).emit('call_ended', {
              call_id: callToEnd.callId,
              duration
            });
          });
        }
        
        activeCalls.delete(callToEnd.callId);
      }
    });

    // ПЕРЕКЛЮЧЕНИЕ МУТЕ
    socket.on('call_mute_toggle', (data) => {
      const { is_muted, to_user_id } = data;
      
      const otherSocketIds = getUserSocketIds(to_user_id);
      if (otherSocketIds.length > 0) {
        otherSocketIds.forEach((socketId) => {
          io.to(socketId).emit('call_mute_status', {
            is_muted
          });
        });
      }
    });

    // ✅ НОВЫЙ ОБРАБОТЧИК: Пользователь открыл чат
    socket.on('set_active_chat', (data) => {
      if (!userId) {
        console.log('⚠️ set_active_chat: user_id не найден в auth');
        return;
      }
      
      const { chat_id, chat_type, timestamp } = data;
      activeChats.set(userId, {
        chatId: chat_id,
        chatType: chat_type,
        timestamp: timestamp ? new Date(timestamp) : new Date()
      });
      
      console.log(`📍 Активный чат пользователя ${userId}: ${chat_type} чат_id=${chat_id}`);
    });
    
    // ✅ НОВЫЙ ОБРАБОТЧИК: Пользователь вышел из чата
    socket.on('clear_active_chat', (data) => {
      if (!userId) {
        console.log('⚠️ clear_active_chat: user_id не найден в auth');
        return;
      }
      
      activeChats.delete(userId);
      console.log(`📭 Очищен активный чат пользователя ${userId}`);
    });

    // ✅ ПРИСОЕДИНЕНИЕ К PERSONAL ROOM
    socket.on('join_personal_room', (other_user_id) => {
      if (userId) {
        socket.join(`user_${userId}`);
        console.log(`✅ Socket ${socket.id} присоединился к personal room: user_${userId}`);
      }
    });

    // ✅ ПОКИДАНИЕ PERSONAL ROOM
    socket.on('leave_personal_room', (other_user_id) => {
      if (userId) {
        socket.leave(`user_${userId}`);
        console.log(`✅ Socket ${socket.id} покинул personal room: user_${userId}`);
      }
    });

    // ОТКЛЮЧЕНИЕ
    socket.on('disconnect', () => {
      console.log('❌ Socket отключен:', socket.id);
      
      if (userId) {
        // Очищаем активный чат при отключении
        activeChats.delete(userId);
        console.log(`📭 Пользователь ${userId} отключился, активный чат очищен`);
        
        // Удаляем сокет из всех маппингов
        const userKey = String(userId);
        const sockets = userSockets.get(userKey);
        if (sockets) {
          const index = sockets.indexOf(socket.id);
          if (index > -1) {
            sockets.splice(index, 1);
            
            // Если нет больше сокетов - ставим офлайн и сохраняем время
            if (sockets.length === 0) {
              userSockets.delete(userKey);
              // ⭐ ИСПРАВЛЕНО: Обновляем статус в БД
              db.query('UPDATE users SET is_online = 0, last_seen = NOW() WHERE id = ?', [userId], (err) => {
                if (err) console.error('Ошибка обновления статуса:', err);
                else console.log(`✅ Пользователь ${userId} установлен офлайн`);
              });
              
              // ⭐ ИСПРАВЛЕНО: Отправляем через room + broadcast друзьям
              io.to(`user_status_${userId}`).emit('user_status_changed', { 
                userId, 
                is_online: false,
                timestamp: new Date().toISOString()
              });
              
              // Отправляем друзьям
              db.query(`
                SELECT DISTINCT
                  CASE 
                    WHEN f.user_id = ? THEN f.friend_id 
                    ELSE f.user_id 
                  END as friend_id
                FROM friends f
                WHERE (f.user_id = ? OR f.friend_id = ?) AND f.status = 'accepted'
              `, [userId, userId, userId], (err, friends) => {
                if (friends && friends.length > 0) {
                  friends.forEach(friend => {
                    io.to(`user_${friend.friend_id}`).emit('user_status_changed', {
                      userId: userId,
                      is_online: false,
                      timestamp: new Date().toISOString()
                    });
                  });
                }
              });
            }
          }
        }
        
        // Завершаем его активные звонки
        for (const [callId, call] of activeCalls) {
          if (call.from_user_id === userId || call.to_user_id === userId) {
            const otherUserId = call.from_user_id === userId 
              ? call.to_user_id 
              : call.from_user_id;
            
            const otherSocketIds = getUserSocketIds(otherUserId);
            if (otherSocketIds.length > 0) {
              otherSocketIds.forEach((socketId) => {
                io.to(socketId).emit('call_ended', {
                  call_id: callId,
                  reason: 'user_disconnected'
                });
              });
            }
            
            activeCalls.delete(callId);
          }
        }
      }
    });
  });

  // ⭐ Email верификация - Шаг 1: Инициирование регистрации с Email
  app.post('/api/register', async (req, res) => {
    const { username, email, password } = req.body;

    console.log('\n' + '='.repeat(70));
    console.log('📝 Запрос регистрации:', { username, email });
    console.log('='.repeat(70));

    try {
      // Валидация входных данных
      if (!username || !email || !password) {
        console.log('❌ Валидация: не все поля заполнены');
        return res.status(400).json({ 
          error: 'Все поля обязательны' 
        });
      }

      console.log('✅ Валидация полей пройдена');

      // Проверка минимальной длины
      if (username.length < 3 || password.length < 6) {
        console.log('❌ Ошибка: имя < 3 символов или пароль < 6 символов');
        return res.status(400).json({ 
          error: 'Имя должно быть минимум 3 символа, пароль 6 символов' 
        });
      }

      // Проверка формата email
      const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      if (!emailRegex.test(email)) {
        console.log('❌ Ошибка: некорректный формат email');
        return res.status(400).json({ 
          error: 'Некорректный формат email' 
        });
      }

      // Проверка, не занят ли email
      db.query(
        'SELECT id FROM users WHERE email = ?',
        [email],
        async (err, results) => {
          if (err) {
            console.error('❌ Ошибка БД при проверке email:', err);
            return res.status(500).json({ error: 'Ошибка БД' });
          }

          if (results.length > 0) {
            console.log('❌ Email уже зарегистрирован:', email);
            return res.status(400).json({ 
              error: 'Email уже зарегистрирован' 
            });
          }

          console.log('✅ Email не занят:', email);

          // Проверка, не занято ли имя пользователя
          db.query(
            'SELECT id FROM users WHERE username = ?',
            [username],
            async (err, results) => {
              if (err) {
                console.error('❌ Ошибка БД при проверке username:', err);
                return res.status(500).json({ error: 'Ошибка БД' });
              }

              if (results.length > 0) {
                console.log('❌ Username уже занят:', username);
                return res.status(400).json({ 
                  error: 'Это имя пользователя уже занято' 
                });
              }

              console.log('✅ Username не занят:', username);

              // Генерируем код и session token
              const code = generateVerificationCode();
              const sessionToken = generateSessionToken();
              const expiresAt = new Date(Date.now() + 5 * 60 * 1000); // 5 минут

              console.log(`⏱️  Срок действия кода: ${expiresAt.toISOString()}`);
              console.log('💾 Сохраняю в БД...');

              // Удаляем старые коды для этого email
              db.query(
                'DELETE FROM verification_codes WHERE email = ? AND is_verified = FALSE',
                [email],
                async (err) => {
                  if (err) console.error('❌ Ошибка удаления старых кодов:', err);

                  // Сохраняем в БД
                  db.query(
                    `INSERT INTO verification_codes (email, phone, code, session_token, expires_at) VALUES (?, ?, ?, ?, ?)`,
                    [email, '', code, sessionToken, expiresAt],
                    async (err) => {
                      if (err) {
                        console.error('❌ Ошибка вставки кода в БД:', err);
                        return res.status(500).json({ error: 'Ошибка сервера' });
                      }

                      console.log('✅ Записано в БД успешно');

                      // ⚡ Отправляем email В ФОНЕ (не ждем ответа)
                      console.log('📧 Ставлю email в очередь отправки...');
                      sendVerificationEmail(email, code, username).catch(err => {
                        console.error('❌ Ошибка отправки email в фоне:', err.message);
                      });

                      // ✅ Сразу отправляем успешный ответ клиенту (не ждем email)
                      console.log('✅ Отправляю ответ клиенту');
                      console.log('🎉 Регистрация инициирована успешно\n');
                      res.json({
                        success: true,
                        message: 'Код подтверждения отправлен на ваш email',
                        sessionToken: sessionToken
                      });
                    }
                  );
                }
              );
            }
          );
        }
      );
    } catch (error) {
      console.error('❌ Ошибка регистрации:', error);
      console.log('');
      res.status(500).json({ 
        error: 'Ошибка сервера. Попробуйте позже.' 
      });
    }
  });

  // ⭐ Email верификация - Шаг 1: Инициирование регистрации с Email
  app.post('/api/register', async (req, res) => {
    const { username, email, password } = req.body;

    try {
      // Валидация входных данных
      if (!username || !email || !password) {
        return res.status(400).json({ 
          error: 'Все поля обязательны' 
        });
      }

      // Проверка минимальной длины
      if (username.length < 3 || password.length < 6) {
        return res.status(400).json({ 
          error: 'Имя должно быть минимум 3 символа, пароль 6 символов' 
        });
      }

      // Проверка формата email
      const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      if (!emailRegex.test(email)) {
        return res.status(400).json({ 
          error: 'Некорректный формат email' 
        });
      }

      // Проверка, не занят ли email
      db.query(
        'SELECT id FROM users WHERE email = ?',
        [email],
        async (err, results) => {
          if (err) {
            return res.status(500).json({ error: 'Ошибка БД' });
          }

          if (results.length > 0) {
            return res.status(400).json({ 
              error: 'Email уже зарегистрирован' 
            });
          }

          // Проверка, не занято ли имя пользователя
          db.query(
            'SELECT id FROM users WHERE username = ?',
            [username],
            async (err, results) => {
              if (err) {
                return res.status(500).json({ error: 'Ошибка БД' });
              }

              if (results.length > 0) {
                return res.status(400).json({ 
                  error: 'Это имя пользователя уже занято' 
                });
              }

              // Генерируем код и session token
              const code = generateVerificationCode();
              const sessionToken = generateSessionToken();
              const expiresAt = new Date(Date.now() + 5 * 60 * 1000); // 5 минут

              // Удаляем старые коды для этого email
              db.query(
                'DELETE FROM verification_codes WHERE email = ? AND is_verified = FALSE',
                [email],
                async (err) => {
                  if (err) console.error('❌ Ошибка удаления старых кодов:', err);

                  // Сохраняем в БД
                  db.query(
                    `INSERT INTO verification_codes (email, phone, code, session_token, expires_at) VALUES (?, ?, ?, ?, ?)`,
                    [email, '', code, sessionToken, expiresAt],
                    async (err) => {
                      if (err) {
                        console.error('❌ Ошибка вставки кода:', err);
                        return res.status(500).json({ error: 'Ошибка сервера' });
                      }

                      // Отправляем email с кодом
                      try {
                        await sendVerificationEmail(email, code, username);
                        
                        res.json({
                          success: true,
                          message: 'Код подтверждения отправлен на ваш email',
                          sessionToken: sessionToken
                        });
                      } catch (error) {
                        console.error('❌ Ошибка отправки email:', error);
                        res.status(500).json({ error: 'Не удалось отправить email' });
                      }
                    }
                  );
                }
              );
            }
          );
        }
      );
    } catch (error) {
      console.error('❌ Ошибка регистрации:', error);
      res.status(500).json({ 
        error: 'Ошибка сервера. Попробуйте позже.' 
      });
    }
  });

  // ⭐ Email верификация - Шаг 2: Подтверждение кода и создание аккаунта
  app.post('/api/verify-code', async (req, res) => {
    const { email, code, username, password } = req.body;

    console.log('\n' + '='.repeat(70));
    console.log('🔐 Запрос подтверждения кода:', { email, code });
    console.log('='.repeat(70));

    try {
      // Валидация
      if (!email || !code || !username || !password) {
        console.log('❌ Валидация: не все поля заполнены');
        return res.status(400).json({ 
          error: 'Все поля обязательны' 
        });
      }

      console.log('✅ Валидация пройдена');

      // Поиск записи верификации
      db.query(
        `SELECT * FROM verification_codes 
         WHERE email = ? AND is_verified = FALSE`,
        [email],
        async (err, results) => {
          if (err) {
            console.error('❌ Ошибка БД:', err);
            return res.status(500).json({ error: 'Ошибка БД' });
          }

          if (results.length === 0) {
            console.log('❌ Запись верификации не найдена для:', email);
            return res.status(400).json({ 
              error: 'Запрос на верификацию не найден' 
            });
          }

          const record = results[0];
          console.log('✅ Запись верификации найдена');
          console.log(`   Попыток: ${record.attempts}/${record.max_attempts}`);
          console.log(`   Срок действия: ${record.expires_at}`);

          // Проверка истечения времени
          if (new Date() > new Date(record.expires_at)) {
            console.log('❌ Код истёк');
            // Удаляем истекшую запись
            db.query(
              'DELETE FROM verification_codes WHERE id = ?',
              [record.id]
            );
            
            return res.status(400).json({ 
              error: 'Код истек. Запросите новый код' 
            });
          }

          // Проверка максимального числа попыток
          if (record.attempts >= record.max_attempts) {
            console.log('❌ Превышено максимальное количество попыток');
            return res.status(400).json({ 
              error: 'Превышено максимальное количество попыток. Запросите новый код' 
            });
          }

          console.log(`🔍 Проверяю код: ${code} === ${record.code}`);

          // Проверка кода
          if (record.code !== code.toString()) {
            console.log('❌ Неверный код! Увеличиваю счётчик попыток');
            // Увеличиваем счетчик попыток
            db.query(
              'UPDATE verification_codes SET attempts = attempts + 1 WHERE id = ?',
              [record.id],
              () => {
                const remainingAttempts = record.max_attempts - record.attempts - 1;
                console.log(`   Осталось попыток: ${remainingAttempts}`);
              }
            );
            
            return res.status(400).json({ 
              error: 'Неверный код подтверждения',
              remainingAttempts: record.max_attempts - record.attempts - 1
            });
          }

          console.log('✅ Код верный!');
          console.log('👤 Создаю пользователя...');

          // Код верный - создаем пользователя
          try {
            const hashedPassword = await bcrypt.hash(password, 10);

            db.query(
              `INSERT INTO users (username, email, password) 
               VALUES (?, ?, ?)`,
              [username, email, hashedPassword],
              (err, result) => {
                if (err) {
                  console.error('❌ Ошибка создания пользователя:', err);
                  if (err.code === 'ER_DUP_ENTRY') {
                    if (err.sqlMessage.includes('username')) {
                      return res.status(400).json({ error: 'Пользователь с таким именем уже существует' });
                    }
                    if (err.sqlMessage.includes('email')) {
                      return res.status(400).json({ error: 'Пользователь с таким email уже существует' });
                    }
                  }
                  return res.status(500).json({ error: 'Ошибка создания аккаунта' });
                }

                const userId = result.insertId;
                console.log(`✅ Пользователь создан. ID: ${userId}`);

                // Отмечаем верификацию как завершенную
                db.query(
                  'UPDATE verification_codes SET is_verified = TRUE WHERE id = ?',
                  [record.id],
                  () => console.log('✅ Верификация отмечена как завершённая')
                );

                // Создаем JWT токен
                const token = jwt.sign(
                  { id: userId, email: email, username: username },
                  process.env.JWT_SECRET || 'your_jwt_secret',
                  { expiresIn: '30d' }
                );

                console.log(`🎫 JWT токен сгенерирован (30 дней)`);

                // Получаем полные данные пользователя
                db.query(
                  'SELECT id, username, email, avatar, bio, status, cardColor, is_banned FROM users WHERE id = ?',
                  [userId],
                  (err, userData) => {
                    if (err) {
                      console.error('❌ Ошибка получения данных пользователя:', err);
                      return res.status(500).json({ error: 'Ошибка сервера' });
                    }

                    console.log('🎉 Аккаунт успешно создан и активирован\n');
                    res.json({
                      success: true,
                      message: 'Аккаунт успешно создан',
                      token: token,
                      user: userData[0]
                    });
                  }
                );
              }
            );
          } catch (hashError) {
            console.error('❌ Ошибка хеширования пароля:', hashError);
            console.log('');
            res.status(500).json({ error: 'Ошибка сервера' });
          }
        }
      );
    } catch (error) {
      console.error('❌ Ошибка верификации кода:', error);
      console.log('');
      res.status(500).json({ 
        error: 'Ошибка сервера' 
      });
    }
  });

  // ⭐ Email верификация - Переотправка кода
  app.post('/api/resend-verification-code', async (req, res) => {
    const { email } = req.body;

    console.log('\n' + '='.repeat(70));
    console.log('🔄 Запрос переотправки кода:', { email });
    console.log('='.repeat(70));

    try {
      // Валидация
      if (!email) {
        console.log('❌ Валидация: email не заполнен');
        return res.status(400).json({ 
          error: 'Email обязателен' 
        });
      }

      // Поиск активной записи верификации
      db.query(
        `SELECT * FROM verification_codes 
         WHERE email = ? AND is_verified = FALSE`,
        [email],
        async (err, results) => {
          if (err) {
            console.error('❌ Ошибка БД:', err);
            return res.status(500).json({ error: 'Ошибка БД' });
          }

          if (results.length === 0) {
            console.log('❌ Запись верификации не найдена для:', email);
            return res.status(400).json({ 
              error: 'Запрос на верификацию не найден' 
            });
          }

          const record = results[0];

          console.log('✅ Запись верификации найдена');
          console.log('🔄 Переотправка кода...');

          // Генерируем новый код
          const newCode = generateVerificationCode();
          const expiresAt = new Date(Date.now() + 5 * 60 * 1000); // Новый 5-минутный таймер

          console.log(`⏱️  Новый срок действия: ${expiresAt.toISOString()}`);
          console.log('💾 Обновляю БД...');

          // Обновляем запись
          db.query(
            `UPDATE verification_codes 
             SET code = ?, expires_at = ?, attempts = 0 
             WHERE email = ?`,
            [newCode, expiresAt, email],
            async (err) => {
              if (err) {
                console.error('❌ Ошибка обновления кода:', err);
                console.log('');
                return res.status(500).json({ error: 'Ошибка сервера' });
              }

              console.log('✅ БД обновлена, счётчик попыток сброшен');

              // ⚡ Отправляем email В ФОНЕ (не ждем ответа)
              console.log('📧 Ставлю email в очередь отправки...');
              sendVerificationEmail(email, newCode, email.split('@')[0]).catch(err => {
                console.error('❌ Ошибка отправки email в фоне:', err.message);
              });

              // ✅ Сразу отправляем успешный ответ клиенту (не ждем email)
              console.log('✅ Отправляю ответ клиенту');
              console.log('🎉 Код переотправлен успешно\n');
              res.json({
                success: true,
                message: 'Новый код отправлен на ваш email'
              });
            }
          );
        }
      );
    } catch (error) {
      console.error('❌ Ошибка повторной отправки кода:', error);
      console.log('');
      res.status(500).json({ 
        error: 'Ошибка сервера' 
      });
    }
  });

  // Вход
  app.post('/api/login', (req, res) => {
    const { email, password } = req.body;
    
    console.log('Данные входа:', { email, password });
    
    if (!email || !password) {
      return res.status(400).json({ error: 'Все поля обязательны' });
    }
    
    db.query('SELECT * FROM users WHERE email = ?', [email], async (err, results) => {
      if (err) {
        console.error('Ошибка БД:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }
      
      if (results.length === 0) {
        return res.status(400).json({ error: 'Пользователь не найден' });
      }
      
      const user = results[0];
      
      // ✅ ПРОВЕРЯЕМ: Не забанен ли пользователь
      if (user.is_banned) {
        // Проверяем, истекла ли дата разбана
        if (user.banned_until && new Date() > new Date(user.banned_until)) {
          // Разбаниваем автоматически
          db.query(
            'UPDATE users SET is_banned = FALSE, ban_reason = NULL, banned_at = NULL, banned_until = NULL, banned_by = NULL WHERE id = ?',
            [user.id],
            (err) => {
              if (err) console.error('❌ Error auto-unbanning user:', err);
              else console.log(`✅ Пользователь ${user.id} автоматически разбанен (истекла дата разбана)`);
            }
          );
          // Позволяем войти - продолжаем дальше
        } else {
          console.log(`❌ Попытка входа забаненного пользователя: ${user.username}`);
          
          // Форматируем информацию о бане
          let banMessage = 'Ваш аккаунт заблокирован';
          let remainingDays = null;
          
          if (user.banned_until) {
            const now = new Date();
            const bannedUntil = new Date(user.banned_until);
            const daysRemaining = Math.ceil((bannedUntil - now) / (1000 * 60 * 60 * 24));
            
            if (daysRemaining > 0) {
              remainingDays = daysRemaining;
              banMessage = `Ваш аккаунт заблокирован на ${daysRemaining} дней`;
            }
          }
          
          return res.status(403).json({ 
            error: banMessage,
            reason: user.ban_reason || 'Нарушение правил сообщества',
            banned_at: user.banned_at,
            banned_until: user.banned_until,
            remainingDays: remainingDays,
            is_permanent: !user.banned_until
          });
        }
      }
      
      const validPassword = await bcrypt.compare(password, user.password);
      
      if (!validPassword) {
        return res.status(400).json({ error: 'Неверный пароль' });
      }
      
      const token = jwt.sign({ id: user.id, username: user.username }, process.env.JWT_SECRET);
      res.json({ 
        token, 
        user: { 
          id: user.id, 
          username: user.username, 
          email: user.email, 
          avatar: user.avatar, 
          bio: user.bio, 
          status: user.status,
          is_admin: user.is_admin || false,
          is_banned: user.is_banned || false
        } 
      });
    });
  });

  // Получить группы пользователя
  app.get('/api/groups', authenticateToken, (req, res) => {
    const userId = req.user.id;
    
    db.query(`
      SELECT 
        g.*,
        COUNT(gm.user_id) as member_count,
        (SELECT gm2.message FROM group_messages gm2
        WHERE gm2.group_id = g.id
        ORDER BY gm2.created_at DESC
        LIMIT 1) as last_message,
        (SELECT gm2.id FROM group_messages gm2
        WHERE gm2.group_id = g.id
        ORDER BY gm2.created_at DESC
        LIMIT 1) as last_message_id,
        (SELECT gm2.created_at FROM group_messages gm2
        WHERE gm2.group_id = g.id
        ORDER BY gm2.created_at DESC
        LIMIT 1) as last_message_time,
        (SELECT COALESCE(MAX(CASE WHEN gmrs.is_read = TRUE THEN 1 ELSE 0 END), 0)
        FROM group_messages gm2
        LEFT JOIN group_message_read_status gmrs ON gm2.id = gmrs.message_id AND gmrs.reader_id = ?
        WHERE gm2.group_id = g.id
        ORDER BY gm2.created_at DESC
        LIMIT 1) as last_message_is_read,
        (SELECT COUNT(*) FROM group_messages gm3
        WHERE gm3.group_id = g.id AND NOT EXISTS (
          SELECT 1 FROM group_message_read_status
          WHERE message_id = gm3.id AND reader_id = ? AND is_read = TRUE
        )) as unread_count
      FROM groups g 
      LEFT JOIN group_members gm ON g.id = gm.group_id 
      WHERE g.id IN (
        SELECT group_id FROM group_members WHERE user_id = ?
      ) 
      GROUP BY g.id
      ORDER BY last_message_time DESC
    `, [userId, userId, userId], (err, results) => {
      if (err) {
        console.error('Ошибка получения групп:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }
      
      // Преобразуем результаты
      const groups = (results || []).map(group => ({
        ...group,
        last_message_is_read: group.last_message_is_read === 1,  // ✅ НОВОЕ: Статус прочтения
        unread_count: group.unread_count || 0
      }));
      
      console.log(`✅ Loaded ${groups.length} groups for user ${userId}`);
      
      res.json(groups);
    });
  });

  // Создать группу
  app.post('/api/groups', authenticateToken, (req, res) => {
    const { name, description, avatar, members = [] } = req.body;
    
    console.log(`[DEBUG] Creating group: name="${name}", members=${JSON.stringify(members)}`);
    
    if (!name || name.trim().length === 0) {
      return res.status(400).json({ error: 'Название группы обязательно' });
    }
    
    db.query(
      'INSERT INTO groups (name, description, avatar, created_by) VALUES (?, ?, ?, ?)',
      [name, description || null, avatar || null, req.user.id],
      (err, result) => {
        if (err) {
          console.error('[ERROR] Failed to create group:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        
        const groupId = result.insertId;
        console.log(`[DEBUG] Group created with ID ${groupId}`);
        
        // Добавить создателя как админа
        db.query(
          'INSERT INTO group_members (group_id, user_id, role) VALUES (?, ?, ?)',
          [groupId, req.user.id, 'admin'],
          (err) => {
            if (err) {
              console.error('[ERROR] Failed to add creator as admin:', err);
              return res.status(500).json({ error: 'Ошибка сервера' });
            }
            
            // Добавить участников
            if (members && members.length > 0) {
              console.log(`[DEBUG] Adding ${members.length} members to group ${groupId}`);
              const memberValues = members.map(memberId => [groupId, memberId]);
              db.query(
                'INSERT INTO group_members (group_id, user_id) VALUES ?',
                [memberValues],
                (err) => {
                  if (err) {
                    console.error('[ERROR] Failed to add members:', err);
                  } else {
                    console.log(`[DEBUG] Successfully added ${members.length} members`);
                  }
                }
              );
            }
            
            console.log(`[DEBUG] Group ${groupId} created successfully`);
            
            // 📡 НОВОЕ: Отправить Socket событие всем участникам группы
            const groupData = {
              id: groupId,
              name,
              description,
              avatar,
              created_by: req.user.id,
              members: [req.user.id, ...members]
            };
            
            // Отправить создателю
            console.log(`📡 [SOCKET] Отправляю group_created событие создателю ${req.user.id}`);
            io.to(`user_${req.user.id}`).emit('group_created', groupData);
            
            // Отправить каждому участнику
            if (members && members.length > 0) {
              members.forEach(memberId => {
                console.log(`📡 [SOCKET] Отправляю group_created событие участнику ${memberId}`);
                io.to(`user_${memberId}`).emit('group_created', groupData);
              });
            }
            
            res.json({ 
              id: groupId, 
              name, 
              description, 
              avatar, 
              created_by: req.user.id, 
              message: 'Группа создана' 
            });
          }
        );
      }
    );
  });

  // Получить статус пользователя
  app.get('/api/users/:userId/status', authenticateToken, (req, res) => {
    const { userId } = req.params;
    
    db.query('SELECT is_online, last_seen FROM users WHERE id = ?', [userId], (err, results) => {
      if (err) {
        console.error('Database error:', err);
        return res.status(500).json({ error: 'Server error' });
      }
      
      if (results.length === 0) {
        return res.status(404).json({ error: 'User not found' });
      }
      
      res.json({ 
        is_online: results[0].is_online || false,
        last_seen: results[0].last_seen  // ⭐ ДОБАВЛЕНО
      });
    });
  });

  // Обновить статус пользователя - ИСПРАВЛЕННАЯ ВЕРСИЯ
  app.post('/api/users/status', authenticateToken, (req, res) => {
    try {
      // Гарантируем что req.body существует
      if (!req.body || typeof req.body !== 'object') {
        console.warn('⚠️ req.body is not an object:', typeof req.body, req.body);
        req.body = {};
      }
      
      const { is_online } = req.body;
      
      // Логируем входящие данные
      console.log('DEBUG /api/users/status:', {
        is_online,
        type: typeof is_online,
        body: req.body,
        user_id: req.user.id
      });
      
      // Преобразуем значение в boolean
      let statusValue = false;
      if (typeof is_online === 'boolean') {
        statusValue = is_online;
      } else if (typeof is_online === 'string') {
        statusValue = is_online === 'true' || is_online === '1';
      } else if (typeof is_online === 'number') {
        statusValue = is_online === 1 || is_online === true;
      }
      
      console.log('Converted status value:', statusValue);
      
      // ⭐ ИЗМЕНЕНО: Добавляем обновление last_seen при переводе в офлайн
      const query = statusValue 
        ? 'UPDATE users SET is_online = 1 WHERE id = ?'
        : 'UPDATE users SET is_online = 0, last_seen = NOW() WHERE id = ?';
      
      db.query(
        query,
        [req.user.id],
        (err, result) => {
          if (err) {
            console.error('Database error:', err);
            return res.status(500).json({ error: 'Server error', details: err.message });
          }
          
          console.log(`✅ User ${req.user.id} status updated to ${statusValue}`);
          
          // ⭐ ИЗМЕНЕНО: Добавляем timestamp в событие
          io.emit('user_status_changed', {
            user_id: req.user.id,
            is_online: statusValue,
            timestamp: new Date().toISOString()  // ⭐ НОВОЕ ПОЛЕ
          });
          
          res.json({ success: true, status: statusValue });
        }
      );
    } catch (error) {
      console.error('Unexpected error in /api/users/status:', error);
      res.status(500).json({ error: 'Server error', message: error.message });
    }
  });

  // Получить сообщения группы
  // Получить сообщения группы
  // Получить сообщения группы
  app.get('/api/groups/:groupId/messages', authenticateToken, (req, res) => {
    const { groupId } = req.params;
    
    console.log(`[DEBUG] Loading messages for group ${groupId}, user ${req.user.id}`);
    
    db.query(`
      SELECT gm.*, u.username as sender_username, u.avatar as sender_avatar,
            rm.message as reply_to_message,
            rm.sender_id as reply_to_sender_id,
            ru.username as reply_to_sender,
            COALESCE(gmrs.is_read, 0) as is_read,
            COUNT(DISTINCT gmrs2.reader_id) as reader_count
      FROM group_messages gm 
      JOIN users u ON gm.sender_id = u.id 
      LEFT JOIN group_messages rm ON gm.reply_to = rm.id
      LEFT JOIN users ru ON rm.sender_id = ru.id
      LEFT JOIN group_message_read_status gmrs ON gm.id = gmrs.message_id AND gmrs.reader_id = ?
      LEFT JOIN group_message_read_status gmrs2 ON gm.id = gmrs2.message_id AND gmrs2.is_read = TRUE
      WHERE gm.group_id = ? 
      GROUP BY gm.id
      ORDER BY gm.created_at ASC
    `, [req.user.id, groupId], (err, results) => {
      if (err) {
        console.error(`[ERROR] Failed to load group messages: ${err.message}`);
        console.error(err);
        return res.status(500).json({ error: 'Ошибка сервера', details: err.message });
      }
      
      console.log(`[DEBUG] Loaded ${results ? results.length : 0} messages for group ${groupId}`);
      if (results && results.length > 0) {
        results.forEach((msg, idx) => {
          if (idx < 3 || idx >= results.length - 3) {
            console.log(`[DEBUG] [${idx}] id=${msg.id}, sender=${msg.sender_username}, is_read=${msg.is_read}, reader_count=${msg.reader_count}`);
          }
        });
      }
      
      res.json(results || []);
    });
  });

  // Отправить сообщение в группу
  app.post('/api/groups/:groupId/messages', authenticateToken, (req, res) => {
    const { groupId } = req.params;
    const { message, reply_to, media_type = 'text', media_url, duration, caption } = req.body;
    const sender_id = req.user.id;

    // 🔒 Проверяем, не заглушен ли пользователь
    if (global.mutedUsers) {
      const muteInfo = global.mutedUsers.get(String(sender_id));
      
      if (muteInfo && new Date() < new Date(muteInfo.muteUntil)) {
        return res.status(403).json({ 
          error: 'Вы заглушены администратором',
          until: muteInfo.muteUntil
        });
      } else if (muteInfo) {
        global.mutedUsers.delete(String(sender_id));
      }
    }
    
    // 🔍 ЛОГИРОВАНИЕ ОТПРАВКИ ГРУППОВОГО СООБЩЕНИЯ
    console.log(`\n${'='.repeat(70)}`);
    console.log(`📤 ОТПРАВКА ГРУППОВОГО СООБЩЕНИЯ:`);
    console.log(`   Группа: ${groupId}`);
    console.log(`   От: ${sender_id}`);
    console.log(`   Текст: ${(message || '').slice(0, 50)}...`);
    console.log(`   Тип: ${media_type}`);
    console.log(`${'='.repeat(70)}`);
    
    db.query(
      'INSERT INTO group_messages (group_id, sender_id, message, reply_to, media_type, media_url, duration, caption) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      [groupId, sender_id, message, reply_to || null, media_type, media_url || null, duration || null, caption || null],
      (err, result) => {
        if (err) {
          console.error('❌ Error inserting group message:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        
        const messageId = result.insertId;
        console.log(`✅ Групповое сообщение ${messageId} вставлено в БД`);
        
        // ⭐ ГЛАВНОЕ ИСПРАВЛЕНИЕ: Получаем аватар из таблицы users
        db.query(
          'SELECT avatar FROM users WHERE id = ?',
          [sender_id],
          (err, userResults) => {
            if (err) {
              console.error('❌ Error fetching user avatar:', err);
            }
            
            const userAvatar = userResults && userResults.length > 0 ? userResults[0].avatar : null;
            
            console.log(`📝 New group message: id=${messageId}, sender_id=${sender_id}, has_avatar=${!!userAvatar}`);
            
            // ⭐ ИСПРАВЛЕНО: Получаем created_at из БД чтобы иметь правильный формат
            db.query(
              'SELECT created_at FROM group_messages WHERE id = ?',
              [messageId],
              (err, timeResults) => {
                const createdAt = !err && timeResults && timeResults.length > 0 
                  ? timeResults[0].created_at 
                  : new Date();
                
                const messageData = {
                  id: messageId,
                  group_id: groupId,
                  sender_id: sender_id,
                  message,
                  reply_to: reply_to || null,
                  media_type,
                  media_url: media_url || null,
                  duration: duration || null,
                  caption: caption || null,
                  sender_username: req.user.username,
                  sender_avatar: userAvatar,  // ⭐ ГЛАВНОЕ: Передаем аватару из БД
                  created_at: createdAt,
                  read_by: [sender_id]  // ⭐ НОВОЕ: Сразу отмечаем что отправитель прочитал
                };
                
                // Автоматически отмечаем групповое сообщение как прочитанное для отправителя
                db.query(
                  `INSERT INTO group_message_read_status (message_id, reader_id, is_read, read_at)
                  VALUES (?, ?, TRUE, CURRENT_TIMESTAMP)`,
                  [messageId, sender_id],
                  (err) => {
                    if (err) console.error('Error marking group message as read for sender:', err);
                  }
                );
                
                // Отправляем ответ клиенту (включая сообщение с аватаркой)
                res.json(messageData);
                console.log(`📤 Ответ отправлен клиенту с аватаркой: ${!!userAvatar}`);

                // 🔴 ОТПРАВЛЯЕМ SOCKET.IO СОБЫТИЕ
                console.log(`\n🔌 Socket.io EVENTS:`);
                
                // 🔧 ГЛАВНОЕ: Отправляем сообщение ВСЕ в комнату группы (включая отправителя)
                io.to(`group_${groupId}`).emit('new_group_message', messageData);
                console.log(`   ✅ Эмит 'new_group_message' в комнату group_${groupId} для ${groupId} с аватаркой`);
                
                console.log(`✅ Group message sent via Socket.io to all users in room`);
                console.log(`${'='.repeat(70)}\n`);
              }
            );
            
            // 🔔 ОТПРАВКА PUSH УВЕДОМЛЕНИЙ ГРУППЕ (только если пользователь не в активном чате)
            (async () => {
              try {
                // Получаем всех членов группы кроме отправителя
                db.query(`
                  SELECT DISTINCT gm.user_id, pt.push_token
                  FROM group_members gm
                  LEFT JOIN push_tokens pt ON gm.user_id = pt.user_id
                  WHERE gm.group_id = ? AND gm.user_id != ?
                `, [groupId, sender_id], async (err, members) => {
                  if (err) {
                    console.error('❌ Ошибка получения членов группы:', err);
                    return;
                  }
                  
                  if (!members || members.length === 0) {
                    console.log(`⚠️ Нет членов группы ${groupId} для отправки push`);
                    return;
                  }
                  
                  // Фильтруем членов: только те, кто ОФЛАЙН или имеет валидный токен
                  const validMembers = members.filter(m => {
                    // Проверить валидность токена
                    if (!m.push_token || !Expo.isExpoPushToken(m.push_token)) {
                      console.log(`   ⏭️ Член ${m.user_id}: нет валидного токена`);
                      return false;
                    }
                    
                    // ГЛАВНОЕ: Проверяем статус пользователя
                    const memberSockets = getUserSocketIds(m.user_id);
                    const isMemberOnline = memberSockets.length > 0;
                    
                    if (!isMemberOnline) {
                      // ОФЛАЙН - отправляем push обязательно
                      console.log(`   ✅ Член ${m.user_id} ОФЛАЙН → отправим push`);
                      return true;
                    }
                    
                    // ОНЛАЙН - проверяем, в активном ли чате группы
                    if (isUserInActiveChat(m.user_id, groupId, 'group')) {
                      // В активном чате - не отправляем push
                      console.log(`   ℹ️ Член ${m.user_id} ОНЛАЙН в этой группе → push не нужен`);
                      return false;
                    }
                    
                    // ОНЛАЙН но в другом чате - не отправляем push (пусть слышит в приложении)
                    console.log(`   ℹ️ Член ${m.user_id} ОНЛАЙН но в другом чате → звук в app`);
                    return false;
                  });
                  
                  if (validMembers.length === 0) {
                    console.log(`⚠️ Нет получателей для push в группе ${groupId} (все либо в чате, либо без токена)`);
                    return;
                  }
                  
                  // Получаем имя группы для заголовка
                  db.query('SELECT name FROM groups WHERE id = ?', [groupId], async (err, groupResults) => {
                    if (err) {
                      console.error('❌ Ошибка получения названия группы:', err);
                      return;
                    }
                    
                    const groupName = groupResults && groupResults.length > 0 ? groupResults[0].name : 'Группа';
                    
                    const pushMessages = validMembers.map(m => ({
                      to: m.push_token,
                      sound: 'default',
                      title: `💬 ${groupName}`,
                      body: `${req.user.username}: ${message.slice(0, 100)}`,
                      data: {
                        type: 'new_group_message',
                        group_id: groupId,
                        sender_id: req.user.id,
                        senderId: req.user.id,
                        senderName: req.user.username,
                        message_id: result.insertId,
                        message: message.slice(0, 100),
                        isGroup: true
                      },
                      badge: 1,
                      priority: 'high'
                    }));
                    
                    try {
                      const tickets = await expo.sendPushNotificationsAsync(pushMessages);
                      console.log(`✅ Push отправлены ${pushMessages.length} членам группы ${groupId}`);
                    } catch (error) {
                      console.error('❌ Ошибка отправки group push:', error.message);
                    }
                  });
                });
              } catch (error) {
                console.error('❌ Ошибка при отправке push:', error.message);
              }
            })();
          }
        );
      }
    );
  });

  // Получить список друзей
  app.get('/api/friends', authenticateToken, (req, res) => {
    db.query(`
      SELECT u.id, u.username, u.email, u.avatar, f.status
      FROM friends f
      JOIN users u ON (f.friend_id = u.id OR f.user_id = u.id)
      WHERE (f.user_id = ? OR f.friend_id = ?) AND u.id != ? AND u.is_admin = FALSE
      ORDER BY f.created_at DESC
    `, [req.user.id, req.user.id, req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      res.json(results);
    });
  });

  // Поиск пользователей
  // Поиск пользователей (ИСКЛЮЧАЕМ АДМИНОВ)
  app.get('/api/users/search', authenticateToken, (req, res) => {
    const { q } = req.query;
    
    if (!q || q.trim().length < 2) {
      return res.status(400).json({ error: 'Запрос должен содержать минимум 2 символа' });
    }
    
    db.query(`
      SELECT u.id, u.username, u.email, u.avatar,
            f.status as friend_status,
            CASE 
              WHEN f.user_id = ? THEN 'sent'
              WHEN f.friend_id = ? THEN 'received'
              ELSE NULL
            END as request_direction
      FROM users u
      LEFT JOIN friends f ON (f.user_id = ? AND f.friend_id = u.id) OR (f.friend_id = ? AND f.user_id = u.id)
      WHERE (u.username LIKE ? OR u.email LIKE ?) AND u.id != ? AND u.is_admin = FALSE
      LIMIT 20
    `, [req.user.id, req.user.id, req.user.id, req.user.id, `%${q}%`, `%${q}%`, req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      res.json(results);
    });
  });

  app.post('/api/users/push-token', authenticateToken, (req, res) => {
    // Debug: логируем заголовок авторизации (токен маскируем)
    try {
      const safeAuth = (req.headers && req.headers.authorization)
        ? req.headers.authorization.replace(/Bearer\s+(.{4}).+/, 'Bearer $1...')
        : 'none';
      console.log(`\n[DEBUG /api/users/push-token]`);
      console.log(`  Authorization: ${safeAuth}`);
      console.log(`  Body keys: ${Object.keys(req.body || {}).join(', ')}`);
    } catch (e) {
      // skip
    }

    const { deviceType, deviceName } = req.body || {};

    // Допускаем несколько вариаций ключа, которые могут прийти от разных клиентов
    const pushToken = (req.body && (req.body.pushToken || req.body.push_token || req.body.token || req.body.expoPushToken)) || null;

    if (!pushToken) {
      console.log(`  ❌ Push token is required`);
      return res.status(400).json({ error: 'Push token is required' });
    }

    // Получаем user_id из JWT токена (аутентификация обязательна)
    const userId = req.user?.id;

    if (!userId) {
      console.log(`  ❌ user_id not found in JWT token`);
      return res.status(401).json({ error: 'User not authenticated' });
    }

    console.log(`  ✓ user_id from JWT: ${userId}`);
    console.log(`  ✓ pushToken: ${String(pushToken).slice(0, 40)}...`);
    console.log(`  ✓ deviceType: ${deviceType || 'unknown'}`);

    // ✨ ГЛАВНОЕ ИЗМЕНЕНИЕ: используем INSERT...ON DUPLICATE KEY UPDATE с ключом (user_id)
    // Это гарантирует что у каждого user_id будет ровно один токен
    db.query(
      `INSERT INTO push_tokens (user_id, push_token, device_type, device_name) 
      VALUES (?, ?, ?, ?)
      ON DUPLICATE KEY UPDATE 
        push_token = VALUES(push_token),
        device_type = VALUES(device_type),
        device_name = VALUES(device_name),
        updated_at = CURRENT_TIMESTAMP`,
      [userId, pushToken, deviceType || 'unknown', deviceName || 'Unknown Device'],
      (err) => {
        if (err) {
          console.error(`  ❌ Database error:`, err.message);
          return res.status(500).json({ error: 'Server error', details: err.message });
        }
        
        console.log(`  ✅ Push token saved for user_id=${userId}: ${String(pushToken).slice(0, 40)}...`);
        
        // Проверяем что было сохранено
        db.query(
          'SELECT user_id, push_token FROM push_tokens WHERE user_id = ?',
          [userId],
          (err, results) => {
            if (!err && results.length > 0) {
              console.log(`  ✓ Verification: found ${results.length} token(s) for user_id=${userId}`);
              results.forEach((row, idx) => {
                console.log(`    [${idx}] token=${row.push_token.slice(0, 40)}...`);
              });
            }
          }
        );

        res.json({ 
          message: 'Push token registered successfully', 
          user_id: userId,
          token_prefix: String(pushToken).slice(0, 40)
        });
      }
    );
  });

  // Удаление push-токена
  app.delete('/api/users/push-token', authenticateToken, (req, res) => {
    const { pushToken } = req.body;

    if (!pushToken) {
      return res.status(400).json({ error: 'Push token is required' });
    }

    db.query(
      'DELETE FROM push_tokens WHERE push_token = ? AND user_id = ?',
      [pushToken, req.user.id],
      (err, result) => {
        if (err) {
          console.error('Database error:', err);
          return res.status(500).json({ error: 'Server error' });
        }
        res.json({ message: 'Push token removed successfully' });
      }
    );
  });

  // Получение push-токенов пользователя
  app.get('/api/users/push-tokens', authenticateToken, (req, res) => {
    db.query(
      `SELECT push_token, device_type, device_name, created_at, updated_at
      FROM push_tokens
      WHERE user_id = ?
      ORDER BY updated_at DESC`,
      [req.user.id],
      (err, results) => {
        if (err) {
          console.error('Database error:', err);
          return res.status(500).json({ error: 'Server error' });
        }
        res.json(results);
      }
    );
  });

  // Утилита для отправки push-уведомлений
  async function sendPushNotifications(userId, title, body, data = {}) {
    db.query(
      'SELECT push_token FROM push_tokens WHERE user_id = ?',
      [userId],
      async (err, results) => {
        if (err) {
          console.error('Error fetching push tokens:', err);
          return;
        }

        const tokens = results.map(row => row.push_token);
        
        if (tokens.length === 0) {
          console.log('No push tokens found for user:', userId);
          return;
        }

        try {
          // Здесь можно добавить логику для отправки уведомлений через Expo Push API
          console.log('Sending push notifications to tokens:', tokens);
          console.log('Notification:', { title, body, data });
        } catch (error) {
          console.error('Error sending push notifications:', error);
        }
      }
    );
  }

  // Отправить заявку в друзья
  app.post('/api/friends/request/:userId', authenticateToken, (req, res) => {
    const { userId } = req.params;
    
    console.log(`[DEBUG] Friend request endpoint - userId: "${userId}", type: ${typeof userId}, req.user.id: ${req.user.id}`);
    
    // Валидация userId
    if (!userId || userId === 'undefined' || isNaN(parseInt(userId))) {
      console.error('❌ Invalid userId:', userId);
      return res.status(400).json({ error: 'Неверный ID пользователя' });
    }
    
    const parsedUserId = parseInt(userId);
    
    if (parsedUserId == req.user.id) {
      return res.status(400).json({ error: 'Нельзя добавить себя в друзья' });
    }
    
    // Проверяем, существует ли пользователь
    db.query('SELECT id FROM users WHERE id = ?', [parsedUserId], (err, results) => {
      if (err) {
        console.error('Database error:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }
      
      if (results.length === 0) {
        return res.status(404).json({ error: 'Пользователь не найден' });
      }
      
      // Проверяем, уже ли есть такая заявка или дружба
      db.query(`
        SELECT status FROM friends 
        WHERE (user_id = ? AND friend_id = ?) 
          OR (user_id = ? AND friend_id = ?)
      `, [req.user.id, parsedUserId, parsedUserId, req.user.id], (err, existingResults) => {
        if (err) {
          console.error('Database error:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        
        if (existingResults.length > 0) {
          const existingFriendship = existingResults[0];
          if (existingFriendship.status === 'accepted') {
            return res.status(400).json({ error: 'Вы уже друзья' });
          } else {
            return res.status(400).json({ error: 'Заявка уже отправлена' });
          }
        }
        
        // Отправляем заявку
        db.query(
          'INSERT INTO friends (user_id, friend_id, status) VALUES (?, ?, "pending")',
          [req.user.id, parsedUserId],
          (err, result) => {
            if (err) {
              console.error('Database error:', err);
              if (err.code === 'ER_DUP_ENTRY') {
                return res.status(400).json({ error: 'Заявка уже отправлена' });
              }
              return res.status(500).json({ error: 'Ошибка сервера' });
            }
            
            console.log(`✅ Friend request sent from ${req.user.id} to ${parsedUserId}`);
            res.json({ success: true, message: 'Заявка в друзья отправлена' });
          }
        );
      });
    });
  });

  // Принять заявку в друзья
  app.post('/api/friends/accept/:userId', authenticateToken, (req, res) => {
    const { userId } = req.params;
    
    db.query(
      'UPDATE friends SET status = "accepted" WHERE user_id = ? AND friend_id = ? AND status = "pending"',
      [userId, req.user.id],
      (err, result) => {
        if (err) return res.status(500).json({ error: 'Ошибка сервера' });
        if (result.affectedRows === 0) {
          return res.status(404).json({ error: 'Заявка не найдена' });
        }
        res.json({ message: 'Заявка принята' });
      }
    );
  });

  // Удалить из друзей
  app.delete('/api/friends/:userId', authenticateToken, (req, res) => {
    const { userId } = req.params;
    
    db.query(
      'DELETE FROM friends WHERE (user_id = ? AND friend_id = ?) OR (user_id = ? AND friend_id = ?)',
      [req.user.id, userId, userId, req.user.id],
      (err, result) => {
        if (err) return res.status(500).json({ error: 'Ошибка сервера' });
        res.json({ message: 'Удалено из друзей' });
      }
    );
  });

  app.post('/api/groups/:groupId/members', authenticateToken, (req, res) => {
    const { groupId } = req.params;
    const { userId } = req.body;

    if (!userId) {
      return res.status(400).json({ error: 'userId обязателен' });
    }

    if (Number(userId) === req.user.id) {
      return res.status(400).json({ error: 'Нельзя добавить себя в качестве нового участника' });
    }

    // Проверяем, является ли текущий пользователь админом группы
    db.query(
      'SELECT role FROM group_members WHERE group_id = ? AND user_id = ?',
      [groupId, req.user.id],
      (err, results) => {
        if (err) {
          console.error('❌ Ошибка проверки роли:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }

        if (results.length === 0 || results[0].role !== 'admin') {
          return res.status(403).json({ error: 'Добавлять участников могут только админы группы' });
        }

        // Проверяем, есть ли пользователь в системе
        db.query('SELECT id FROM users WHERE id = ?', [userId], (err, userResults) => {
          if (err) {
            console.error('❌ Ошибка проверки пользователя:', err);
            return res.status(500).json({ error: 'Ошибка сервера' });
          }

          if (userResults.length === 0) {
            return res.status(404).json({ error: 'Пользователь не найден' });
          }

          // Пробуем добавить участника
          db.query(
            `INSERT INTO group_members (group_id, user_id, role)
            VALUES (?, ?, 'member')`,
            [groupId, userId],
            (err) => {
              if (err) {
                if (err.code === 'ER_DUP_ENTRY') {
                  return res.status(400).json({ error: 'Пользователь уже состоит в группе' });
                }
                console.error('❌ Ошибка добавления участника:', err);
                return res.status(500).json({ error: 'Ошибка сервера' });
              }

              res.json({ success: true, message: 'Участник добавлен' });
            }
          );
        });
      }
    );
  });

  // Получить список переписок пользователя (включая тех, кто не в друзьях)
  app.get('/api/chats', authenticateToken, (req, res) => {
    const userId = req.user.id;

    // SQL запрос получает всех пользователей, с которыми у текущего юзера есть переписка
    // Включает даже тех, кто не в списке друзей
    const query = `
      SELECT 
        u.id,
        u.username,
        u.avatar,
        u.is_online,
        u.status,
        u.bio,
        u.email,
        u.created_at,
        MAX(m.created_at) as last_message_time,
        (SELECT m2.id FROM messages m2 
        WHERE ((m2.sender_id = ? AND m2.receiver_id = u.id) OR 
                (m2.sender_id = u.id AND m2.receiver_id = ?))
        ORDER BY m2.created_at DESC 
        LIMIT 1) as last_message_id,
        (SELECT m2.message FROM messages m2 
        WHERE ((m2.sender_id = ? AND m2.receiver_id = u.id) OR 
                (m2.sender_id = u.id AND m2.receiver_id = ?))
        ORDER BY m2.created_at DESC 
        LIMIT 1) as last_message,
        (SELECT COALESCE(MAX(CASE WHEN mrs.is_read = TRUE THEN 1 ELSE 0 END), 0)
        FROM messages m2
        LEFT JOIN message_read_status mrs ON m2.id = mrs.message_id AND mrs.reader_id = ?
        WHERE ((m2.sender_id = ? AND m2.receiver_id = u.id) OR 
                (m2.sender_id = u.id AND m2.receiver_id = ?))
        ORDER BY m2.created_at DESC 
        LIMIT 1) as last_message_is_read,
        (SELECT COUNT(*) FROM messages m3 
        WHERE m3.sender_id = u.id AND m3.receiver_id = ? AND NOT EXISTS (
          SELECT 1 FROM message_read_status 
          WHERE message_id = m3.id AND reader_id = ? AND is_read = TRUE
        )) as unread_count,
        MAX(CASE WHEN f.status = 'accepted' THEN 1 ELSE 0 END) AS is_friend
      FROM users u
      INNER JOIN messages m ON 
        (m.sender_id = u.id AND m.receiver_id = ?) OR 
        (m.sender_id = ? AND m.receiver_id = u.id)
      LEFT JOIN friends f ON (
        (f.user_id = ? AND f.friend_id = u.id)
        OR
        (f.friend_id = ? AND f.user_id = u.id)
      )
      WHERE u.id != ?
      GROUP BY u.id, u.username, u.avatar, u.is_online, u.status, u.bio, u.email, u.created_at
      ORDER BY last_message_time DESC
    `;

    db.query(query, [userId, userId, userId, userId, userId, userId, userId, userId, userId, userId, userId, userId, userId, userId], (err, results) => {
      if (err) {
        console.error('Ошибка получения списка переписок:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }

      // Преобразуем результаты для совместимости с фронтендом
      const conversations = (results || []).map(user => ({
        id: user.id,
        username: user.username,
        avatar: user.avatar,
        is_online: user.is_online,
        status: user.status,
        bio: user.bio,
        email: user.email,
        created_at: user.created_at,
        last_message: user.last_message,
        last_message_id: user.last_message_id,
        last_message_time: user.last_message_time,
        last_message_is_read: user.last_message_is_read === 1,  // ✅ НОВОЕ: Статус прочтения
        unread_count: user.unread_count || 0,
        is_friend: user.is_friend,
        type: 'personal'  // Добавляем тип для совместимости с фронтендом
      }));

      console.log(`✅ Loaded ${conversations.length} conversations for user ${userId}`);

      res.json(conversations);
    });
  });

  // GET /api/pinned-chats
  app.get('/api/pinned-chats', authenticateToken, (req, res) => {
    const userId = req.user.id;
    db.query(
      `SELECT id, user_id, chat_type, chat_id, pinned_at
      FROM pinned_chats
      WHERE user_id = ?
      ORDER BY pinned_at DESC`,
      [userId],
      (err, results) => {
        if (err) {
          console.error('Ошибка получения закрепленных чатов:', err);
          return res.status(500).json({ error: 'Ошибка сервера при получении закреплений' });
        }
        res.json({ success: true, data: results });
      }
    );
  });

  // POST /api/pinned-chats
  app.post('/api/pinned-chats', authenticateToken, (req, res) => {
    const userId = req.user.id;
    const { chat_type, chat_id } = req.body;

    if (!chat_type || !chat_id) {
      return res.status(400).json({ error: 'chat_type и chat_id обязательны' });
    }

    const chatType = chat_type === 'group' ? 'group' : 'personal';
    const chatId = parseInt(chat_id, 10);
    if (!Number.isInteger(chatId) || chatId <= 0) {
      return res.status(400).json({ error: 'Некорректный chat_id' });
    }

    db.query(
      `INSERT INTO pinned_chats (user_id, chat_type, chat_id, pinned_at)
      VALUES (?, ?, ?, NOW())
      ON DUPLICATE KEY UPDATE pinned_at = NOW()`,
      [userId, chatType, chatId],
      (err, result) => {
        if (err) {
          console.error('Ошибка сохранения закрепа:', err);
          return res.status(500).json({ error: 'Ошибка сервера при сохранении закрепления' });
        }

        db.query(
          `SELECT id, user_id, chat_type, chat_id, pinned_at
          FROM pinned_chats
          WHERE user_id = ? AND chat_type = ? AND chat_id = ?`,
          [userId, chatType, chatId],
          (err, rows) => {
            if (err) {
              console.error('Ошибка получения закрепа после вставки:', err);
              return res.status(500).json({ error: 'Ошибка сервера' });
            }
            res.json({ success: true, data: rows.length ? rows[0] : null });
          }
        );
      }
    );
  });

  // DELETE /api/pinned-chats/:chatType/:chatId
  app.delete('/api/pinned-chats/:chatType/:chatId', authenticateToken, (req, res) => {
    const userId = req.user.id;
    const chatType = req.params.chatType === 'group' ? 'group' : 'personal';
    const chatId = parseInt(req.params.chatId, 10);

    if (!Number.isInteger(chatId) || chatId <= 0) {
      return res.status(400).json({ error: 'Некорректный chatId' });
    }

    db.query(
      `DELETE FROM pinned_chats
      WHERE user_id = ? AND chat_type = ? AND chat_id = ?`,
      [userId, chatType, chatId],
      (err, result) => {
        if (err) {
          console.error('Ошибка удаления закрепа:', err);
          return res.status(500).json({ error: 'Ошибка сервера при удалении закрепления' });
        }
        res.json({ success: true });
      }
    );
  });

  // Получить участников группы
  app.get('/api/groups/:groupId/members', authenticateToken, (req, res) => {
    const { groupId } = req.params;
    
    db.query(`
      SELECT u.id, u.username, u.avatar, gm.role 
      FROM group_members gm 
      JOIN users u ON gm.user_id = u.id 
      WHERE gm.group_id = ?
      ORDER BY gm.role DESC, u.username ASC
    `, [groupId], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      res.json(results);
    });
  });

  // Обновить группу
  app.put('/api/groups/:groupId', authenticateToken, (req, res) => {
    const { groupId } = req.params;
    const { name, description, avatar } = req.body;
    
    db.query(
      'UPDATE groups SET name = ?, description = ?, avatar = ? WHERE id = ?',
      [name, description || null, avatar || null, groupId],
      (err, result) => {
        if (err) return res.status(500).json({ error: 'Ошибка сервера' });
        res.json({ success: true, message: 'Группа обновлена' });
      }
    );
  });

  // Удалить участника группы
  app.delete('/api/groups/:groupId/members/:userId', authenticateToken, (req, res) => {
    const { groupId, userId } = req.params;
    
    db.query(
      'DELETE FROM group_members WHERE group_id = ? AND user_id = ?',
      [groupId, userId],
      (err, result) => {
        if (err) return res.status(500).json({ error: 'Ошибка сервера' });
        res.json({ success: true, message: 'Участник удален' });
      }
    );
  });

  // Получить всех пользователей
  app.get('/api/users', authenticateToken, (req, res) => {
    db.query('SELECT id, username, email, avatar, bio, status, cardColor FROM users WHERE id != ? AND is_admin = FALSE', [req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      res.json(results);
    });
  });

  // Получить профиль пользователя по ID
  app.get('/api/users/:userId', (req, res) => {
    const userId = req.params.userId;
    
    if (!userId) {
      return res.status(400).json({ error: 'userId is required' });
    }
    
    const query = 'SELECT id, username, email, avatar, bio, status, is_online, cardColor, created_at FROM users WHERE id = ?';
    
    db.query(query, [userId], (err, results) => {
      if (err) {
        console.error('❌ Ошибка получения пользователя:', err);
        return res.status(500).json({ error: err.message });
      }
      
      if (!results || results.length === 0) {
        return res.status(404).json({ error: 'User not found' });
      }
      
      console.log(`✅ Профиль пользователя ${userId} загружен`);
      res.json(results[0]);
    });
  });

  // ========== BAN ENDPOINTS ==========

  // Забанить пользователя (ADMIN)
  app.post('/api/admin/users/:userId/ban', authenticateToken, (req, res) => {
    const { userId } = req.params;
    const { reason = 'Нарушение правил сообщества', ban_duration_days } = req.body;
    
    // Проверяем, является ли текущий пользователь админом
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен. Требуются права администратора' });
      }
      
      // Проверяем, что админ не забанивает сам себя
      if (parseInt(userId) === req.user.id) {
        return res.status(400).json({ error: 'Нельзя забанить самого себя' });
      }
      
      // Рассчитываем дату разбана
      let bannedUntil = null;
      if (ban_duration_days !== null && ban_duration_days !== undefined && ban_duration_days > 0) {
        const now = new Date();
        bannedUntil = new Date(now.getTime() + ban_duration_days * 24 * 60 * 60 * 1000);
      }
      // Если null или undefined - бан навсегда, banned_until остается null
      
      // Забаниваем пользователя
      db.query(
        'UPDATE users SET is_banned = TRUE, ban_reason = ?, banned_at = CURRENT_TIMESTAMP, banned_until = ?, banned_by = ? WHERE id = ?',
        [reason, bannedUntil, req.user.id, userId],
        (err, result) => {
          if (err) {
            console.error('❌ Error banning user:', err);
            return res.status(500).json({ error: 'Ошибка сервера' });
          }
          
          if (result.affectedRows === 0) {
            return res.status(404).json({ error: 'Пользователь не найден' });
          }
          
          const durationText = ban_duration_days === null || ban_duration_days === undefined
            ? 'навсегда'
            : `на ${ban_duration_days} дней`;
          
          console.log(`🚫 Пользователь ${userId} забанен администратором ${req.user.id} ${durationText}. Причина: ${reason}`);
          
          // Отправляем уведомление о бане через WebSocket если пользователь онлайн
          const userSocketIds = getUserSocketIds(userId);
          if (userSocketIds.length > 0) {
            userSocketIds.forEach((socketId) => {
              io.to(socketId).emit('user_banned', {
                reason: reason,
                message: 'Ваш аккаунт заблокирован администратором',
                bannedUntil: bannedUntil
              });
            });
          }
          
          res.json({ 
            success: true, 
            message: `Пользователь ${userId} успешно забанен ${durationText}`,
            reason: reason,
            bannedUntil: bannedUntil
          });
        }
      );
    });
  });

  // Разбанить пользователя (ADMIN)
  app.post('/api/admin/users/:userId/unban', authenticateToken, (req, res) => {
    const { userId } = req.params;
    
    // Проверяем, является ли текущий пользователь админом
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен. Требуются права администратора' });
      }
      
      // Разбаниваем пользователя
      db.query(
        'UPDATE users SET is_banned = FALSE, ban_reason = NULL, banned_at = NULL, banned_until = NULL, banned_by = NULL WHERE id = ?',
        [userId],
        (err, result) => {
          if (err) {
            console.error('❌ Error unbanning user:', err);
            return res.status(500).json({ error: 'Ошибка сервера' });
          }
          
          if (result.affectedRows === 0) {
            return res.status(404).json({ error: 'Пользователь не найден' });
          }
          
          console.log(`✅ Пользователь ${userId} разбанен администратором ${req.user.id}`);
          
          res.json({ 
            success: true, 
            message: `Пользователь ${userId} успешно разбанен`
          });
        }
      );
    });
  });

  // Получить информацию о бане пользователя (ADMIN)
  app.get('/api/admin/users/:userId/ban-info', authenticateToken, (req, res) => {
    const { userId } = req.params;
    
    // Проверяем, является ли текущий пользователь админом
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен. Требуются права администратора' });
      }
      
      // Получаем информацию о бане
      db.query(
        `SELECT u.id, u.username, u.email, u.is_banned, u.ban_reason, u.banned_at, u.banned_by,
                admin.username as banned_by_username
        FROM users u
        LEFT JOIN users admin ON u.banned_by = admin.id
        WHERE u.id = ?`,
        [userId],
        (err, results) => {
          if (err) {
            console.error('❌ Error fetching ban info:', err);
            return res.status(500).json({ error: 'Ошибка сервера' });
          }
          
          if (results.length === 0) {
            return res.status(404).json({ error: 'Пользователь не найден' });
          }
          
          res.json({ 
            success: true,
            data: results[0]
          });
        }
      );
    });
  });

  // Получить всех забаненных пользователей (ADMIN)
  app.get('/api/admin/banned-users', authenticateToken, (req, res) => {
    // Проверяем, является ли текущий пользователь админом
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен. Требуются права администратора' });
      }
      
      // Получаем всех забаненных пользователей
      db.query(
        `SELECT u.id, u.username, u.email, u.ban_reason, u.banned_at,
                admin.username as banned_by_username
        FROM users u
        LEFT JOIN users admin ON u.banned_by = admin.id
        WHERE u.is_banned = TRUE
        ORDER BY u.banned_at DESC`,
        (err, results) => {
          if (err) {
            console.error('❌ Error fetching banned users:', err);
            return res.status(500).json({ error: 'Ошибка сервера' });
          }
          
          res.json({ 
            success: true,
            count: results.length,
            data: results
          });
        }
      );
    });
  });

  // Health check endpoint
  app.get('/api/health', (req, res) => {
    res.status(200).json({
      status: 'OK',
      message: 'Server is running',
      timestamp: new Date().toISOString()
    });
  });

  // Создаем папку uploads если её нет
  const uploadsDir = path.join(__dirname, 'uploads');
  if (!fs.existsSync(uploadsDir)) {
    fs.mkdirSync(uploadsDir, { recursive: true });
  }

  // Настройка multer для загрузки файлов
  const storage = multer.diskStorage({
    destination: function (req, file, cb) {
      cb(null, 'uploads/');
    },
    filename: function (req, file, cb) {
      const uniqueSuffix = Date.now() + '-' + Math.round(Math.random() * 1E9);
      cb(null, file.fieldname + '-' + uniqueSuffix + path.extname(file.originalname));
    }
  });

  const upload = multer({ 
    storage: storage,
    limits: {
      fileSize: 50 * 1024 * 1024 // 50MB лимит
    },
    fileFilter: function (req, file, cb) {
      // Разрешаем изображения, видео и аудио
      if (file.mimetype.startsWith('image/') || file.mimetype.startsWith('video/') || file.mimetype.startsWith('audio/')) {
        cb(null, true);
      } else {
        cb(null, true); // Разрешаем все файлы для тестирования
      }
    }
  });

  // Статическая раздача файлов
  app.use('/uploads', express.static(path.join(__dirname, 'uploads')));

  // Endpoint для загрузки файлов
  app.post('/api/upload', authenticateToken, upload.single('media'), (req, res) => {
    try {
      if (!req.file) {
        return res.status(400).json({ error: 'Файл не найден' });
      }

      const fileUrl = `http://151.241.228.247:3001/uploads/${req.file.filename}`;
      
      res.json({
        success: true,
        url: fileUrl,
        filename: req.file.filename,
        originalName: req.file.originalname,
        size: req.file.size
      });
    } catch (error) {
      console.error('Ошибка загрузки файла:', error);
      res.status(500).json({ error: 'Ошибка загрузки файла' });
    }
  });

  // Получить сообщения между пользователями
  app.get('/api/messages/:userId', authenticateToken, (req, res) => {
    const { userId } = req.params;
    const currentUserId = req.user.id;
    
    db.query(`
      SELECT m.*, 
            su.username as sender_username,
            su.avatar as sender_avatar,
            rm.message as reply_to_message,
            rm.sender_id as reply_to_sender_id,
            ru.username as reply_to_sender,
            COALESCE(mrs.is_read, 0) as is_read
      FROM messages m
      JOIN users su ON m.sender_id = su.id
      LEFT JOIN messages rm ON m.reply_to = rm.id
      LEFT JOIN users ru ON rm.sender_id = ru.id
      LEFT JOIN message_read_status mrs ON m.id = mrs.message_id AND mrs.reader_id = ?
      WHERE (m.sender_id = ? AND m.receiver_id = ?) 
        OR (m.sender_id = ? AND m.receiver_id = ?)
      ORDER BY m.created_at ASC
    `, [currentUserId, currentUserId, userId, userId, currentUserId], (err, results) => {
      if (err) {
        console.error('❌ Ошибка получения сообщений:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }
      
      console.log(`📤 GET /api/messages/${userId}: Возвращаю ${results.length} сообщений`);
      if (results.length > 0) {
        const lastMsg = results[results.length - 1];
        console.log(`   Последнее сообщение: id=${lastMsg.id}, sender_id=${lastMsg.sender_id}, is_read=${lastMsg.is_read}`);
      }
      
      res.json(results);
    });
  });

  // Получить количество непрочитанных сообщений от пользователя
  app.get('/api/messages/:userId/unread-count', authenticateToken, (req, res) => {
    const { userId } = req.params;
    
    db.query(`
      SELECT COUNT(*) as unread_count
      FROM messages m
      WHERE m.receiver_id = ?
      AND NOT EXISTS (
        SELECT 1 FROM message_read_status 
        WHERE message_id = m.id AND reader_id = ? AND is_read = TRUE
      )
    `, [req.user.id, req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      res.json({ unread_count: results[0].unread_count });
    });
  });

  // Отметить сообщение как прочитанное
  app.post('/api/messages/:messageId/read', authenticateToken, (req, res) => {
    const { messageId } = req.params;
    
    db.query(
      `INSERT INTO message_read_status (message_id, reader_id, is_read, read_at)
      VALUES (?, ?, TRUE, CURRENT_TIMESTAMP)
      ON DUPLICATE KEY UPDATE is_read = TRUE, read_at = CURRENT_TIMESTAMP`,
      [messageId, req.user.id],
      (err) => {
        if (err) return res.status(500).json({ error: 'Ошибка сервера' });
        res.json({ success: true });
      }
    );
  });

  // Отметить все сообщения от пользователя как прочитанные
  app.post('/api/messages/:userId/read-all', authenticateToken, (req, res) => {
    const { userId } = req.params;
    
    db.query(`
      INSERT INTO message_read_status (message_id, reader_id, is_read, read_at)
      SELECT m.id, ?, TRUE, CURRENT_TIMESTAMP
      FROM messages m
      WHERE m.sender_id = ? AND m.receiver_id = ?
      ON DUPLICATE KEY UPDATE is_read = TRUE, read_at = CURRENT_TIMESTAMP
    `, [req.user.id, userId, req.user.id], (err) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      res.json({ success: true });
    });
  });

  // Отметить все сообщения группы как прочитанные
  app.post('/api/groups/:groupId/read-all', authenticateToken, (req, res) => {
    const { groupId } = req.params;
    
    db.query(`
      INSERT INTO group_message_read_status (message_id, reader_id, is_read, read_at)
      SELECT gm.id, ?, TRUE, CURRENT_TIMESTAMP
      FROM group_messages gm
      WHERE gm.group_id = ? AND gm.sender_id != ?
      ON DUPLICATE KEY UPDATE is_read = TRUE, read_at = CURRENT_TIMESTAMP
    `, [req.user.id, groupId, req.user.id], (err) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      res.json({ success: true });
    });
  });

  // POST /api/groups/:groupId/leave
  // Выход пользователя из группы
  app.post('/api/groups/:groupId/leave', authenticateToken, (req, res) => {
    const { groupId } = req.params;
    const userId = req.user.id;

    // Шаг 1: Проверяем что группа существует
    db.query(
      'SELECT id FROM groups WHERE id = ?',
      [groupId],
      (err, groups) => {
        if (err) {
          console.error('❌ Error checking group:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }

        if (groups.length === 0) {
          return res.status(404).json({ error: 'Группа не найдена' });
        }

        // Шаг 2: Проверяем что пользователь в группе
        db.query(
          'SELECT id FROM group_members WHERE group_id = ? AND user_id = ?',
          [groupId, userId],
          (err, members) => {
            if (err) {
              console.error('❌ Error checking membership:', err);
              return res.status(500).json({ error: 'Ошибка сервера' });
            }

            if (members.length === 0) {
              return res.status(400).json({ error: 'Вы не являетесь участником этой группы' });
            }

            // Шаг 3: Удаляем пользователя из группы
            db.query(
              'DELETE FROM group_members WHERE group_id = ? AND user_id = ?',
              [groupId, userId],
              (err) => {
                if (err) {
                  console.error('❌ Error removing member:', err);
                  return res.status(500).json({ error: 'Ошибка сервера' });
                }

                console.log(`✅ User ${userId} left group ${groupId}`);

                // Отправляем уведомление через WebSocket
                io.emit('user_left_group', {
                  group_id: groupId,
                  user_id: userId
                });

                res.json({ 
                  success: true, 
                  message: 'Вы покинули группу' 
                });
              }
            );
          }
        );
      }
    );
  });

  // ========== ОБНОВИТЬ АВАТАР ГРУППЫ ==========
  app.post('/api/groups/:groupId/avatar', authenticateToken, (req, res) => {
    const { groupId } = req.params;
    const { avatar } = req.body;
    
    if (!avatar) {
      return res.status(400).json({ error: 'Аватар обязателен' });
    }
    
    // Проверяем, является ли пользователь админом группы
    db.query(
      'SELECT role FROM group_members WHERE group_id = ? AND user_id = ?',
      [groupId, req.user.id],
      (err, results) => {
        if (err) {
          console.error('❌ Ошибка проверки роли:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        
        if (results.length === 0 || results[0].role !== 'admin') {
          return res.status(403).json({ error: 'Только админы группы могут обновлять аватар' });
        }
        
        // Обновляем аватар группы
        db.query(
          'UPDATE groups SET avatar = ? WHERE id = ?',
          [avatar, groupId],
          (err, result) => {
            if (err) {
              console.error('❌ Ошибка обновления аватара:', err);
              return res.status(500).json({ error: 'Ошибка сервера' });
            }
            
            if (result.affectedRows === 0) {
              return res.status(404).json({ error: 'Группа не найдена' });
            }
            
            console.log(`✅ Аватар группы ${groupId} обновлен пользователем ${req.user.id}`);
            
            // Отправляем обновление через Socket.io
            io.to(`group_${groupId}`).emit('group_avatar_updated', {
              group_id: groupId,
              avatar: avatar
            });
            
            res.json({ 
              success: true, 
              message: 'Аватар группы обновлен',
              avatar: avatar
            });
          }
        );
      }
    );
  });

  // Получить количество непрочитанных групповых сообщений
  app.get('/api/groups/:groupId/unread-count', authenticateToken, (req, res) => {
    const { groupId } = req.params;
    
    db.query(`
      SELECT COUNT(*) as unread_count
      FROM group_messages gm
      WHERE gm.group_id = ?
      AND NOT EXISTS (
        SELECT 1 FROM group_message_read_status 
        WHERE message_id = gm.id AND reader_id = ? AND is_read = TRUE
      )
    `, [groupId, req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      res.json({ unread_count: results[0].unread_count });
    });
  });

  // Получить закреплённые сообщения в личном чате
  app.get('/api/chats/:chatId/pinned-messages', authenticateToken, (req, res) => {
    const { chatId } = req.params;

    db.query(`
      SELECT DISTINCT pm.id, pm.message_id, pm.chat_type, pm.chat_id, pm.pinned_at, pm.user_id, pm.is_visible_to_all
      FROM pinned_messages pm
      WHERE pm.chat_id = ? AND pm.chat_type = 'personal'
      AND (
        pm.is_visible_to_all = 1
        OR pm.user_id = ?
      )
      ORDER BY pm.pinned_at DESC
    `, [chatId, req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      res.json(results);
    });
  });

  // Получить закреплённые сообщения в групповом чате
  app.get('/api/groups/:groupId/pinned-messages', authenticateToken, (req, res) => {
    const { groupId } = req.params;

    db.query(`
      SELECT DISTINCT pm.id, pm.message_id, pm.chat_type, pm.chat_id, pm.pinned_at, pm.user_id, pm.is_visible_to_all
      FROM pinned_messages pm
      WHERE pm.chat_id = ? AND pm.chat_type = 'group'
      ORDER BY pm.pinned_at DESC
    `, [groupId], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      res.json(results);
    });
  });

  // Закрепить сообщение
  app.post('/api/pinned-messages', authenticateToken, (req, res) => {
    const { message_id, chat_type, chat_id, is_visible_to_all } = req.body;

    console.log(`\n📌 POST /api/pinned-messages: message_id=${message_id}, chat_type=${chat_type}, chat_id=${chat_id}, is_visible_to_all=${is_visible_to_all}`);

    if (!message_id || !chat_type || !chat_id) {
      return res.status(400).json({ error: 'Все поля обязательны' });
    }

    if (!['personal', 'group'].includes(chat_type)) {
      return res.status(400).json({ error: 'Неверный тип чата' });
    }

    db.query(
      `INSERT INTO pinned_messages (user_id, message_id, chat_type, chat_id, is_visible_to_all)
      VALUES (?, ?, ?, ?, ?)`,
      [req.user.id, message_id, chat_type, chat_id, is_visible_to_all ? 1 : 0],
      (err, result) => {
        if (err) {
          if (err.code === 'ER_DUP_ENTRY') {
            console.log(`⚠️ Сообщение уже закреплено`);
            return res.status(400).json({ error: 'Сообщение уже закреплено' });
          }
          console.error('❌ Database error:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        console.log(`✅ Сообщение закреплено: id=${result.insertId}, is_visible_to_all=${is_visible_to_all ? 1 : 0}`);
        res.json({ success: true, id: result.insertId });
      }
    );
  });

  // Открепить сообщение
  app.delete('/api/pinned-messages/:messageId', authenticateToken, (req, res) => {
    const { messageId } = req.params;

    db.query(
      `DELETE FROM pinned_messages 
      WHERE user_id = ? AND message_id = ?`,
      [req.user.id, messageId],
      (err, result) => {
        if (err) {
          console.error('Database error:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        if (result.affectedRows === 0) {
          return res.status(404).json({ error: 'Закреп не найден' });
        }
        res.json({ success: true });
      }
    );
  });

  // Отправить сообщение
  app.post('/api/messages', authenticateToken, (req, res) => {
    const { receiver_id, message, reply_to, media_type = 'text', media_url, duration, caption } = req.body;
    const sender_id = req.user.id;

    // 🔒 Проверяем, не заглушен ли пользователь
    if (global.mutedUsers) {
      const muteInfo = global.mutedUsers.get(String(sender_id));
      
      if (muteInfo && new Date() < new Date(muteInfo.muteUntil)) {
        return res.status(403).json({ 
          error: 'Вы заглушены администратором',
          until: muteInfo.muteUntil
        });
      } else if (muteInfo) {
        global.mutedUsers.delete(String(sender_id));
      }
    }
    
    // 🔍 ЛОГИРОВАНИЕ ОТПРАВКИ
    console.log(`\n${'='.repeat(70)}`);
    console.log(`📤 ОТПРАВКА ЛИЧНОГО СООБЩЕНИЯ:`);
    console.log(`   От: ${sender_id}`);
    console.log(`   Кому: ${receiver_id}`);
    console.log(`   Текст: ${(message || '').slice(0, 50)}...`);
    console.log(`   Тип: ${media_type}`);
    console.log(`${'='.repeat(70)}`);
    
    db.query(
      'INSERT INTO messages (sender_id, receiver_id, message, reply_to, media_type, media_url, duration, caption) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      [sender_id, receiver_id, message, reply_to || null, media_type, media_url || null, duration || null, caption || null],
      (err, result) => {
        if (err) {
          console.error('❌ Ошибка вставки сообщения в БД:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        
        const insertedMessageId = result.insertId;
        console.log(`✅ Сообщение ${insertedMessageId} вставлено в БД`);

        // Автоматически отмечаем сообщение как прочитанное для отправителя
        db.query(
          `INSERT INTO message_read_status (message_id, reader_id, is_read, read_at)
          VALUES (?, ?, TRUE, CURRENT_TIMESTAMP)`,
          [insertedMessageId, sender_id],
          (err) => {
            if (err) console.error('Error marking message as read for sender:', err);
          }
        );

        const sendResponseAndEvents = (messageData) => {
          // Отправляем ответ клиенту
          res.json(messageData);
          console.log(`📤 Ответ отправлен клиенту`);

          // 🔴 ОТПРАВЛЯЕМ SOCKET.IO СОБЫТИЕ
          console.log(`\n🔌 Socket.io EVENTS:`);
          
          // Отправляем отправителю
          io.to(`user_${sender_id}`).emit('new_message', messageData);
          console.log(`   ✅ Эмит 'new_message' в комнату: user_${sender_id}`);
          
          // Отправляем получателю
          io.to(`user_${receiver_id}`).emit('new_message', messageData);
          console.log(`   ✅ Эмит 'new_message' в комнату: user_${receiver_id}`);
          
          console.log(`✅ Personal message sent via Socket.io`);
          console.log(`${'='.repeat(70)}\n`);

          // 🔔 ОТПРАВКА PUSH УВЕДОМЛЕНИЯ ВСЕГДА (даже если онлайн)
          (async () => {
            try {
              console.log(`\n${'='.repeat(70)}`);
              console.log(`📱 PUSH ОТПРАВКА для получателя ${receiver_id}:`);

              const receiverToken = await getPushTokenFromDB(receiver_id);

              if (!receiverToken) {
                console.log(`   ⚠️ Push-токен не найден для пользователя ${receiver_id}`);
                console.log(`${'='.repeat(70)}\n`);
                return;
              }

              if (!Expo.isExpoPushToken(receiverToken)) {
                console.log(`   ⚠️ Невалидный токен: ${receiverToken}`);
                console.log(`${'='.repeat(70)}\n`);
                return;
              }

              const senderName = messageData?.sender_username || 'Пользователь';

              const pushMessage = {
                to: receiverToken,
                sound: 'default',
                title: `📨 ${senderName}`,
                body: (message || '').slice(0, 100),
                data: {
                  type: 'new_message',
                  sender_id: sender_id,
                  senderId: sender_id,
                  senderName: senderName,
                  message_id: insertedMessageId,
                  message: (message || '').slice(0, 100),
                  isGroup: false
                },
                badge: 1,
                priority: 'high'
              };

              try {
                const tickets = await expo.sendPushNotificationsAsync([pushMessage]);
                console.log(`   ✅ Push успешно отправлен`);
                console.log(`   📲 Токен: ${receiverToken.slice(0, 40)}...`);
                console.log(`${'='.repeat(70)}\n`);
              } catch (error) {
                console.error('   ❌ Ошибка отправки push:', error.message);
                console.log(`${'='.repeat(70)}\n`);
              }
            } catch (error) {
              console.error('❌ Ошибка при отправке push:', error.message);
            }
          })();
        };

        db.query(`
          SELECT m.*, 
                su.username as sender_username,
                su.avatar as sender_avatar,
                rm.message as reply_to_message,
                rm.sender_id as reply_to_sender_id,
                ru.username as reply_to_sender
          FROM messages m
          JOIN users su ON m.sender_id = su.id
          LEFT JOIN messages rm ON m.reply_to = rm.id
          LEFT JOIN users ru ON rm.sender_id = ru.id
          WHERE m.id = ?
        `, [insertedMessageId], (err, rows) => {
          if (err) {
            console.error('❌ Ошибка получения отправленного сообщения:', err);
            return sendResponseAndEvents({
              id: insertedMessageId,
              sender_id: sender_id,
              receiver_id,
              message,
              reply_to,
              media_type,
              media_url,
              duration: duration || null,
              caption: caption || null,
              created_at: new Date()
            });
          }

          const enrichedMessage = rows && rows[0] ? rows[0] : {
            id: insertedMessageId,
            sender_id: sender_id,
            receiver_id,
            message,
            reply_to,
            media_type,
            media_url,
            duration: duration || null,
            caption: caption || null,
            created_at: new Date()
          };

          sendResponseAndEvents(enrichedMessage);
        });
      }
    );
  });

  // Удалить личное сообщение
  app.delete('/api/messages/:messageId', authenticateToken, (req, res) => {
    const { messageId } = req.params;
    
    db.query(
      'SELECT sender_id, receiver_id FROM messages WHERE id = ?',
      [messageId],
      (err, results) => {
        if (err) return res.status(500).json({ error: 'Ошибка сервера' });
        if (results.length === 0) return res.status(404).json({ error: 'Сообщение не найдено' });
        
        const message = results[0];
        if (message.sender_id !== req.user.id) {
          return res.status(403).json({ error: 'Нет прав для удаления этого сообщения' });
        }
        
        db.query(
          'DELETE FROM messages WHERE id = ?',
          [messageId],
          (err, result) => {
            if (err) return res.status(500).json({ error: 'Ошибка сервера' });
            
            // ✅ ИСПРАВЛЕННО: Отправляем уведомление об удалении в комнаты пользователей
            io.to(`user_${message.sender_id}`).emit('message_deleted', { messageId });
            io.to(`user_${message.receiver_id}`).emit('message_deleted', { messageId });
            
            console.log(`✅ Message delete notification sent to user_${message.sender_id} and user_${message.receiver_id}`);
            
            res.json({ success: true, message: 'Сообщение удалено' });
          }
        );
      }
    );
  });

  // Удалить групповое сообщение
  app.delete('/api/groups/messages/:messageId', authenticateToken, (req, res) => {
    const { messageId } = req.params;
    
    // Проверяем что пользователь может удалить это сообщение
    db.query(
      'SELECT sender_id, group_id FROM group_messages WHERE id = ?',
      [messageId],
      (err, results) => {
        if (err) return res.status(500).json({ error: 'Ошибка сервера' });
        if (results.length === 0) return res.status(404).json({ error: 'Сообщение не найдено' });
        
        const message = results[0];
        if (message.sender_id !== req.user.id) {
          return res.status(403).json({ error: 'Нет прав для удаления этого сообщения' });
        }
        
        // Удаляем сообщение
        db.query(
          'DELETE FROM group_messages WHERE id = ?',
          [messageId],
          (err, result) => {
            if (err) return res.status(500).json({ error: 'Ошибка сервера' });
            
            // ✅ ИСПРАВЛЕННО: Отправляем уведомление об удалении только в комнату группы
            io.to(`group_${message.group_id}`).emit('group_message_deleted', { messageId });
            
            console.log(`✅ Group message delete notification sent to group_${message.group_id}`);
            
            res.json({ success: true, message: 'Сообщение удалено' });
          }
        );
      }
    );
  });

  // ========== ЭНДПОИНТЫ ДЛЯ ОЧИСТКИ И УДАЛЕНИЯ ЧАТА ==========

  // Очистить чат (удалить все сообщения между пользователями)
  app.post('/api/messages/clear-chat/:userId', authenticateToken, (req, res) => {
    const { userId } = req.params;
    const currentUserId = req.user.id;
    
    if (!userId || isNaN(parseInt(userId))) {
      return res.status(400).json({ error: 'userId обязателен и должен быть числом' });
    }
    
    const parsedUserId = parseInt(userId);
    
    if (parsedUserId === currentUserId) {
      return res.status(400).json({ error: 'Нельзя очистить чат с самим собой' });
    }
    
    // Шаг 1: Обнулить reply_to для предотвращения циклических ссылок
    db.query(`
      UPDATE messages SET reply_to = NULL
      WHERE (sender_id = ? AND receiver_id = ?) 
        OR (sender_id = ? AND receiver_id = ?)
    `, [currentUserId, parsedUserId, parsedUserId, currentUserId], (err) => {
      if (err) {
        console.error('⚠️ Error clearing reply_to:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }
      
      // Получаем все сообщения между пользователями
      db.query(`
        SELECT id FROM messages 
        WHERE (sender_id = ? AND receiver_id = ?) 
          OR (sender_id = ? AND receiver_id = ?)
      `, [currentUserId, parsedUserId, parsedUserId, currentUserId], (err, messages) => {
        if (err) {
          console.error('❌ Error fetching messages:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        
        const messageIds = messages.map(m => m.id);
        
        // Шаг 2: Удаляем закреплённые сообщения
        if (messageIds.length > 0) {
          db.query(`DELETE FROM pinned_messages WHERE message_id IN (${messageIds.join(',')})`, (err) => {
            if (err) console.error('⚠️ Error deleting pinned messages:', err);
            deleteReadStatus();
          });
        } else {
          deleteReadStatus();
        }
        
        function deleteReadStatus() {
          // Шаг 3: Удаляем статус прочтения сообщений
          db.query(`
            DELETE FROM message_read_status 
            WHERE message_id IN (
              SELECT id FROM messages 
              WHERE (sender_id = ? AND receiver_id = ?) 
                OR (sender_id = ? AND receiver_id = ?)
            )
          `, [currentUserId, parsedUserId, parsedUserId, currentUserId], (err) => {
            if (err) console.error('⚠️ Error deleting read status:', err);
            deleteMessages();
          });
        }
        
        function deleteMessages() {
          // Шаг 4: Удаляем все сообщения между пользователями
          db.query(`
            DELETE FROM messages 
            WHERE (sender_id = ? AND receiver_id = ?) 
              OR (sender_id = ? AND receiver_id = ?)
          `, [currentUserId, parsedUserId, parsedUserId, currentUserId], (err, result) => {
            if (err) {
              console.error('❌ Error deleting messages:', err);
              return res.status(500).json({ error: 'Ошибка сервера' });
            }
            
            console.log(`✅ Chat cleared between users ${currentUserId} and ${parsedUserId}. Deleted ${result.affectedRows} messages.`);
            
            // 📡 Отправляем Socket.IO событие для синхронизации очистки чата обоим пользователям
            const chatClearedData = {
              initiatorId: currentUserId,
              otherUserId: parsedUserId,
              timestamp: new Date().toISOString()
            };
            
            // Отправляем событие обоим пользователям
            io.to(`user_${currentUserId}`).emit('chat_cleared', chatClearedData);
            io.to(`user_${parsedUserId}`).emit('chat_cleared', chatClearedData);
            
            console.log(`📡 Socket.IO: chat_cleared отправлено пользователям ${currentUserId} и ${parsedUserId}`);
            
            res.json({ 
              success: true, 
              message: 'Чат очищен',
              deletedCount: result.affectedRows
            });
          });
        }
      });
    });
  });

  // Удалить чат (синоним для очистки, может использоваться как удаление)
  app.post('/api/messages/delete-chat/:userId', authenticateToken, (req, res) => {
    const { userId } = req.params;
    const currentUserId = req.user.id;
    
    if (!userId || isNaN(parseInt(userId))) {
      return res.status(400).json({ error: 'userId обязателен и должен быть числом' });
    }
    
    const parsedUserId = parseInt(userId);
    
    if (parsedUserId === currentUserId) {
      return res.status(400).json({ error: 'Нельзя удалить чат с самим собой' });
    }
    
    // Шаг 1: Обнулить reply_to для предотвращения циклических ссылок
    db.query(`
      UPDATE messages SET reply_to = NULL
      WHERE (sender_id = ? AND receiver_id = ?) 
        OR (sender_id = ? AND receiver_id = ?)
    `, [currentUserId, parsedUserId, parsedUserId, currentUserId], (err) => {
      if (err) {
        console.error('⚠️ Error clearing reply_to:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }
      
      // Получаем все сообщения между пользователями
      db.query(`
        SELECT id FROM messages 
        WHERE (sender_id = ? AND receiver_id = ?) 
          OR (sender_id = ? AND receiver_id = ?)
      `, [currentUserId, parsedUserId, parsedUserId, currentUserId], (err, messages) => {
        if (err) {
          console.error('❌ Error fetching messages:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        
        const messageIds = messages.map(m => m.id);
        
        // Шаг 2: Удаляем закреплённые сообщения
        if (messageIds.length > 0) {
          db.query(`DELETE FROM pinned_messages WHERE message_id IN (${messageIds.join(',')})`, (err) => {
            if (err) console.error('⚠️ Error deleting pinned messages:', err);
            deleteReadStatus();
          });
        } else {
          deleteReadStatus();
        }
        
        function deleteReadStatus() {
          // Шаг 3: Удаляем статус прочтения сообщений
          db.query(`
            DELETE FROM message_read_status 
            WHERE message_id IN (
              SELECT id FROM messages 
              WHERE (sender_id = ? AND receiver_id = ?) 
                OR (sender_id = ? AND receiver_id = ?)
            )
          `, [currentUserId, parsedUserId, parsedUserId, currentUserId], (err) => {
            if (err) console.error('⚠️ Error deleting read status:', err);
            deleteMessages();
          });
        }
        
        function deleteMessages() {
          // Шаг 4: Удаляем все сообщения между пользователями
          db.query(`
            DELETE FROM messages 
            WHERE (sender_id = ? AND receiver_id = ?) 
              OR (sender_id = ? AND receiver_id = ?)
          `, [currentUserId, parsedUserId, parsedUserId, currentUserId], (err, result) => {
            if (err) {
              console.error('❌ Error deleting messages:', err);
              return res.status(500).json({ error: 'Ошибка сервера' });
            }
            
            console.log(`✅ Chat deleted between users ${currentUserId} and ${parsedUserId}. Deleted ${result.affectedRows} messages.`);
            
            res.json({ 
              success: true, 
              message: 'Чат удален',
              deletedCount: result.affectedRows
            });
          });
        }
      });
    });
  });

  // WebSocket соединения
  // Получить профиль пользователя
  app.get('/api/profile', authenticateToken, (req, res) => {
    db.query('SELECT id, username, email, avatar, bio, status, cardColor FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      if (results.length === 0) return res.status(404).json({ error: 'Пользователь не найден' });
      res.json(results[0]);
    });
  });

  // Обновить профиль
  app.put('/api/profile', authenticateToken, (req, res) => {
    const { username, bio, status, avatar, cardColor } = req.body;
    
    // Валидация username ТОЛЬКО если он предоставлен
    if (username !== undefined && username !== null && username.length < 3) {
      return res.status(400).json({ error: 'Имя должно быть минимум 3 символа' });
    }
    
    // Подготавливаем SQL динамически
    const updates = [];
    const values = [];
    
    if (username !== undefined && username !== null) {
      updates.push('username = ?');
      values.push(username);
    }
    if (bio !== undefined) {
      updates.push('bio = ?');
      values.push(bio || null);
    }
    if (status !== undefined) {
      updates.push('status = ?');
      values.push(status || 'Онлайн');
    }
    if (avatar !== undefined) {
      updates.push('avatar = ?');
      values.push(avatar || null);
    }
    if (cardColor !== undefined) {
      updates.push('cardColor = ?');
      values.push(cardColor);
    }
    
    // Если ничего не было передано - возвращаем ошибку
    if (updates.length === 0) {
      return res.status(400).json({ error: 'Нет данных для обновления' });
    }
    
    // Добавляем ID пользователя в конец
    values.push(req.user.id);
    
    const query = `UPDATE users SET ${updates.join(', ')} WHERE id = ?`;
    
    console.log('🔧 SQL Query:', query);
    console.log('📊 Values:', values);
    
    db.query(query, values, (err, result) => {
      if (err) {
        console.error('❌ Ошибка обновления профиля:', err);
        if (err.code === 'ER_DUP_ENTRY') {
          return res.status(400).json({ error: 'Пользователь с таким именем уже существует' });
        }
        return res.status(500).json({ error: 'Ошибка сервера', details: err.message });
      }
      
      console.log(`✅ Профиль пользователя ${req.user.id} обновлен`);
      if (cardColor) console.log(`   Цвет карточки: ${cardColor}`);
      
      res.json({ 
        success: true,
        message: 'Профиль обновлен' 
      });
    });
  });

  // Получить предпочтения пользователя
  app.get('/api/user/preferences', authenticateToken, (req, res) => {
    db.query(
      'SELECT chat_background FROM user_preferences WHERE user_id = ?',
      [req.user.id],
      (err, results) => {
        if (err) {
          console.error('Database error:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        
        if (results.length === 0) {
          return res.json({ chat_background: 'default' });
        }
        
        res.json({ chat_background: results[0].chat_background });
      }
    );
  });

  // Сохранить предпочтения пользователя
  app.post('/api/user/preferences', authenticateToken, (req, res) => {
    const { chat_background } = req.body;
    
    if (!chat_background) {
      return res.status(400).json({ error: 'chat_background обязателен' });
    }
    
    db.query(
      `INSERT INTO user_preferences (user_id, chat_background) 
      VALUES (?, ?)
      ON DUPLICATE KEY UPDATE 
        chat_background = ?,
        updated_at = CURRENT_TIMESTAMP`,
      [req.user.id, chat_background, chat_background],
      (err) => {
        if (err) {
          console.error('Database error:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        
        res.json({ 
          success: true, 
          message: 'Предпочтения сохранены',
          chat_background 
        });
      }
    );
  });

  // Получить все посты
  app.get('/api/posts', authenticateToken, (req, res) => {
    db.query(`
      SELECT p.*, u.username, u.avatar,
            COUNT(DISTINCT l.id) as likes_count,
            COUNT(DISTINCT c.id) as comments_count,
            EXISTS(SELECT 1 FROM likes WHERE user_id = ? AND post_id = p.id) as is_liked,
            CASE WHEN p.user_id = ? THEN true ELSE false END as is_owner,
            p.created_at,
            p.updated_at
      FROM posts p
      JOIN users u ON p.user_id = u.id
      LEFT JOIN likes l ON p.id = l.post_id
      LEFT JOIN comments c ON p.id = c.post_id
      GROUP BY p.id
      ORDER BY p.created_at DESC
    `, [req.user.id, req.user.id], async (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      
      // ✨ ПРИМЕНЯЕМ ТРАНСФОРМАЦИЮ
      const transformedPosts = transformPostsData(results || []);
      
      // ✨ НОВОЕ: Получаем хештеги для каждого поста
      const postsWithHashtags = await Promise.all(
        transformedPosts.map(post => {
          return new Promise((resolve) => {
            db.query(`
              SELECT h.id, h.name
              FROM hashtags h
              JOIN post_hashtags ph ON h.id = ph.hashtag_id
              WHERE ph.post_id = ?
            `, [post.id], (err, hashtags) => {
              post.hashtags = hashtags || [];
              resolve(post);
            });
          });
        })
      );
      
      res.json(postsWithHashtags);
    });
  });

  // Создать пост - ОБНОВЛЕНО (поддержка множественных изображений)
  app.post('/api/posts', authenticateToken, (req, res) => {
    const { content, image, images } = req.body;
    
    console.log('\n' + '='.repeat(70));
    console.log('📤 СОЗДАНИЕ ПОСТА');
    console.log('   Содержание:', content ? content.substring(0, 50) + '...' : 'пусто');
    console.log('   Одиночное изображение:', !!image);
    console.log('   Массив изображений:', Array.isArray(images) ? `${images.length} шт` : 'нет');
    console.log('='.repeat(70));
    
    if (!content || content.trim().length === 0) {
      return res.status(400).json({ error: 'Текст поста обязателен' });
    }
    
    // Сохраняем массив изображений как JSON строку
    const imagesJson = Array.isArray(images) && images.length > 0 
      ? JSON.stringify(images) 
      : null;
    
    console.log('💾 Сохраняю в БД:');
    console.log('   image (первое):', image ? image.substring(0, 40) + '...' : 'null');
    console.log('   images (массив):', imagesJson ? `JSON с ${images.length} изображениями` : 'null');
    
    db.query(
      'INSERT INTO posts (user_id, content, image, images) VALUES (?, ?, ?, ?)',
      [req.user.id, content, image || null, imagesJson],
      (err, result) => {
        if (err) {
          console.error('❌ Ошибка сохранения поста:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        
        const postId = result.insertId;
        console.log(`✅ Пост ${postId} создан`);
        console.log('='.repeat(70) + '\n');
        
        // ✨ ТРАНСФОРМИРУЕМ: Преобразуем в нужный формат
        const transformedImages = Array.isArray(images) ? images : (image ? [image] : []);
        
        res.json({ 
          id: postId, 
          message: 'Пост создан',
          images: transformedImages
        });
      }
    );
  });

  // ========== HASHTAG ENDPOINTS ==========

  // Получить все хештеги
  app.get('/api/hashtags', authenticateToken, (req, res) => {
    db.query(`
      SELECT 
        h.id,
        h.name,
        COUNT(ph.post_id) as post_count,
        h.created_at
      FROM hashtags h
      LEFT JOIN post_hashtags ph ON h.id = ph.hashtag_id
      GROUP BY h.id, h.name, h.created_at
      ORDER BY post_count DESC, h.created_at DESC
      LIMIT 100
    `, (err, results) => {
      if (err) {
        console.error('Database error:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }
      res.json({ success: true, data: results });
    });
  });

  // Получить популярные хештеги
  app.get('/api/hashtags/trending', authenticateToken, (req, res) => {
    const limit = req.query.limit ? parseInt(req.query.limit) : 10;
    
    db.query(`
      SELECT 
        h.id,
        h.name,
        COUNT(ph.post_id) as post_count
      FROM hashtags h
      LEFT JOIN post_hashtags ph ON h.id = ph.hashtag_id
      GROUP BY h.id, h.name
      ORDER BY post_count DESC
      LIMIT ?
    `, [limit], (err, results) => {
      if (err) {
        console.error('Database error:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }
      res.json({ success: true, data: results });
    });
  });

  // Получить посты по хештегу
  app.get('/api/hashtags/:hashtag/posts', authenticateToken, (req, res) => {
    const { hashtag } = req.params;
    const limit = req.query.limit ? parseInt(req.query.limit) : 20;
    const offset = req.query.offset ? parseInt(req.query.offset) : 0;
    
    const hashtagName = hashtag.startsWith('#') ? hashtag.toLowerCase() : '#' + hashtag.toLowerCase();
    
    db.query(`
      SELECT 
        p.id,
        p.content,
        p.image,
        p.user_id,
        u.username as author_name,
        u.avatar,
        COUNT(DISTINCT l.id) as likes_count,
        COUNT(DISTINCT c.id) as comments_count,
        EXISTS(SELECT 1 FROM likes WHERE user_id = ? AND post_id = p.id) as is_liked,
        p.created_at,
        p.updated_at
      FROM posts p
      JOIN users u ON p.user_id = u.id
      LEFT JOIN likes l ON p.id = l.post_id
      LEFT JOIN comments c ON p.id = c.post_id
      WHERE p.id IN (
        SELECT ph.post_id 
        FROM post_hashtags ph
        JOIN hashtags h ON ph.hashtag_id = h.id
        WHERE h.name = ?
      )
      GROUP BY p.id
      ORDER BY p.created_at DESC
      LIMIT ? OFFSET ?
    `, [req.user.id, hashtagName, limit, offset], (err, results) => {
      if (err) {
        console.error('Database error:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }
      
      // Получаем общее количество постов
      db.query(`
        SELECT COUNT(DISTINCT ph.post_id) as total
        FROM post_hashtags ph
        JOIN hashtags h ON ph.hashtag_id = h.id
        WHERE h.name = ?
      `, [hashtagName], (err, countResults) => {
        if (err) console.error('Error getting count:', err);
        
        const total = countResults && countResults.length > 0 ? countResults[0].total : 0;
        
        res.json({
          success: true,
          data: results,
          total: total,
          limit: limit,
          offset: offset
        });
      });
    });
  });

  // Удалить хештег (ADMIN)
  app.delete('/api/admin/hashtags/:hashtagId', authenticateToken, (req, res) => {
    const { hashtagId } = req.params;
    
    // Проверяем администратора
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен' });
      }
      
      // Удаляем хештег (cascade удалит post_hashtags)
      db.query('DELETE FROM hashtags WHERE id = ?', [hashtagId], (err, result) => {
        if (err) {
          console.error('Error deleting hashtag:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        
        if (result.affectedRows === 0) {
          return res.status(404).json({ error: 'Хештег не найден' });
        }
        
        res.json({ success: true, message: 'Хештег удален' });
      });
    });
  });

  // Удалить пользователя (для админа) - НОВЫЙ ЭНДПОИНТ
  app.delete('/api/admin/users/:userId', authenticateToken, (req, res) => {
    const { userId } = req.params;
    
    // Проверяем, является ли текущий пользователь админом
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен. Требуются права администратора' });
      }
      
      // Проверяем, что админ не удаляет сам себя
      if (parseInt(userId) === req.user.id) {
        return res.status(400).json({ error: 'Нельзя удалить самого себя' });
      }
      
      // Шаг 1: Получаем все посты пользователя для удаления медиа
      db.query('SELECT id FROM posts WHERE user_id = ?', [userId], (err, posts) => {
        if (err) {
          console.error('❌ Error fetching user posts:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        
        const postIds = posts.map(p => p.id);
        
        // Шаг 2: Удаляем лайки постов
        if (postIds.length > 0) {
          db.query(`DELETE FROM likes WHERE post_id IN (${postIds.join(',')})`, (err) => {
            if (err) console.error('⚠️ Error deleting likes:', err);
            deleteComments();
          });
        } else {
          deleteComments();
        }
        
        function deleteComments() {
          // Шаг 3: Удаляем комментарии постов
          if (postIds.length > 0) {
            db.query(`DELETE FROM comments WHERE post_id IN (${postIds.join(',')})`, (err) => {
              if (err) console.error('⚠️ Error deleting comments:', err);
              deletePostHashtags();
            });
          } else {
            deletePostHashtags();
          }
        }
        
        function deletePostHashtags() {
          // Шаг 4: Удаляем связи хештегов с постами
          if (postIds.length > 0) {
            db.query(`DELETE FROM post_hashtags WHERE post_id IN (${postIds.join(',')})`, (err) => {
              if (err) console.error('⚠️ Error deleting post hashtags:', err);
              deletePosts();
            });
          } else {
            deletePosts();
          }
        }
        
        function deletePosts() {
          // Шаг 5: Удаляем посты пользователя
          db.query('DELETE FROM posts WHERE user_id = ?', [userId], (err) => {
            if (err) console.error('⚠️ Error deleting posts:', err);
            deleteMessages();
          });
        }
        
        function deleteMessages() {
          // Шаг 6: Удаляем личные сообщения (отправленные и полученные)
          db.query(`
            SELECT id FROM messages 
            WHERE sender_id = ? OR receiver_id = ?
          `, [userId, userId], (err, messages) => {
            if (err) console.error('⚠️ Error fetching messages:', err);
            
            const messageIds = messages ? messages.map(m => m.id) : [];
            
            if (messageIds.length > 0) {
              db.query(`DELETE FROM message_read_status WHERE message_id IN (${messageIds.join(',')})`, (err) => {
                if (err) console.error('⚠️ Error deleting message read status:', err);
                deleteMessagesPinned();
              });
            } else {
              deleteMessagesPinned();
            }
          });
        }
        
        function deleteMessagesPinned() {
          // Шаг 7: Удаляем закреплённые сообщения пользователя
          db.query('DELETE FROM pinned_messages WHERE user_id = ?', [userId], (err) => {
            if (err) console.error('⚠️ Error deleting pinned messages:', err);
            deleteMessages2();
          });
        }
        
        function deleteMessages2() {
          // Шаг 8: Удаляем все личные сообщения
          db.query('DELETE FROM messages WHERE sender_id = ? OR receiver_id = ?', [userId, userId], (err) => {
            if (err) console.error('⚠️ Error deleting messages:', err);
            deleteGroupMessages();
          });
        }
        
        function deleteGroupMessages() {
          // Шаг 9: Получаем все групповые сообщения пользователя
          db.query(`
            SELECT id FROM group_messages WHERE sender_id = ?
          `, [userId], (err, groupMessages) => {
            if (err) console.error('⚠️ Error fetching group messages:', err);
            
            const groupMessageIds = groupMessages ? groupMessages.map(m => m.id) : [];
            
            if (groupMessageIds.length > 0) {
              db.query(`DELETE FROM group_message_read_status WHERE message_id IN (${groupMessageIds.join(',')})`, (err) => {
                if (err) console.error('⚠️ Error deleting group message read status:', err);
                deletePinnedGroupMessages();
              });
            } else {
              deletePinnedGroupMessages();
            }
          });
        }
        
        function deletePinnedGroupMessages() {
          // Шаг 10: Удаляем закреплённые групповые сообщения пользователя
          // (они хранят message_id, поэтому нужно это сделать перед удалением самих сообщений)
          db.query(`
            DELETE FROM pinned_messages 
            WHERE message_id IN (
              SELECT id FROM group_messages WHERE sender_id = ?
            )
          `, [userId], (err) => {
            if (err) console.error('⚠️ Error deleting pinned group messages:', err);
            deleteGroupMessages2();
          });
        }
        
        function deleteGroupMessages2() {
          // Шаг 11: Удаляем групповые сообщения пользователя
          db.query('DELETE FROM group_messages WHERE sender_id = ?', [userId], (err) => {
            if (err) console.error('⚠️ Error deleting group messages:', err);
            deleteGroupMembers();
          });
        }
        
        function deleteGroupMembers() {
          // Шаг 12: Удаляем пользователя из групп
          db.query('DELETE FROM group_members WHERE user_id = ?', [userId], (err) => {
            if (err) console.error('⚠️ Error deleting from groups:', err);
            deleteFriends();
          });
        }
        
        function deleteFriends() {
          // Шаг 13: Удаляем друзей
          db.query('DELETE FROM friends WHERE user_id = ? OR friend_id = ?', [userId, userId], (err) => {
            if (err) console.error('⚠️ Error deleting friends:', err);
            deletePushTokens();
          });
        }
        
        function deletePushTokens() {
          // Шаг 14: Удаляем push-токены
          db.query('DELETE FROM push_tokens WHERE user_id = ?', [userId], (err) => {
            if (err) console.error('⚠️ Error deleting push tokens:', err);
            deleteUserPreferences();
          });
        }
        
        function deleteUserPreferences() {
          // Шаг 15: Удаляем предпочтения пользователя
          db.query('DELETE FROM user_preferences WHERE user_id = ?', [userId], (err) => {
            if (err) console.error('⚠️ Error deleting user preferences:', err);
            deleteCallsAndParticipants();
          });
        }
        
        function deleteCallsAndParticipants() {
          // Шаг 16: Удаляем звонки пользователя
          db.query(`
            SELECT id FROM calls 
            WHERE caller_id = ? OR receiver_id = ?
          `, [userId, userId], (err, calls) => {
            if (err) console.error('⚠️ Error fetching calls:', err);
            
            const callIds = calls ? calls.map(c => c.id) : [];
            
            if (callIds.length > 0) {
              db.query(`DELETE FROM call_participants WHERE call_id IN (${callIds.join(',')})`, (err) => {
                if (err) console.error('⚠️ Error deleting call participants:', err);
                deleteCalls();
              });
            } else {
              deleteCalls();
            }
          });
        }
        
        function deleteCalls() {
          // Шаг 17: Удаляем записи звонков
          db.query('DELETE FROM calls WHERE caller_id = ? OR receiver_id = ?', [userId, userId], (err) => {
            if (err) console.error('⚠️ Error deleting calls:', err);
            deletePinnedChats();
          });
        }
        
        function deletePinnedChats() {
          // Шаг 18: Удаляем закреплённые чаты
          db.query('DELETE FROM pinned_chats WHERE user_id = ?', [userId], (err) => {
            if (err) console.error('⚠️ Error deleting pinned chats:', err);
            deleteUser();
          });
        }
        
        function deleteUser() {
          // Шаг 19: Удаляем самого пользователя
          db.query('DELETE FROM users WHERE id = ?', [userId], (err, result) => {
            if (err) {
              console.error('❌ Error deleting user:', err);
              return res.status(500).json({ error: 'Ошибка сервера' });
            }
            
            if (result.affectedRows === 0) {
              return res.status(404).json({ error: 'Пользователь не найден' });
            }
            
            console.log(`🗑️ Пользователь ${userId} удален администратором ${req.user.id}`);
            
            res.json({ 
              success: true, 
              message: `Пользователь успешно удален`
            });
          });
        }
      });
    });
  });

  // Удалить пост (для админа) - НОВЫЙ ЭНДПОИНТ
  app.delete('/api/admin/posts/:postId', authenticateToken, (req, res) => {
    const { postId } = req.params;
    
    // Проверяем, является ли текущий пользователь админом
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен. Требуются права администратора' });
      }
      
      // Проверяем, существует ли пост
      db.query('SELECT user_id FROM posts WHERE id = ?', [postId], (err, results) => {
        if (err) {
          console.error('❌ Error checking post:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        
        if (results.length === 0) {
          return res.status(404).json({ error: 'Пост не найден' });
        }
        
        // Шаг 1: Удаляем лайки
        db.query('DELETE FROM likes WHERE post_id = ?', [postId], (err) => {
          if (err) console.error('⚠️ Error deleting likes:', err);
          
          // Шаг 2: Удаляем комментарии
          db.query('DELETE FROM comments WHERE post_id = ?', [postId], (err) => {
            if (err) console.error('⚠️ Error deleting comments:', err);
            
            // Шаг 3: Удаляем хештеги
            db.query('DELETE FROM post_hashtags WHERE post_id = ?', [postId], (err) => {
              if (err) console.error('⚠️ Error deleting post hashtags:', err);
              
              // Шаг 4: Удаляем пост
              db.query('DELETE FROM posts WHERE id = ?', [postId], (err, result) => {
                if (err) {
                  console.error('❌ Error deleting post:', err);
                  return res.status(500).json({ error: 'Ошибка сервера' });
                }
                
                console.log(`🗑️ Пост ${postId} удален администратором ${req.user.id}`);
                
                res.json({ 
                  success: true, 
                  message: 'Пост успешно удален'
                });
              });
            });
          });
        });
      });
    });
  });

  // ========== КОНЕЦ HASHTAG ENDPOINTS ==========;

  // GET /api/posts/authors/:authorId - Получить все посты конкретного автора
  app.get('/api/posts/authors/:authorId', authenticateToken, (req, res) => {
    const { authorId } = req.params;
    const userId = req.user.id;
    
    if (!authorId) {
      return res.status(400).json({ error: 'authorId is required' });
    }
    
    const query = `
      SELECT 
        p.id,
        p.content,
        p.image,
        p.images,
        p.created_at,
        p.updated_at,
        p.user_id,
        u.username,
        u.avatar,
        (SELECT COUNT(*) FROM likes WHERE post_id = p.id) as likes_count,
        (SELECT COUNT(*) FROM comments WHERE post_id = p.id) as comments_count,
        CASE WHEN EXISTS(SELECT 1 FROM likes WHERE post_id = p.id AND user_id = ?) THEN true ELSE false END as is_liked,
        CASE WHEN EXISTS(SELECT 1 FROM bookmarks WHERE post_id = p.id AND user_id = ?) THEN true ELSE false END as is_bookmarked
      FROM posts p
      LEFT JOIN users u ON p.user_id = u.id
      WHERE p.user_id = ?
      ORDER BY p.created_at DESC
    `;
    
    db.query(query, [userId, userId, parseInt(authorId)], (err, results) => {
      if (err) {
        console.error('❌ Ошибка загрузки постов автора:', err);
        return res.status(500).json({ error: err.message });
      }
      
      // ✨ ТРАНСФОРМИРУЕМ (нормализуем images поле)
      const transformedPosts = transformPostsData(results || []);
      
      console.log(`✅ Загружено ${results.length} постов для пользователя ${authorId}`);
      res.json(transformedPosts);
    });
  });

  // Получить конкретный пост по ID
  app.get('/api/posts/:id', authenticateToken, (req, res) => {
    const { id } = req.params;
    
    const query = `
      SELECT 
        p.id, 
        p.content, 
        p.image,
        p.images,
        p.user_id,
        u.username as author_name,
        u.avatar,
        (SELECT COUNT(*) FROM likes WHERE post_id = p.id) as likes_count,
        (SELECT COUNT(*) FROM comments WHERE post_id = p.id) as comments_count,
        CASE WHEN EXISTS(SELECT 1 FROM likes WHERE post_id = p.id AND user_id = ?) THEN true ELSE false END as is_liked,
        p.created_at,
        p.updated_at
      FROM posts p
      JOIN users u ON p.user_id = u.id
      WHERE p.id = ?
    `;
    
    db.query(query, [req.user.id, id], async (err, results) => {
      if (err) {
        console.error('❌ Ошибка получения поста:', err);
        return res.status(500).json({ error: 'Ошибка получения поста' });
      }
      
      if (results.length === 0) {
        return res.status(404).json({ error: 'Пост не найден' });
      }
      
      const post = results[0];
      
      // ✨ ТРАНСФОРМИРУЕМ
      const transformedPost = transformPostsData([post])[0];
      
      // ✨ НОВОЕ: Получаем хештеги поста
      db.query(`
        SELECT h.id, h.name
        FROM hashtags h
        JOIN post_hashtags ph ON h.id = ph.hashtag_id
        WHERE ph.post_id = ?
      `, [id], (err, hashtags) => {
        transformedPost.hashtags = hashtags || [];
        res.json(transformedPost);
      });
    });
  });

  // Обновить пост (только автор) - ОБНОВЛЕНО (поддержка множественных изображений)
  app.put('/api/posts/:id', authenticateToken, (req, res) => {
    const { id } = req.params;
    const { content, image, images } = req.body;
    
    if (!content && !image && !images) {
      return res.status(400).json({ error: 'Содержание или изображения обязательны' });
    }
    
    // Проверяем, является ли пользователь автором
    const checkQuery = 'SELECT user_id FROM posts WHERE id = ?';
    
    db.query(checkQuery, [id], async (err, results) => {
      if (err) {
        console.error('❌ Ошибка проверки автора:', err);
        return res.status(500).json({ error: 'Ошибка обновления поста' });
      }
      
      if (results.length === 0) {
        return res.status(404).json({ error: 'Пост не найден' });
      }
      
      if (results[0].user_id !== req.user.id) {
        return res.status(403).json({ error: 'Вы не можете редактировать чужой пост' });
      }
      
      // Подготавливаем данные
      const imagesJson = Array.isArray(images) && images.length > 0 
        ? JSON.stringify(images) 
        : null;
      
      // Обновляем пост
      const updateQuery = 'UPDATE posts SET content = ?, image = ?, images = ?, updated_at = NOW() WHERE id = ?';
      
      db.query(updateQuery, [content || null, image || null, imagesJson, id], async (err) => {
        if (err) {
          console.error('❌ Ошибка обновления поста:', err);
          return res.status(500).json({ error: 'Ошибка обновления поста' });
        }
        
        // ✨ НОВОЕ: Удаляем старые хештеги и сохраняем новые
        db.query('DELETE FROM post_hashtags WHERE post_id = ?', [id], async (err) => {
          if (err) console.error('❌ Ошибка удаления старых хештегов:', err);
          
          try {
            await extractAndSaveHashtags(content, id);
          } catch (err) {
            console.error('❌ Ошибка сохранения новых хештегов:', err);
          }
          
          // Возвращаем обновленный пост
          const getQuery = `
            SELECT 
              p.id, 
              p.content, 
              p.image,
              p.images,
              p.user_id,
              u.username as author_name,
              u.avatar,
              (SELECT COUNT(*) FROM likes WHERE post_id = p.id) as likes_count,
              (SELECT COUNT(*) FROM comments WHERE post_id = p.id) as comments_count,
              CASE WHEN EXISTS(SELECT 1 FROM likes WHERE post_id = p.id AND user_id = ?) THEN true ELSE false END as is_liked,
              p.created_at,
              p.updated_at
            FROM posts p
            JOIN users u ON p.user_id = u.id
            WHERE p.id = ?
          `;
          
          db.query(getQuery, [req.user.id, id], (err, results) => {
            if (err) return res.status(500).json({ error: 'Ошибка получения обновленного поста' });
            
            const post = results[0];
            const transformedPost = transformPostsData([post])[0];
            
            db.query(`
              SELECT h.id, h.name
              FROM hashtags h
              JOIN post_hashtags ph ON h.id = ph.hashtag_id
              WHERE ph.post_id = ?
            `, [id], (err, hashtags) => {
              transformedPost.hashtags = hashtags || [];
              res.json(transformedPost);
            });
          });
        });
      });
    });
  });

  // Удалить пост (только автор)
  app.delete('/api/posts/:id', authenticateToken, (req, res) => {
    const { id } = req.params;
    
    // Проверяем, является ли пользователь автором
    const checkQuery = 'SELECT user_id FROM posts WHERE id = ?';
    
    db.query(checkQuery, [id], (err, results) => {
      if (err) {
        console.error('Ошибка проверки автора:', err);
        return res.status(500).json({ error: 'Ошибка удаления поста' });
      }
      
      if (results.length === 0) {
        return res.status(404).json({ error: 'Пост не найден' });
      }
      
      if (results[0].user_id !== req.user.id) {
        return res.status(403).json({ error: 'Вы не можете удалить чужой пост' });
      }
      
      // Удаляем лайки
      db.query('DELETE FROM likes WHERE post_id = ?', [id], (err) => {
        if (err) console.error('Ошибка удаления лайков:', err);
        
        // Удаляем комментарии
        db.query('DELETE FROM comments WHERE post_id = ?', [id], (err) => {
          if (err) console.error('Ошибка удаления комментариев:', err);
          
          // Удаляем пост
          db.query('DELETE FROM posts WHERE id = ?', [id], (err) => {
            if (err) {
              console.error('Ошибка удаления поста:', err);
              return res.status(500).json({ error: 'Ошибка удаления поста' });
            }
            
            res.json({ success: true, message: 'Пост удален' });
          });
        });
      });
    });
  });

  // Лайкнуть/убрать лайк
  app.post('/api/posts/:postId/like', authenticateToken, (req, res) => {
    const { postId } = req.params;
    
    console.log(`[DEBUG] Like endpoint - postId: ${postId}, userId: ${req.user.id}`);
    
    db.query('SELECT * FROM likes WHERE user_id = ? AND post_id = ?', [req.user.id, postId], (err, results) => {
      if (err) {
        console.error('Database error in like endpoint:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }
      
      if (results.length > 0) {
        // Убрать лайк
        db.query('DELETE FROM likes WHERE user_id = ? AND post_id = ?', [req.user.id, postId], (err) => {
          if (err) {
            console.error('Error removing like:', err);
            return res.status(500).json({ error: 'Ошибка сервера' });
          }
          res.json({ liked: false, message: 'Лайк убран' });
        });
      } else {
        // Поставить лайк
        db.query('INSERT INTO likes (user_id, post_id) VALUES (?, ?)', [req.user.id, postId], (err) => {
          if (err) {
            console.error('Error adding like:', err);
            return res.status(500).json({ error: 'Ошибка сервера' });
          }
          res.json({ liked: true, message: 'Лайк поставлен' });
        });
      }
    });
  });

  // Добавить комментарий
  app.post('/api/posts/:postId/comments', authenticateToken, (req, res) => {
    const { postId } = req.params;
    const { comment } = req.body;
    
    if (!comment || comment.trim().length === 0) {
      return res.status(400).json({ error: 'Комментарий не может быть пустым' });
    }
    
    db.query(
      'INSERT INTO comments (user_id, post_id, comment) VALUES (?, ?, ?)',
      [req.user.id, postId, comment],
      (err, result) => {
        if (err) return res.status(500).json({ error: 'Ошибка сервера' });
        res.json({ id: result.insertId, message: 'Комментарий добавлен' });
      }
    );
  });

  // Получить комментарии к посту
  app.get('/api/posts/:postId/comments', authenticateToken, (req, res) => {
    const { postId } = req.params;
    
    db.query(`
      SELECT c.*, u.username, u.avatar
      FROM comments c
      JOIN users u ON c.user_id = u.id
      WHERE c.post_id = ?
      ORDER BY c.created_at ASC
    `, [postId], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      res.json(results);
    });
  });

  // Редактировать комментарий (только автор)
  app.put('/api/comments/:id', authenticateToken, (req, res) => {
    const { id } = req.params;
    const { comment } = req.body;
    
    if (!comment || !comment.trim()) {
      return res.status(400).json({ error: 'Комментарий не может быть пустым' });
    }
    
    // Проверяем, является ли пользователь автором
    const checkQuery = 'SELECT user_id FROM comments WHERE id = ?';
    
    db.query(checkQuery, [id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка' });
      if (results.length === 0) return res.status(404).json({ error: 'Комментарий не найден' });
      if (results[0].user_id !== req.user.id) {
        return res.status(403).json({ error: 'Вы не можете редактировать чужой комментарий' });
      }
      
      db.query('UPDATE comments SET comment = ?, updated_at = NOW() WHERE id = ?', 
        [comment.trim(), id], 
        (err) => {
          if (err) return res.status(500).json({ error: 'Ошибка обновления' });
          res.json({ success: true });
        }
      );
    });
  });

  // Удалить комментарий (только автор)
  app.delete('/api/comments/:id', authenticateToken, (req, res) => {
    const { id } = req.params;
    
    // Проверяем, является ли пользователь автором
    const checkQuery = 'SELECT user_id FROM comments WHERE id = ?';
    
    db.query(checkQuery, [id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка' });
      if (results.length === 0) return res.status(404).json({ error: 'Комментарий не найден' });
      if (results[0].user_id !== req.user.id) {
        return res.status(403).json({ error: 'Вы не можете удалить чужой комментарий' });
      }
      
      db.query('DELETE FROM comments WHERE id = ?', [id], (err) => {
        if (err) return res.status(500).json({ error: 'Ошибка удаления' });
        res.json({ success: true });
      });
    });
  });

  // ============================================
  // 📌 API ENDPOINTS ДЛЯ НОВЫХ ФУНКЦИЙ ПОСТОВ
  // ============================================

  // 🔧 ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ
  function updateUserInterests(userId, postId) {
    db.query(
      `SELECT hashtag_id FROM post_hashtags WHERE post_id = ?`,
      [postId],
      (err, hashtags) => {
        if (err || !hashtags) return;

        hashtags.forEach(({ hashtag_id }) => {
          db.query(
            `INSERT INTO user_interests (user_id, hashtag_id, interest_score) VALUES (?, ?, 1.0)
            ON DUPLICATE KEY UPDATE interest_score = interest_score + 0.5`,
            [userId, hashtag_id]
          );
        });

        db.query(
          'UPDATE users SET interests_updated_at = NOW() WHERE id = ?',
          [userId]
        );
      }
    );
  }

  function trackPostView(userId, postId) {
    db.query(
      `INSERT INTO post_views (post_id, user_id) VALUES (?, ?)
      ON DUPLICATE KEY UPDATE viewed_at = NOW()`,
      [postId, userId],
      (err) => {
        if (!err) {
          db.query(
            `UPDATE posts SET views_count = (SELECT COUNT(*) FROM post_views WHERE post_id = ?) WHERE id = ?`,
            [postId, postId]
          );
          updateUserInterests(userId, postId);
        }
      }
    );
  }

  // 1️⃣ ПОЛУЧИТЬ ПОСТЫ С СОРТИРОВКОЙ И ФИЛЬТРАЦИЕЙ
  app.get('/api/posts/feed', authenticateToken, (req, res) => {
    const userId = req.user.id;
    const { 
      sort = 'newest',
      filter = 'all',
      search = '',
      author_id = null,
      page = 1,
      limit = 20
    } = req.query;

    const offset = (parseInt(page) - 1) * parseInt(limit);
    let whereConditions = [];
    let params = [];

    if (filter === 'friends') {
      whereConditions.push(`p.user_id IN (
        SELECT CASE 
          WHEN f.user_id = ? THEN f.friend_id 
          ELSE f.user_id 
        END
        FROM friends f 
        WHERE (f.user_id = ? OR f.friend_id = ?) AND f.status = 'accepted'
      )`);
      params.push(userId, userId, userId);
    } else if (author_id) {
      whereConditions.push('p.user_id = ?');
      params.push(parseInt(author_id));
    }

    if (search && search.trim()) {
      const searchTerm = `%${search.trim()}%`;
      whereConditions.push(`(
        p.content LIKE ? OR 
        u.username LIKE ?
      )`);
      params.push(searchTerm, searchTerm);
    }

    let query = `
      SELECT 
        p.*,
        u.username,
        u.avatar,
        (SELECT COUNT(*) FROM likes WHERE post_id = p.id) as likes_count,
        (SELECT COUNT(*) FROM comments WHERE post_id = p.id) as comments_count,
        (SELECT COUNT(*) FROM post_views WHERE post_id = p.id) as views_count,
        EXISTS(SELECT 1 FROM likes WHERE user_id = ? AND post_id = p.id) as is_liked,
        EXISTS(SELECT 1 FROM bookmarks WHERE user_id = ? AND post_id = p.id) as is_bookmarked,
        CASE WHEN p.user_id = ? THEN true ELSE false END as is_owner
      FROM posts p
      JOIN users u ON p.user_id = u.id
    `;

    params.unshift(userId, userId, userId);

    if (whereConditions.length > 0) {
      query += ' WHERE ' + whereConditions.join(' AND ');
    }

    if (sort === 'popular') {
      query += ` ORDER BY (SELECT COUNT(*) FROM likes WHERE post_id = p.id) DESC, p.created_at DESC`;
    } else {
      query += ` ORDER BY p.created_at DESC`;
    }

    query += ` LIMIT ? OFFSET ?`;
    params.push(parseInt(limit), offset);

    db.query(query, params, (err, posts) => {
      if (err) {
        console.error('❌ Ошибка получения постов:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }

      // ✨ ТРАНСФОРМИРУЕМ
      const transformedPosts = transformPostsData(posts || []);

      let countQuery = `SELECT COUNT(*) as total FROM posts p JOIN users u ON p.user_id = u.id`;
      if (whereConditions.length > 0) {
        countQuery += ' WHERE ' + whereConditions.join(' AND ');
      }

      db.query(countQuery, params.slice(0, params.length - 2), (err, countResult) => {
        const total = countResult?.[0]?.total || 0;

        res.json({
          success: true,
          data: transformedPosts,
          pagination: {
            total,
            page: parseInt(page),
            limit: parseInt(limit),
            pages: Math.ceil(total / parseInt(limit))
          }
        });
      });
    });
  });

  // 2️⃣ ПОИСК ПОСТОВ
  app.get('/api/posts/search', authenticateToken, (req, res) => {
    const userId = req.user.id;
    const { q = '', limit = 20 } = req.query;

    if (!q || q.trim().length < 2) {
      return res.status(400).json({ error: 'Запрос должен содержать минимум 2 символа' });
    }

    const searchTerm = `%${q.trim()}%`;
    let query = `
      SELECT DISTINCT
        p.*,
        u.username,
        u.avatar,
        EXISTS(SELECT 1 FROM likes WHERE user_id = ? AND post_id = p.id) as is_liked,
        EXISTS(SELECT 1 FROM bookmarks WHERE user_id = ? AND post_id = p.id) as is_bookmarked
      FROM posts p
      JOIN users u ON p.user_id = u.id
      WHERE 
        p.content LIKE ? OR
        u.username LIKE ?
      ORDER BY p.created_at DESC
      LIMIT ?
    `;

    db.query(query, [userId, userId, searchTerm, searchTerm, parseInt(limit)], 
      (err, results) => {
        if (err) {
          console.error('❌ Ошибка поиска:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }

        // ✨ ТРАНСФОРМИРУЕМ
        const transformedResults = transformPostsData(results || []);

        res.json({
          success: true,
          query: q,
          results: transformedResults,
          count: transformedResults?.length || 0
        });
      }
    );
  });

  // 3️⃣ ЗАКЛАДКИ (СОХРАНЕНИЕ ПОСТОВ)
  app.post('/api/posts/:postId/bookmark', authenticateToken, (req, res) => {
    const { postId } = req.params;
    const userId = req.user.id;

    db.query(
      'INSERT INTO bookmarks (user_id, post_id) VALUES (?, ?)',
      [userId, postId],
      (err, result) => {
        if (err) {
          if (err.code === 'ER_DUP_ENTRY') {
            return res.status(400).json({ error: 'Пост уже добавлен в закладки' });
          }
          return res.status(500).json({ error: 'Ошибка сервера' });
        }

        updateUserInterests(userId, postId);
        console.log(`✅ Пост ${postId} добавлен в закладки пользователя ${userId}`);
        res.json({ success: true, message: 'Пост добавлен в закладки' });
      }
    );
  });

  app.delete('/api/posts/:postId/bookmark', authenticateToken, (req, res) => {
    const { postId } = req.params;
    const userId = req.user.id;

    db.query(
      'DELETE FROM bookmarks WHERE user_id = ? AND post_id = ?',
      [userId, postId],
      (err) => {
        if (err) {
          return res.status(500).json({ error: 'Ошибка сервера' });
        }

        console.log(`✅ Пост ${postId} удален из закладок пользователя ${userId}`);
        res.json({ success: true, message: 'Пост удален из закладок' });
      }
    );
  });

  app.get('/api/posts/bookmarks/my', authenticateToken, (req, res) => {
    const userId = req.user.id;
    const { page = 1, limit = 20 } = req.query;
    const offset = (parseInt(page) - 1) * parseInt(limit);

    const query = `
      SELECT 
        p.*,
        u.username,
        u.avatar,
        (SELECT COUNT(*) FROM likes WHERE post_id = p.id) as likes_count,
        (SELECT COUNT(*) FROM comments WHERE post_id = p.id) as comments_count,
        EXISTS(SELECT 1 FROM likes WHERE user_id = ? AND post_id = p.id) as is_liked,
        EXISTS(SELECT 1 FROM bookmarks WHERE user_id = ? AND post_id = p.id) as is_bookmarked,
        b.bookmarked_at
      FROM bookmarks b
      JOIN posts p ON b.post_id = p.id
      JOIN users u ON p.user_id = u.id
      WHERE b.user_id = ?
      ORDER BY b.bookmarked_at DESC
      LIMIT ? OFFSET ?
    `;

    db.query(query, [userId, userId, userId, parseInt(limit), offset], (err, bookmarks) => {
      if (err) {
        console.error('❌ Ошибка получения закладок:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }

      // ✨ ТРАНСФОРМИРУЕМ
      const transformedBookmarks = transformPostsData(bookmarks || []);

      db.query(
        'SELECT COUNT(*) as total FROM bookmarks WHERE user_id = ?',
        [userId],
        (err, countResult) => {
          const total = countResult?.[0]?.total || 0;

          res.json({
            success: true,
            data: transformedBookmarks,
            pagination: {
              total,
              page: parseInt(page),
              limit: parseInt(limit),
              pages: Math.ceil(total / parseInt(limit))
            }
          });
        }
      );
    });
  });

  // 4️⃣ ФИЛЬТР ПО АВТОРАМ (ДРУЗЬЯ)
  app.get('/api/posts/authors/friends', authenticateToken, (req, res) => {
    const userId = req.user.id;
    const { page = 1, limit = 20 } = req.query;
    const offset = (parseInt(page) - 1) * parseInt(limit);

    const query = `
      SELECT 
        p.*,
        u.username,
        u.avatar,
        (SELECT COUNT(*) FROM likes WHERE post_id = p.id) as likes_count,
        (SELECT COUNT(*) FROM comments WHERE post_id = p.id) as comments_count,
        EXISTS(SELECT 1 FROM likes WHERE user_id = ? AND post_id = p.id) as is_liked,
        EXISTS(SELECT 1 FROM bookmarks WHERE user_id = ? AND post_id = p.id) as is_bookmarked
      FROM posts p
      JOIN users u ON p.user_id = u.id
      WHERE p.user_id IN (
        SELECT CASE 
          WHEN f.user_id = ? THEN f.friend_id 
          ELSE f.user_id 
        END as friend_id
        FROM friends f 
        WHERE (f.user_id = ? OR f.friend_id = ?) 
        AND f.status = 'accepted'
      )
      ORDER BY p.created_at DESC
      LIMIT ? OFFSET ?
    `;

    db.query(query, [userId, userId, userId, userId, userId, parseInt(limit), offset], 
      (err, posts) => {
        if (err) {
          console.error('❌ Ошибка получения постов друзей:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }

        // ✨ ТРАНСФОРМИРУЕМ
        const transformedPosts = transformPostsData(posts || []);

        res.json({
          success: true,
          data: transformedPosts,
          filter: 'friends_only',
          count: transformedPosts.length
        });
      }
    );
  });

  // 5️⃣ ОТСЛЕЖИВАНИЕ ПРОСМОТРОВ
  app.post('/api/posts/:postId/view', authenticateToken, (req, res) => {
    const { postId } = req.params;
    const userId = req.user.id;

    trackPostView(userId, postId);
    res.json({ success: true, message: 'Просмотр зафиксирован' });
  });

  // ========== ADMIN ENDPOINTS ==========

  // Проверить является ли пользователь администратором
  app.get('/api/admin/check', authenticateToken, (req, res) => {
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      if (results.length === 0) return res.status(404).json({ error: 'Пользователь не найден' });
      res.json({ is_admin: results[0].is_admin || false });
    });
  });

  // Получить все группы (для админа)
  app.get('/api/admin/groups', authenticateToken, (req, res) => {
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен' });
      }
      
      db.query(`
        SELECT g.*, COUNT(gm.user_id) as member_count, u.username as creator_name
        FROM groups g
        LEFT JOIN group_members gm ON g.id = gm.group_id
        LEFT JOIN users u ON g.created_by = u.id
        GROUP BY g.id
        ORDER BY g.created_at DESC
      `, (err, results) => {
        if (err) return res.status(500).json({ error: 'Ошибка сервера' });
        res.json(results);
      });
    });
  });

  // Удалить группу (для админа)
  app.delete('/api/admin/groups/:groupId', authenticateToken, (req, res) => {
    const { groupId } = req.params;
    
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err) {
        console.error('❌ Admin check error:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }
      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен' });
      }
      
      // Сначала получаем все сообщения группы
      db.query('SELECT id FROM group_messages WHERE group_id = ?', [groupId], (err, messages) => {
        if (err) {
          console.error('❌ Error fetching group messages:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        
        const messageIds = messages.map(m => m.id);
        
        // Шаг 1: Обнулить reply_to для предотвращения циклических ссылок
        if (messageIds.length > 0) {
          db.query(`UPDATE group_messages SET reply_to = NULL WHERE group_id = ?`, [groupId], (err) => {
            if (err) console.error('⚠️ Error clearing reply_to:', err);
            continueGroupDeletion();
          });
        } else {
          continueGroupDeletion();
        }
        
        function continueGroupDeletion() {
          // Шаг 2: Удаляем закреплённые сообщения группы
          if (messageIds.length > 0) {
            db.query(`DELETE FROM pinned_messages WHERE message_id IN (${messageIds.join(',')})`, (err) => {
              if (err) console.error('⚠️ Error deleting pinned messages:', err);
              deleteReadStatus();
            });
          } else {
            deleteReadStatus();
          }
        }
        
        function deleteReadStatus() {
          // Шаг 3: Удаляем статус прочтения сообщений группы (по message_id)
          if (messageIds.length > 0) {
            db.query(`DELETE FROM group_message_read_status WHERE message_id IN (${messageIds.join(',')})`, (err) => {
              if (err) console.error('⚠️ Error deleting read status:', err);
              deleteMembers();
            });
          } else {
            deleteMembers();
          }
        }
        
        function deleteMembers() {
          // Шаг 4: Удаляем членов группы
          db.query('DELETE FROM group_members WHERE group_id = ?', [groupId], (err) => {
            if (err) {
              console.error('❌ Error deleting group members:', err);
              return res.status(500).json({ error: 'Ошибка сервера' });
            }
            deleteMessages();
          });
        }
        
        function deleteMessages() {
          // Шаг 5: Удаляем сообщения группы
          db.query('DELETE FROM group_messages WHERE group_id = ?', [groupId], (err) => {
            if (err) {
              console.error('❌ Error deleting group messages:', err);
              return res.status(500).json({ error: 'Ошибка сервера' });
            }
            deleteGroup();
          });
        }
        
        function deleteGroup() {
          // Шаг 6: Удаляем саму группу
          db.query('DELETE FROM groups WHERE id = ?', [groupId], (err) => {
            if (err) {
              console.error('❌ Error deleting group:', err);
              return res.status(500).json({ error: 'Ошибка сервера' });
            }
            console.log(`✅ Group ${groupId} deleted successfully`);
            res.json({ success: true, message: 'Группа удалена' });
          });
        }
      });
    });
  });

  // Получить все чаты (для админа)
  app.get('/api/admin/chats', authenticateToken, (req, res) => {
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен' });
      }
      
      db.query(`
        SELECT DISTINCT
          LEAST(m.sender_id, m.receiver_id) as user1_id,
          GREATEST(m.sender_id, m.receiver_id) as user2_id,
          MAX(m.created_at) as last_message_time,
          COUNT(m.id) as message_count
        FROM messages m
        GROUP BY user1_id, user2_id
        ORDER BY last_message_time DESC
      `, (err, results) => {
        if (err) return res.status(500).json({ error: 'Ошибка сервера' });
        
        // Получаем информацию о пользователях для каждого чата
        const chatsWithUsers = results.map(chat => {
          return new Promise((resolve, reject) => {
            db.query(`
              SELECT id, username, avatar FROM users WHERE id IN (?, ?)
            `, [chat.user1_id, chat.user2_id], (err, users) => {
              if (err) reject(err);
              resolve({
                user1: users.find(u => u.id === chat.user1_id),
                user2: users.find(u => u.id === chat.user2_id),
                message_count: chat.message_count,
                last_message_time: chat.last_message_time
              });
            });
          });
        });
        
        Promise.all(chatsWithUsers)
          .then(chats => res.json(chats))
          .catch(err => res.status(500).json({ error: 'Ошибка сервера' }));
      });
    });
  });

  // Удалить чат (для админа)
  app.delete('/api/admin/chats/:userId1/:userId2', authenticateToken, (req, res) => {
    const { userId1, userId2 } = req.params;
    
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err) {
        console.error('❌ Admin check error:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }
      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен' });
      }
      
      // Шаг 1: Обнулить reply_to для предотвращения циклических ссылок
      db.query(`
        UPDATE messages SET reply_to = NULL
        WHERE (sender_id = ? AND receiver_id = ?) 
          OR (sender_id = ? AND receiver_id = ?)
      `, [userId1, userId2, userId2, userId1], (err) => {
        if (err) console.error('⚠️ Error clearing reply_to:', err);
        fetchAndDelete();
      });
      
      function fetchAndDelete() {
        // Получаем все сообщения между пользователями
        db.query(`
          SELECT id FROM messages 
          WHERE (sender_id = ? AND receiver_id = ?) 
            OR (sender_id = ? AND receiver_id = ?)
        `, [userId1, userId2, userId2, userId1], (err, messages) => {
          if (err) {
            console.error('❌ Error fetching messages:', err);
            return res.status(500).json({ error: 'Ошибка сервера' });
          }
          
          const messageIds = messages.map(m => m.id);
          
          // Шаг 2: Удаляем закреплённые сообщения
          if (messageIds.length > 0) {
            db.query(`DELETE FROM pinned_messages WHERE message_id IN (${messageIds.join(',')})`, (err) => {
              if (err) console.error('⚠️ Error deleting pinned messages:', err);
              deleteReadStatus();
            });
          } else {
            deleteReadStatus();
          }
          
          function deleteReadStatus() {
            // Шаг 3: Удаляем статус прочтения сообщений
            db.query(`
              DELETE FROM message_read_status 
              WHERE message_id IN (
                SELECT id FROM messages 
                WHERE (sender_id = ? AND receiver_id = ?) 
                  OR (sender_id = ? AND receiver_id = ?)
              )
            `, [userId1, userId2, userId2, userId1], (err) => {
              if (err) console.error('⚠️ Error deleting read status:', err);
              deleteMessages();
            });
          }
          
          function deleteMessages() {
            // Шаг 4: Удаляем все сообщения между этими пользователями
            db.query(`
              DELETE FROM messages 
              WHERE (sender_id = ? AND receiver_id = ?) 
                OR (sender_id = ? AND receiver_id = ?)
            `, [userId1, userId2, userId2, userId1], (err) => {
              if (err) {
                console.error('❌ Error deleting messages:', err);
                return res.status(500).json({ error: 'Ошибка сервера' });
              }
              console.log(`✅ Chat between users ${userId1} and ${userId2} deleted successfully`);
              res.json({ success: true, message: 'Чат удален' });
            });
          }
        });
      }
    });
  });

  // ========== CALL ENDPOINTS ==========

  // Инициировать аудиозвонок
  app.post('/api/calls/audio/initiate', authenticateToken, (req, res) => {
    const { receiver_id } = req.body;
    const caller_id = req.user.id;
    
    if (!receiver_id) {
      return res.status(400).json({ error: 'receiver_id обязателен' });
    }
    
    if (receiver_id == caller_id) {
      return res.status(400).json({ error: 'Нельзя позвонить самому себе' });
    }
    
    // Проверяем, существует ли получатель
    db.query('SELECT id FROM users WHERE id = ?', [receiver_id], (err, results) => {
      if (err) {
        console.error('Database error:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }
      
      if (results.length === 0) {
        return res.status(404).json({ error: 'Пользователь не найден' });
      }
      
      // Создаем запись звонка
      db.query(
        'INSERT INTO calls (caller_id, receiver_id, call_type, status) VALUES (?, ?, ?, ?)',
        [caller_id, receiver_id, 'audio', 'pending'],
        (err, result) => {
          if (err) {
            console.error('Database error:', err);
            return res.status(500).json({ error: 'Ошибка сервера' });
          }
          
          const callId = result.insertId;
          
          // Добавляем участников звонка
          const participants = [
            [callId, caller_id],
            [callId, receiver_id]
          ];
          
          db.query(
            'INSERT INTO call_participants (call_id, user_id) VALUES ?',
            [participants],
            (err) => {
              if (err) console.error('Error adding participants:', err);
            }
          );
          
          // Отправляем уведомление через WebSocket
          io.emit('incoming_call', {
            call_id: callId,
            caller_id,
            receiver_id,
            call_type: 'audio',
            caller_name: req.user.username,
            created_at: new Date()
          });
          
          console.log(`✅ Audio call initiated: ${caller_id} -> ${receiver_id} (Call ID: ${callId})`);
          
          res.json({
            success: true,
            call_id: callId,
            call_type: 'audio',
            receiver_id,
            status: 'pending',
            message: 'Аудиозвонок инициирован'
          });
        }
      );
    });
  });

  // Инициировать видеозвонок
  app.post('/api/calls/video/initiate', authenticateToken, (req, res) => {
    const { receiver_id } = req.body;
    const caller_id = req.user.id;
    
    if (!receiver_id) {
      return res.status(400).json({ error: 'receiver_id обязателен' });
    }
    
    if (receiver_id == caller_id) {
      return res.status(400).json({ error: 'Нельзя позвонить самому себе' });
    }
    
    // Проверяем, существует ли получатель
    db.query('SELECT id FROM users WHERE id = ?', [receiver_id], (err, results) => {
      if (err) {
        console.error('Database error:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }
      
      if (results.length === 0) {
        return res.status(404).json({ error: 'Пользователь не найден' });
      }
      
      // Создаем запись звонка
      db.query(
        'INSERT INTO calls (caller_id, receiver_id, call_type, status) VALUES (?, ?, ?, ?)',
        [caller_id, receiver_id, 'video', 'pending'],
        (err, result) => {
          if (err) {
            console.error('Database error:', err);
            return res.status(500).json({ error: 'Ошибка сервера' });
          }
          
          const callId = result.insertId;
          
          // Добавляем участников звонка
          const participants = [
            [callId, caller_id],
            [callId, receiver_id]
          ];
          
          db.query(
            'INSERT INTO call_participants (call_id, user_id) VALUES ?',
            [participants],
            (err) => {
              if (err) console.error('Error adding participants:', err);
            }
          );
          
          // Отправляем уведомление через WebSocket
          io.emit('incoming_call', {
            call_id: callId,
            caller_id,
            receiver_id,
            call_type: 'video',
            caller_name: req.user.username,
            created_at: new Date()
          });
          
          console.log(`✅ Video call initiated: ${caller_id} -> ${receiver_id} (Call ID: ${callId})`);
          
          res.json({
            success: true,
            call_id: callId,
            call_type: 'video',
            receiver_id,
            status: 'pending',
            message: 'Видеозвонок инициирован'
          });
        }
      );
    });
  });

  // Ответить на звонок
  app.post('/api/calls/:callId/respond', authenticateToken, (req, res) => {
    const { callId } = req.params;
    const { accept = true } = req.body;
    const userId = req.user.id;
    
    // Получаем информацию о звонке
    db.query('SELECT * FROM calls WHERE id = ?', [callId], (err, results) => {
      if (err) {
        console.error('Database error:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }
      
      if (results.length === 0) {
        return res.status(404).json({ error: 'Звонок не найден' });
      }
      
      const call = results[0];
      
      // Проверяем, что это получатель звонка
      if (call.receiver_id != userId) {
        return res.status(403).json({ error: 'Это не ваш звонок' });
      }
      
      // Проверяем статус звонка
      if (call.status !== 'pending') {
        return res.status(400).json({ error: 'Звонок уже завершен или отклонен' });
      }
      
      const newStatus = accept ? 'accepted' : 'rejected';
      const startedAt = accept ? new Date() : null;
      
      // Обновляем статус звонка
      db.query(
        'UPDATE calls SET status = ?, started_at = ? WHERE id = ?',
        [newStatus, startedAt, callId],
        (err) => {
          if (err) {
            console.error('Database error:', err);
            return res.status(500).json({ error: 'Ошибка сервера' });
          }
          
          // Уведомляем через WebSocket
          io.emit('call_response', {
            call_id: callId,
            status: newStatus,
            receiver_id: userId,
            caller_id: call.caller_id
          });
          
          console.log(`✅ Call ${callId} ${newStatus} by user ${userId}`);
          
          res.json({
            success: true,
            call_id: callId,
            status: newStatus,
            message: accept ? 'Звонок принят' : 'Звонок отклонен'
          });
        }
      );
    });
  });

  // Завершить звонок
  app.post('/api/calls/:callId/end', authenticateToken, (req, res) => {
    const { callId } = req.params;
    const userId = req.user.id;
    
    // Получаем информацию о звонке
    db.query('SELECT * FROM calls WHERE id = ?', [callId], (err, results) => {
      if (err) {
        console.error('Database error:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }
      
      if (results.length === 0) {
        return res.status(404).json({ error: 'Звонок не найден' });
      }
      
      const call = results[0];
      
      // Проверяем, что это участник звонка
      if (call.caller_id != userId && call.receiver_id != userId) {
        return res.status(403).json({ error: 'Вы не участник этого звонка' });
      }
      
      // Рассчитываем длительность звонка
      let duration = 0;
      if (call.started_at) {
        const endTime = new Date();
        const startTime = new Date(call.started_at);
        duration = Math.floor((endTime - startTime) / 1000); // в секундах
      }
      
      // Обновляем статус звонка
      const finalStatus = call.status === 'pending' ? 'missed' : 'ended';
      
      db.query(
        'UPDATE calls SET status = ?, ended_at = ?, duration = ? WHERE id = ?',
        [finalStatus, new Date(), duration, callId],
        (err) => {
          if (err) {
            console.error('Database error:', err);
            return res.status(500).json({ error: 'Ошибка сервера' });
          }
          
          // Уведомляем через WebSocket
          io.emit('call_ended', {
            call_id: callId,
            status: finalStatus,
            duration,
            ended_by: userId
          });
          
          console.log(`✅ Call ${callId} ended. Duration: ${duration}s`);
          
          res.json({
            success: true,
            call_id: callId,
            status: finalStatus,
            duration,
            message: 'Звонок завершен'
          });
        }
      );
    });
  });

  // Получить статус звонка
  app.get('/api/calls/:callId/status', authenticateToken, (req, res) => {
    const { callId } = req.params;
    
    db.query('SELECT * FROM calls WHERE id = ?', [callId], (err, results) => {
      if (err) {
        console.error('Database error:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }
      
      if (results.length === 0) {
        return res.status(404).json({ error: 'Звонок не найден' });
      }
      
      const call = results[0];
      
      res.json({
        call_id: call.id,
        caller_id: call.caller_id,
        receiver_id: call.receiver_id,
        call_type: call.call_type,
        status: call.status,
        duration: call.duration,
        started_at: call.started_at,
        ended_at: call.ended_at,
        created_at: call.created_at
      });
    });
  });

  // Получить историю звонков пользователя
  app.get('/api/calls/history', authenticateToken, (req, res) => {
    const userId = req.user.id;
    const limit = req.query.limit ? parseInt(req.query.limit) : 50;
    
    db.query(`
      SELECT 
        c.*,
        u_caller.username as caller_name,
        u_caller.avatar as caller_avatar,
        u_receiver.username as receiver_name,
        u_receiver.avatar as receiver_avatar
      FROM calls c
      LEFT JOIN users u_caller ON c.caller_id = u_caller.id
      LEFT JOIN users u_receiver ON c.receiver_id = u_receiver.id
      WHERE c.caller_id = ? OR c.receiver_id = ?
      ORDER BY c.created_at DESC
      LIMIT ?
    `, [userId, userId, limit], (err, results) => {
      if (err) {
        console.error('Database error:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }
      
      res.json(results);
    });
  });

  // ========== ТЕСТИРОВАНИЕ PUSH-УВЕДОМЛЕНИЙ ==========
  app.post('/api/test-push/:userId', async (req, res) => {
    const { userId } = req.params;
    
    try {
      const pushToken = await getPushTokenFromDB(userId);
      
      if (!pushToken) {
        return res.status(400).json({ error: 'Push токен не найден' });
      }
      
      if (!Expo.isExpoPushToken(pushToken)) {
        return res.status(400).json({ error: 'Невалидный push токен' });
      }
      
      const message = {
        to: pushToken,
        sound: 'default',
        title: '🧪 Тестовое уведомление',
        body: 'Если ты это видишь - push работают! 🎉',
        data: {
          type: 'test',
          timestamp: new Date().toISOString()
        },
        badge: 1,
        priority: 'high'
      };
      
      const tickets = await expo.sendPushNotificationsAsync([message]);
      console.log(`📬 Тестовый push отправлен пользователю ${userId}`);
      
      res.json({ 
        success: true, 
        message: 'Тестовый push отправлен',
        token: pushToken.slice(0, 40) + '...'
      });
    } catch (error) {
      console.error('❌ Ошибка тестового push:', error);
      res.status(500).json({ error: error.message });
    }
  });

  // 🔍 ОТЛАДОЧНЫЙ ЭНДПОИНТ: Получить информацию о push-токене пользователя
  app.get('/api/debug/push-token/:userId', (req, res) => {
    const { userId } = req.params;
    
    console.log(`\n📊 [DEBUG] Запрос информации о push-токене для user_id=${userId}`);
    
    db.query(
      'SELECT id, user_id, push_token, device_type, device_name, updated_at FROM push_tokens WHERE user_id = ?',
      [userId],
      (err, results) => {
        if (err) {
          console.error(`❌ Ошибка запроса БД:`, err.message);
          return res.status(500).json({ error: err.message });
        }
        
        if (!results || results.length === 0) {
          console.log(`❌ Токен не найден для user_id=${userId}`);
          return res.status(404).json({ 
            error: 'Токен не найден',
            userId,
            message: 'Пользователь не зарегистрировал push-токен'
          });
        }
        
        const token = results[0];
        const pushToken = token.push_token;
        const isValid = Expo.isExpoPushToken(pushToken);
        
        console.log(`✅ Найден токен для user_id=${userId}:`);
        console.log(`   Токен: ${pushToken.substring(0, 50)}...`);
        console.log(`   Валиден: ${isValid}`);
        console.log(`   Устройство: ${token.device_type} (${token.device_name})`);
        console.log(`   Обновлен: ${token.updated_at}`);
        
        res.json({
          userId,
          found: true,
          token: {
            tokenPrefix: pushToken.substring(0, 50) + '...',
            fullToken: pushToken,
            isValid: isValid,
            deviceType: token.device_type,
            deviceName: token.device_name,
            updatedAt: token.updated_at
          },
          message: isValid ? '✅ Токен валиден и готов к использованию' : '❌ Токен невалиден'
        });
      }
    );
  });

  // 🔍 ОТЛАДОЧНЫЙ ЭНДПОИНТ: Получить все push-токены в системе
  app.get('/api/debug/push-tokens/all', (req, res) => {
    console.log(`\n📊 [DEBUG] Запрос всех push-токенов`);
    
    db.query(
      'SELECT id, user_id, push_token, device_type, device_name, updated_at FROM push_tokens ORDER BY updated_at DESC',
      [],
      (err, results) => {
        if (err) {
          console.error(`❌ Ошибка запроса БД:`, err.message);
          return res.status(500).json({ error: err.message });
        }
        
        const tokenStats = {
          totalTokens: results ? results.length : 0,
          validTokens: 0,
          invalidTokens: 0,
          tokens: []
        };
        
        if (results && results.length > 0) {
          results.forEach((token) => {
            const isValid = Expo.isExpoPushToken(token.push_token);
            if (isValid) tokenStats.validTokens++;
            else tokenStats.invalidTokens++;
            
            tokenStats.tokens.push({
              userId: token.user_id,
              tokenPrefix: token.push_token.substring(0, 40) + '...',
              isValid: isValid,
              deviceType: token.device_type,
              deviceName: token.device_name,
              updatedAt: token.updated_at
            });
          });
        }
        
        console.log(`📊 Статистика push-токенов:`);
        console.log(`   Всего токенов: ${tokenStats.totalTokens}`);
        console.log(`   Валидных: ${tokenStats.validTokens}`);
        console.log(`   Невалидных: ${tokenStats.invalidTokens}`);
        
        res.json(tokenStats);
      }
    );
  });

  // ========== АВТОСОХРАНЕНИЕ ТОКЕНОВ КАЖДЫЕ 5 МИНУТ ==========
  setInterval(() => {
    saveTokensToFile(userPushTokens);
    console.log(`💾 Токены сохранены в файл (${Object.keys(userPushTokens).length} токенов)`);
  }, 5 * 60 * 1000);

  // ========== МОНИТОРИНГ СЕРВЕРА: Статус ==========
  const serverStartTime = Date.now();
  const serverLogs = [];

  // Добавляем лог в очередь
  const addLog = (message, level = 'info') => {
    const log = {
      timestamp: new Date().toISOString(),
      message,
      level
    };
    serverLogs.push(log);
    // Хранимм максимум 200 логов
    if (serverLogs.length > 200) {
      serverLogs.shift();
    }
    console.log(`[${level.toUpperCase()}] ${message}`);
  };

  // Эндпоинт: Статус сервера
  app.get('/api/server/status', authenticateToken, (req, res) => {
    const uptime = Math.floor((Date.now() - serverStartTime) / 1000);
    
    // Подсчитываем активных пользователей (в Socket.io комнатах)
    const activeUsers = userSockets.size;
    
    // Получаем статистику из БД
    db.query('SELECT COUNT(*) as count FROM groups', (err, groupResults) => {
      const activeGroups = groupResults?.[0]?.count || 0;
      
      db.query('SELECT COUNT(*) as count FROM messages UNION ALL SELECT COUNT(*) as count FROM group_messages', (err, messageResults) => {
        const totalMessages = messageResults?.reduce((sum, row) => sum + (row.count || 0), 0) || 0;
        
        // Получаем информацию об использовании памяти
        const memUsage = process.memoryUsage();
        const memMB = (memUsage.heapUsed / 1024 / 1024).toFixed(2);
        
        res.json({
          status: 'online',
          uptime,
          activeUsers,
          activeGroups,
          totalMessages,
          memory: `${memMB} MB`,
          nodeVersion: process.version,
          platform: process.platform
        });
      });
    });
  });

  // Эндпоинт: Логи сервера
  app.get('/api/admin/server/logs', authenticateToken, (req, res) => {
    // Проверяем что это админ
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err || !results?.length || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен' });
      }
      
      res.json({ logs: serverLogs });
    });
  });

  // Эндпоинт: Перезагрузка сервера
  app.post('/api/admin/server/restart', authenticateToken, (req, res) => {
    // Проверяем что это админ
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err || !results?.length || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен' });
      }
      
      addLog('Сервер перезагружается по команде админа', 'warning');
      res.json({ message: 'Сервер будет перезагружен...' });
      
      // Отправляем уведомление всем пользователям
      io.emit('server_restarting', { message: 'Сервер сейчас перезагружается' });
      
      // Перезагружаем через 2 секунды
      setTimeout(() => {
        process.exit(0);
      }, 2000);
    });
  });

  // 🔑 РЕГИСТРАЦИЯ PUSH ТОКЕНА (REST API)
  app.post('/users/push-token', authenticateToken, (req, res) => {
    const { pushToken, deviceType, deviceName } = req.body;
    const userId = req.user.id;
    
    if (!pushToken) {
      console.log('❌ Push-токен не предоставлен');
      return res.status(400).json({ error: 'Push токен не предоставлен' });
    }
    
    console.log(`🔑 Регистрация push-токена для пользователя ${userId}`);
    console.log(`   Токен: ${String(pushToken).slice(0, 40)}...`);
    console.log(`   Устройство: ${deviceType || 'unknown'}`);
    
    db.query(
      `INSERT INTO push_tokens (user_id, push_token, device_type, device_name) 
      VALUES (?, ?, ?, ?)
      ON DUPLICATE KEY UPDATE 
        push_token = VALUES(push_token),
        device_type = VALUES(device_type),
        device_name = VALUES(device_name),
        updated_at = CURRENT_TIMESTAMP`,
      [userId, pushToken, deviceType || 'unknown', deviceName || 'unknown'],
      (err, result) => {
        if (err) {
          console.error('❌ Ошибка регистрации токена:', err.message);
          return res.status(500).json({ error: 'Ошибка при регистрации токена' });
        }
        
        console.log(`✅ Push-токен успешно зарегистрирован для пользователя ${userId}`);
        res.json({ success: true, message: 'Push-токен зарегистрирован' });
      }
    );
  });

  // ============================================
  // ЖАЛОБЫ НА ПОСТЫ
  // ============================================

  // 1️⃣ ENDPOINT: Пожаловаться на пост
  app.post('/api/posts/:postId/report', authenticateToken, (req, res) => {
    const { postId } = req.params;
    const { reason } = req.body;
    const userId = req.user.id;

    if (!reason || reason.trim().length === 0) {
      return res.status(400).json({ error: 'Причина жалобы обязательна' });
    }

    if (reason.trim().length > 500) {
      return res.status(400).json({ error: 'Причина не должна превышать 500 символов' });
    }

    // Проверяем, существует ли пост
    db.query('SELECT id FROM posts WHERE id = ?', [postId], (err, results) => {
      if (err) {
        console.error('❌ Ошибка проверки поста:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }

      if (results.length === 0) {
        return res.status(404).json({ error: 'Пост не найден' });
      }

      // Проверяем, не жаловался ли уже этот пользователь на этот пост
      db.query(
        'SELECT id FROM post_reports WHERE post_id = ? AND reporter_id = ? AND status != "rejected"',
        [postId, userId],
        (err, existingReports) => {
          if (err) {
            console.error('❌ Ошибка проверки существующей жалобы:', err);
            return res.status(500).json({ error: 'Ошибка сервера' });
          }

          if (existingReports.length > 0) {
            return res.status(400).json({ 
              error: 'Вы уже пожаловались на этот пост' 
            });
          }

          // Добавляем новую жалобу
          db.query(
            'INSERT INTO post_reports (post_id, reporter_id, reason) VALUES (?, ?, ?)',
            [postId, userId, reason.trim()],
            (err, result) => {
              if (err) {
                console.error('❌ Ошибка создания жалобы:', err);
                return res.status(500).json({ error: 'Ошибка сервера' });
              }

              console.log(`✅ Жалоба на пост ${postId} создана пользователем ${userId}`);

              res.json({
                success: true,
                message: 'Жалоба отправлена администраторам',
                reportId: result.insertId
              });
            }
          );
        }
      );
    });
  });

  // 2️⃣ ENDPOINT: Получить все жалобы (админ)
  app.get('/api/admin/post-reports', authenticateToken, (req, res) => {
    const userId = req.user.id;

    // Проверяем, является ли пользователь админом
    db.query('SELECT is_admin FROM users WHERE id = ?', [userId], (err, results) => {
      if (err) {
        console.error('❌ Ошибка проверки роли пользователя:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }

      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ только для администраторов' });
      }

      // Получаем все жалобы с информацией о постах и пользователях
      const query = `
        SELECT 
          pr.id,
          pr.post_id,
          pr.reporter_id,
          pr.reason,
          pr.status,
          pr.reviewed_by,
          pr.reviewed_at,
          pr.created_at,
          p.content as post_content,
          p.user_id as post_author_id,
          u_reporter.username as reporter_username,
          u_reporter.avatar as reporter_avatar,
          u_author.username as post_author_username,
          u_author.avatar as post_author_avatar,
          u_reviewer.username as reviewer_username
        FROM post_reports pr
        JOIN posts p ON pr.post_id = p.id
        JOIN users u_reporter ON pr.reporter_id = u_reporter.id
        JOIN users u_author ON p.user_id = u_author.id
        LEFT JOIN users u_reviewer ON pr.reviewed_by = u_reviewer.id
        ORDER BY 
          CASE 
            WHEN pr.status = 'pending' THEN 1
            WHEN pr.status = 'reviewed' THEN 2
            ELSE 3
          END,
          pr.created_at DESC
      `;

      db.query(query, (err, reports) => {
        if (err) {
          console.error('❌ Ошибка получения жалоб:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }

        res.json(reports);
      });
    });
  });

  // 3️⃣ ENDPOINT: Получить жалобы конкретного поста
  app.get('/api/posts/:postId/reports', authenticateToken, (req, res) => {
    const { postId } = req.params;
    const userId = req.user.id;

    // Проверяем, является ли пользователь админом
    db.query('SELECT is_admin FROM users WHERE id = ?', [userId], (err, results) => {
      if (err) {
        console.error('❌ Ошибка проверки роли пользователя:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }

      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ только для администраторов' });
      }

      const query = `
        SELECT 
          pr.*,
          u_reporter.username as reporter_username,
          u_reporter.avatar as reporter_avatar,
          u_reviewer.username as reviewer_username
        FROM post_reports pr
        JOIN users u_reporter ON pr.reporter_id = u_reporter.id
        LEFT JOIN users u_reviewer ON pr.reviewed_by = u_reviewer.id
        WHERE pr.post_id = ?
        ORDER BY pr.created_at DESC
      `;

      db.query(query, [postId], (err, reports) => {
        if (err) {
          console.error('❌ Ошибка получения жалоб поста:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }

        res.json(reports);
      });
    });
  });

  // 4️⃣ ENDPOINT: Обработать жалобу (удалить пост или отклонить жалобу)
  app.post('/api/admin/post-reports/:reportId/:action', authenticateToken, (req, res) => {
    const { reportId, action } = req.params;
    const { ban_duration_days } = req.body;
    const adminId = req.user.id;

    // Проверяем, является ли пользователь админом
    db.query('SELECT is_admin FROM users WHERE id = ?', [adminId], (err, results) => {
      if (err) {
        console.error('❌ Ошибка проверки роли администратора:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }

      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ только для администраторов' });
      }

      // Получаем информацию о жалобе
      db.query(
        'SELECT post_id, reporter_id FROM post_reports WHERE id = ?',
        [reportId],
        (err, reportResults) => {
          if (err) {
            console.error('❌ Ошибка получения жалобы:', err);
            return res.status(500).json({ error: 'Ошибка сервера' });
          }

          if (reportResults.length === 0) {
            return res.status(404).json({ error: 'Жалоба не найдена' });
          }

          const { post_id, reporter_id } = reportResults[0];

          if (action === 'approve') {
            // Удаляем пост
            db.query('DELETE FROM posts WHERE id = ?', [post_id], (err) => {
              if (err) {
                console.error('❌ Ошибка удаления поста:', err);
                return res.status(500).json({ error: 'Ошибка сервера' });
              }

              // Отмечаем жалобу как approved
              db.query(
                'UPDATE post_reports SET status = "approved", reviewed_by = ?, reviewed_at = NOW() WHERE id = ?',
                [adminId, reportId],
                (err) => {
                  if (err) {
                    console.error('❌ Ошибка обновления жалобы:', err);
                    return res.status(500).json({ error: 'Ошибка сервера' });
                  }

                  console.log(`✅ Жалоба ${reportId} одобрена. Пост ${post_id} удален администратором ${adminId}`);

                  // Отправляем обновление через Socket.io
                  io.emit('post_deleted_by_admin', {
                    post_id: post_id,
                    reason: 'Пост удален по жалобе'
                  });

                  res.json({
                    success: true,
                    message: 'Пост удален по жалобе'
                  });
                }
              );
            });
          } else if (action === 'reject') {
            // Отмечаем жалобу как rejected
            db.query(
              'UPDATE post_reports SET status = "rejected", reviewed_by = ?, reviewed_at = NOW() WHERE id = ?',
              [adminId, reportId],
              (err) => {
                if (err) {
                  console.error('❌ Ошибка обновления жалобы:', err);
                  return res.status(500).json({ error: 'Ошибка сервера' });
                }

                console.log(`✅ Жалоба ${reportId} отклонена администратором ${adminId}`);

                res.json({
                  success: true,
                  message: 'Жалоба отклонена'
                });
              }
            );
          } else if (action === 'ban-user') {
            // Блокируем пользователя-нарушителя
            const reason = 'Заблокирован за нарушения содержания (жалоба на пост)';
            const { ban_duration_days } = req.body;
            let bannedUntil = null;

            // Рассчитываем дату разбана
            if (ban_duration_days !== null && ban_duration_days !== undefined) {
              const now = new Date();
              bannedUntil = new Date(now.getTime() + ban_duration_days * 24 * 60 * 60 * 1000);
            }

            db.query(
              'UPDATE users SET is_banned = TRUE, ban_reason = ?, banned_at = CURRENT_TIMESTAMP, banned_by = ?, banned_until = ? WHERE id = ?',
              [reason, adminId, bannedUntil, reporter_id],
              (err, result) => {
                if (err) {
                  console.error('❌ Ошибка блокирования пользователя:', err);
                  return res.status(500).json({ error: 'Ошибка сервера', details: err.message });
                }

                if (result.affectedRows === 0) {
                  return res.status(404).json({ error: 'Пользователь не найден' });
                }

                // Отмечаем жалобу как approved
                db.query(
                  'UPDATE post_reports SET status = "approved", reviewed_by = ?, reviewed_at = NOW() WHERE id = ?',
                  [adminId, reportId],
                  (err) => {
                    if (err) {
                      console.error('❌ Ошибка обновления жалобы:', err);
                      return res.status(500).json({ error: 'Ошибка сервера' });
                    }

                    console.log(`✅ Пользователь ${reporter_id} заблокирован администратором ${adminId} за нарушение содержания`);

                    const message = ban_duration_days === null 
                      ? 'Пользователь заблокирован навсегда'
                      : `Пользователь заблокирован на ${ban_duration_days} дней`;

                    res.json({
                      success: true,
                      message: message
                    });
                  }
                );
              }
            );
          } else {
            return res.status(400).json({ error: 'Неизвестное действие' });
          }
        }
      );
    });
  });

  // 5️⃣ ENDPOINT: Получить статистику жалоб
  app.get('/api/admin/post-reports-stats', authenticateToken, (req, res) => {
    const userId = req.user.id;

    // Проверяем, является ли пользователь админом
    db.query('SELECT is_admin FROM users WHERE id = ?', [userId], (err, results) => {
      if (err) {
        console.error('❌ Ошибка проверки роли пользователя:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }

      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ только для администраторов' });
      }

      const query = `
        SELECT 
          status,
          COUNT(*) as count
        FROM post_reports
        GROUP BY status
      `;

      db.query(query, (err, stats) => {
        if (err) {
          console.error('❌ Ошибка получения статистики жалоб:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }

        const statsObj = {
          pending: 0,
          reviewed: 0,
          approved: 0,
          rejected: 0,
          total: 0
        };

        stats.forEach(stat => {
          statsObj[stat.status] = stat.count;
          statsObj.total += stat.count;
        });

        res.json(statsObj);
      });
    });
  });

  // 6️⃣ ENDPOINT: Отметить жалобу как "просмотрено"
  app.patch('/api/admin/post-reports/:reportId/mark-reviewed', authenticateToken, (req, res) => {
    const { reportId } = req.params;
    const adminId = req.user.id;

    // Проверяем, является ли пользователь админом
    db.query('SELECT is_admin FROM users WHERE id = ?', [adminId], (err, results) => {
      if (err) {
        console.error('❌ Ошибка проверки роли администратора:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }

      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ только для администраторов' });
      }

      db.query(
        'UPDATE post_reports SET status = "reviewed" WHERE id = ? AND status = "pending"',
        [reportId],
        (err) => {
          if (err) {
            console.error('❌ Ошибка обновления жалобы:', err);
            return res.status(500).json({ error: 'Ошибка сервера' });
          }

          console.log(`✅ Жалоба ${reportId} отмечена как просмотрено администратором ${adminId}`);

          res.json({
            success: true,
            message: 'Жалоба отмечена как просмотрено'
          });
        }
      );
    });
  });

  // ============================================
  // БАН ПОЛЬЗОВАТЕЛЯ (из админ-панели "Люди")
  // ============================================

  app.post('/admin/users/:userId/ban', authenticateToken, (req, res) => {
    const { userId } = req.params;
    const { reason = 'Нарушение правил сообщества', ban_duration_days } = req.body;
    
    // Проверяем, является ли текущий пользователь админом
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен. Требуются права администратора' });
      }
      
      // Проверяем, что админ не забанивает сам себя
      if (parseInt(userId) === req.user.id) {
        return res.status(400).json({ error: 'Нельзя забанить самого себя' });
      }
      
      // Рассчитываем дату разбана
      let bannedUntil = null;
      if (ban_duration_days !== null && ban_duration_days !== undefined && ban_duration_days > 0) {
        const now = new Date();
        bannedUntil = new Date(now.getTime() + ban_duration_days * 24 * 60 * 60 * 1000);
      }
      // Если null или undefined - бан навсегда, banned_until остается null
      
      // Забаниваем пользователя
      db.query(
        'UPDATE users SET is_banned = TRUE, ban_reason = ?, banned_at = CURRENT_TIMESTAMP, banned_until = ?, banned_by = ? WHERE id = ?',
        [reason, bannedUntil, req.user.id, userId],
        (err, result) => {
          if (err) {
            console.error('❌ Error banning user:', err);
            return res.status(500).json({ error: 'Ошибка сервера' });
          }
          
          if (result.affectedRows === 0) {
            return res.status(404).json({ error: 'Пользователь не найден' });
          }
          
          const durationText = ban_duration_days === null 
            ? 'навсегда' 
            : `на ${ban_duration_days} дней`;
          
          console.log(`🚫 Пользователь ${userId} забанен администратором ${req.user.id} ${durationText}. Причина: ${reason}`);
          
          // Отправляем уведомление о бане через WebSocket если пользователь онлайн
          const userSocketIds = getUserSocketIds(userId);
          if (userSocketIds.length > 0) {
            userSocketIds.forEach((socketId) => {
              io.to(socketId).emit('user_banned', {
                reason: reason,
                message: 'Ваш аккаунт заблокирован администратором',
                bannedUntil: bannedUntil
              });
            });
          }
          
          res.json({ 
            success: true, 
            message: `Пользователь ${userId} успешно забанен ${durationText}`,
            reason: reason,
            bannedUntil: bannedUntil
          });
        }
      );
    });
  });

  // Разбанить пользователя (ADMIN)
  app.post('/api/admin/users/:userId/unban', authenticateToken, (req, res) => {
    const { userId } = req.params;
    
    // Проверяем, является ли текущий пользователь админом
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен. Требуются права администратора' });
      }
      
      // Разбаниваем пользователя
      db.query(
        'UPDATE users SET is_banned = FALSE, ban_reason = NULL, banned_at = NULL, banned_until = NULL, banned_by = NULL WHERE id = ?',
        [userId],
        (err, result) => {
          if (err) {
            console.error('❌ Error unbanning user:', err);
            return res.status(500).json({ error: 'Ошибка сервера' });
          }
          
          if (result.affectedRows === 0) {
            return res.status(404).json({ error: 'Пользователь не найден' });
          }
          
          console.log(`✅ Пользователь ${userId} разбанен администратором ${req.user.id}`);
          
          res.json({ 
            success: true, 
            message: `Пользователь ${userId} успешно разбанен`
          });
        }
      );
    });
  });

  // ============================================
  // 📋 ДОПОЛНИТЕЛЬНЫЕ ENDPOINT'Ы ДЛЯ АДМИНИСТРАЦИИ
  // ============================================

  // ============================================
  // 1️⃣ ОЧИСТКА БАЗЫ ДАННЫХ (ADMIN)
  // ============================================
  app.post('/api/admin/database/clear', authenticateToken, async (req, res) => {
    const { confirmation } = req.body;
    
    // Проверяем, является ли пользователь админом
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], async (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен. Требуются права администратора' });
      }
      
      // Требуем подтверждение
      if (confirmation !== 'CLEAR_ALL_DATA') {
        return res.status(400).json({ 
          error: 'Требуется подтверждение',
          message: 'Отправьте { confirmation: "CLEAR_ALL_DATA" } чтобы подтвердить очистку'
        });
      }
      
      try {
        console.log(`\n${'='.repeat(70)}`);
        console.log(`🗑️  НАЧАЛО ОЧИСТКИ БАЗЫ ДАННЫХ`);
        console.log(`   Администратор: ${req.user.id}`);
        console.log(`   Время: ${new Date().toISOString()}`);
        console.log(`${'='.repeat(70)}\n`);
        
        // Отключаем проверку FOREIGN KEY перед удалением
        await new Promise((resolve, reject) => {
          db.query('SET FOREIGN_KEY_CHECKS = 0', (err) => {
            if (err) reject(err);
            else resolve();
          });
        });
        
        // Получаем все таблицы в БД
        const getTables = () => new Promise((resolve, reject) => {
          db.query(`SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME NOT IN ('information_schema')`, (err, tables) => {
            if (err) reject(err);
            else resolve(tables.map(t => t.TABLE_NAME));
          });
        });
        
        const tables = await getTables();
        console.log(`📊 Найдено таблиц для очистки: ${tables.length}`);
        
        // Удаляем данные из всех таблиц КРОМЕ пользователя, который выполнил очистку
        let totalDeleted = 0;
        
        for (const table of tables) {
          const query = table === 'users' 
            ? `DELETE FROM ${table} WHERE id != ${req.user.id}`
            : `DELETE FROM ${table}`;
          
          const result = await new Promise((resolve, reject) => {
            db.query(query, (err, result) => {
              if (err) {
                console.warn(`⚠️  Ошибка при очистке ${table}: ${err.message}`);
                resolve(null);
              } else {
                console.log(`✅ Очищена таблица: ${table} (${result.affectedRows || 0} строк)`);
                totalDeleted += result.affectedRows || 0;
                resolve(result);
              }
            });
          });
        }
        
        // Включаем обратно проверку FOREIGN KEY
        await new Promise((resolve, reject) => {
          db.query('SET FOREIGN_KEY_CHECKS = 1', (err) => {
            if (err) reject(err);
            else resolve();
          });
        });
        
        console.log(`\n${'='.repeat(70)}`);
        console.log(`✅ БАЗА ДАННЫХ ОЧИЩЕНА УСПЕШНО`);
        console.log(`${'='.repeat(70)}\n`);
        
        // Отправляем уведомление всем онлайн пользователям
        io.emit('database_cleared', {
          message: 'База данных была очищена администратором',
          cleared_by: req.user.id,
          cleared_at: new Date().toISOString()
        });
        
        res.json({ 
          success: true, 
          message: 'База данных полностью очищена',
          tables_cleared: tables.length,
          total_rows_deleted: totalDeleted
        });
        
      } catch (error) {
        console.error('❌ Критическая ошибка при очистке БД:', error);
        res.status(500).json({ 
          error: 'Ошибка при очистке базы данных',
          details: error.message 
        });
      }
    });
  });

  // ============================================
  // 2️⃣ ПОЛУЧИТЬ СТАТИСТИКУ СЕРВЕРА (ADMIN)
  // ============================================
  app.get('/admin/statistics', authenticateToken, (req, res) => {
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен' });
      }
      
      const stats = {};
      let completedQueries = 0;
      const totalQueries = 10;
      
      db.query('SELECT COUNT(*) as count FROM users', (err, result) => {
        stats.totalUsers = result?.[0]?.count || 0;
        completedQueries++;
        if (completedQueries === totalQueries) sendStats();
      });
      
      db.query('SELECT COUNT(*) as count FROM users WHERE is_online = TRUE', (err, result) => {
        stats.onlineUsers = result?.[0]?.count || 0;
        completedQueries++;
        if (completedQueries === totalQueries) sendStats();
      });
      
      db.query('SELECT COUNT(*) as count FROM users WHERE is_banned = TRUE', (err, result) => {
        stats.bannedUsers = result?.[0]?.count || 0;
        completedQueries++;
        if (completedQueries === totalQueries) sendStats();
      });
      
      db.query('SELECT COUNT(*) as count FROM messages', (err, result) => {
        stats.totalMessages = result?.[0]?.count || 0;
        completedQueries++;
        if (completedQueries === totalQueries) sendStats();
      });
      
      db.query('SELECT COUNT(*) as count FROM group_messages', (err, result) => {
        stats.groupMessages = result?.[0]?.count || 0;
        completedQueries++;
        if (completedQueries === totalQueries) sendStats();
      });
      
      db.query('SELECT COUNT(*) as count FROM groups', (err, result) => {
        stats.totalGroups = result?.[0]?.count || 0;
        completedQueries++;
        if (completedQueries === totalQueries) sendStats();
      });
      
      db.query('SELECT COUNT(*) as count FROM posts', (err, result) => {
        stats.totalPosts = result?.[0]?.count || 0;
        completedQueries++;
        if (completedQueries === totalQueries) sendStats();
      });
      
      db.query('SELECT COUNT(*) as count FROM likes', (err, result) => {
        stats.totalLikes = result?.[0]?.count || 0;
        completedQueries++;
        if (completedQueries === totalQueries) sendStats();
      });
      
      db.query('SELECT COUNT(*) as count FROM comments', (err, result) => {
        stats.totalComments = result?.[0]?.count || 0;
        completedQueries++;
        if (completedQueries === totalQueries) sendStats();
      });
      
      db.query('SELECT COUNT(*) as count FROM hashtags', (err, result) => {
        stats.totalHashtags = result?.[0]?.count || 0;
        completedQueries++;
        if (completedQueries === totalQueries) sendStats();
      });
      
      function sendStats() {
        res.json({
          success: true,
          data: stats,
          timestamp: new Date().toISOString()
        });
      }
    });
  });

  // ============================================
  // 3️⃣ ПОЛУЧИТЬ ИНФОРМАЦИЮ О ВСЕХ ПОЛЬЗОВАТЕЛЯХ (ADMIN)
  // ============================================
  app.get('/admin/all-users', authenticateToken, (req, res) => {
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен' });
      }
      
      db.query(`
        SELECT 
          id,
          username,
          email,
          avatar,
          bio,
          status,
          is_online,
          is_banned,
          ban_reason,
          banned_at,
          banned_until,
          is_admin,
          created_at
        FROM users
        ORDER BY created_at DESC
      `, (err, results) => {
        if (err) return res.status(500).json({ error: 'Ошибка сервера' });
        res.json(results);
      });
    });
  });

  // ============================================
  // 4️⃣ ПОИСК ПОЛЬЗОВАТЕЛЕЙ ПО ИМЕНИ ИЛИ EMAIL (ADMIN)
  // ============================================
  app.get('/admin/users/search', authenticateToken, (req, res) => {
    const { q } = req.query;
    
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен' });
      }
      
      if (!q || q.trim().length < 2) {
        return res.status(400).json({ error: 'Запрос должен содержать минимум 2 символа' });
      }
      
      db.query(`
        SELECT 
          id,
          username,
          email,
          avatar,
          is_online,
          is_banned,
          ban_reason,
          is_admin,
          created_at
        FROM users
        WHERE username LIKE ? OR email LIKE ?
        LIMIT 20
      `, [`%${q}%`, `%${q}%`], (err, results) => {
        if (err) return res.status(500).json({ error: 'Ошибка сервера' });
        res.json(results);
      });
    });
  });

  // ============================================
  // 5️⃣ СБРОС ПАРОЛЯ ПОЛЬЗОВАТЕЛЯ (ADMIN)
  // ============================================
  app.post('/admin/users/:userId/reset-password', authenticateToken, async (req, res) => {
    const { userId } = req.params;
    const { newPassword } = req.body;
    
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен' });
      }
      
      if (!newPassword || newPassword.length < 6) {
        return res.status(400).json({ error: 'Пароль должен быть минимум 6 символов' });
      }
      
      bcrypt.hash(newPassword, 10, (err, hashedPassword) => {
        if (err) {
          console.error('❌ Error hashing password:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        
        db.query(
          'UPDATE users SET password = ? WHERE id = ?',
          [hashedPassword, userId],
          (err, result) => {
            if (err) {
              console.error('❌ Error updating password:', err);
              return res.status(500).json({ error: 'Ошибка сервера' });
            }
            
            if (result.affectedRows === 0) {
              return res.status(404).json({ error: 'Пользователь не найден' });
            }
            
            console.log(`✅ Пароль пользователя ${userId} изменен администратором ${req.user.id}`);
            
            res.json({ 
              success: true, 
              message: 'Пароль пользователя успешно изменен'
            });
          }
        );
      });
    });
  });

  // ============================================
  // 6️⃣ ПОЛУЧИТЬ ИНФОРМАЦИЮ О КОНКРЕТНОМ ПОЛЬЗОВАТЕЛЕ (ADMIN)
  // ============================================
  app.get('/admin/users/:userId/info', authenticateToken, (req, res) => {
    const { userId } = req.params;
    
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен' });
      }
      
      db.query(`
        SELECT 
          u.id,
          u.username,
          u.email,
          u.avatar,
          u.bio,
          u.status,
          u.is_online,
          u.is_banned,
          u.ban_reason,
          u.banned_at,
          u.banned_until,
          u.is_admin,
          u.created_at,
          (SELECT COUNT(*) FROM posts WHERE user_id = u.id) as posts_count,
          (SELECT COUNT(*) FROM messages WHERE sender_id = u.id) as messages_sent,
          (SELECT COUNT(*) FROM messages WHERE receiver_id = u.id) as messages_received,
          (SELECT COUNT(*) FROM friends WHERE user_id = u.id OR friend_id = u.id) as friends_count,
          (SELECT COUNT(*) FROM group_members WHERE user_id = u.id) as groups_count
        FROM users u
        WHERE u.id = ?
      `, [userId], (err, results) => {
        if (err) return res.status(500).json({ error: 'Ошибка сервера' });
        if (results.length === 0) return res.status(404).json({ error: 'Пользователь не найден' });
        
        res.json(results[0]);
      });
    });
  });

  // ============================================
  // 7️⃣ ПОЛУЧИТЬ ИСТОРИИ ДЕЙСТВИЙ (ADMIN LOGS)
  // ============================================
  app.get('/admin/action-logs', authenticateToken, (req, res) => {
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен' });
      }
      
      res.json({
        success: true,
        logs: serverLogs.slice(-100)
      });
    });
  });

  // ============================================
  // 8️⃣ ПОЛУЧИТЬ ВСЕ АКТИВНЫЕ ЗВОНКИ (ADMIN)
  // ============================================
  app.get('/admin/active-calls', authenticateToken, (req, res) => {
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен' });
      }
      
      const calls = Array.from(activeCalls.entries()).map(([callId, call]) => ({
        callId,
        ...call,
        durationSeconds: Math.floor((new Date() - call.started_at) / 1000)
      }));
      
      res.json({
        success: true,
        active_calls: calls,
        total_active: calls.length
      });
    });
  });

  // ============================================
  // 9️⃣ ЗАВЕРШИТЬ ЗВОНОК ПОЛЬЗОВАТЕЛЯ (ADMIN)
  // ============================================
  app.post('/admin/calls/:callId/terminate', authenticateToken, (req, res) => {
    const { callId } = req.params;
    
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен' });
      }
      
      const call = activeCalls.get(callId);
      
      if (!call) {
        return res.status(404).json({ error: 'Звонок не найден' });
      }
      
      io.to(`user_${call.from_user_id}`).emit('call_terminated_by_admin', {
        callId,
        message: 'Звонок завершен администратором'
      });
      
      io.to(`user_${call.to_user_id}`).emit('call_terminated_by_admin', {
        callId,
        message: 'Звонок завершен администратором'
      });
      
      activeCalls.delete(callId);
      
      console.log(`✅ Звонок ${callId} завершен администратором ${req.user.id}`);
      
      res.json({ 
        success: true, 
        message: 'Звонок завершен'
      });
    });
  });

  // ============================================
  // 🔟 ПОЛУЧИТЬ ИНФОРМАЦИЮ О ГРУППЕ (ADMIN)
  // ============================================
  app.get('/admin/groups/:groupId/info', authenticateToken, (req, res) => {
    const { groupId } = req.params;
    
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен' });
      }
      
      db.query(`
        SELECT 
          g.id,
          g.name,
          g.description,
          g.avatar,
          g.created_by,
          u.username as creator_username,
          (SELECT COUNT(*) FROM group_members WHERE group_id = g.id) as members_count,
          (SELECT COUNT(*) FROM group_messages WHERE group_id = g.id) as messages_count,
          g.created_at
        FROM groups g
        LEFT JOIN users u ON g.created_by = u.id
        WHERE g.id = ?
      `, [groupId], (err, results) => {
        if (err) return res.status(500).json({ error: 'Ошибка сервера' });
        if (results.length === 0) return res.status(404).json({ error: 'Группа не найдена' });
        
        res.json(results[0]);
      });
    });
  });

  // ============================================
  // ⓫ ЭКСПОРТИРОВАТЬ БАЗУ ДАННЫХ (ADMIN)
  // ============================================
  app.get('/admin/database/export', authenticateToken, (req, res) => {
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен' });
      }
      
      const tables = [
        'users',
        'messages',
        'groups',
        'group_members',
        'group_messages',
        'posts',
        'likes',
        'comments',
        'friends',
        'calls',
        'hashtags',
        'post_hashtags',
        'post_reports'
      ];
      
      const exportData = {};
      let completed = 0;
      
      tables.forEach(table => {
        db.query(`SELECT * FROM ${table}`, (err, results) => {
          exportData[table] = results || [];
          completed++;
          
          if (completed === tables.length) {
            res.json({
              success: true,
              export_date: new Date().toISOString(),
              data: exportData
            });
          }
        });
      });
    });
  });

  // ============================================
  // ⓬ ОТПРАВИТЬ УВЕДОМЛЕНИЕ ВСЕМ ПОЛЬЗОВАТЕЛЯМ (ADMIN)
  // ============================================
  app.post('/admin/broadcast-notification', authenticateToken, (req, res) => {
    const { title, message } = req.body;
    
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен' });
      }
      
      if (!title || !message) {
        return res.status(400).json({ error: 'Заголовок и сообщение обязательны' });
      }
      
      io.emit('admin_notification', {
        title,
        message,
        sent_by: req.user.username,
        sent_at: new Date().toISOString()
      });
      
      console.log(`📢 Администратор ${req.user.id} отправил уведомление всем пользователям`);
      
      res.json({ 
        success: true, 
        message: 'Уведомление отправлено всем пользователям'
      });
    });
  });

  // ============================================
  // ⓭ БЛОКИРОВКА/РАЗБЛОКИРОВКА ПОЛЬЗОВАТЕЛЯ ОТ ЧАТОВ (ADMIN)
  // ============================================
  app.post('/admin/users/:userId/mute', authenticateToken, (req, res) => {
    const { userId } = req.params;
    const { duration_minutes = 60 } = req.body;
    
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен' });
      }
      
      const muteUntil = new Date(Date.now() + duration_minutes * 60 * 1000);
      
      if (!global.mutedUsers) {
        global.mutedUsers = new Map();
      }
      
      global.mutedUsers.set(String(userId), {
        muteUntil,
        mutedBy: req.user.id,
        reason: req.body.reason || 'Администратор заглушил'
      });
      
      io.to(`user_${userId}`).emit('user_muted', {
        duration_minutes,
        reason: req.body.reason || 'Вы заглушены администратором',
        until: muteUntil
      });
      
      console.log(`🔇 Пользователь ${userId} заглушен на ${duration_minutes} минут админом ${req.user.id}`);
      
      res.json({ 
        success: true, 
        message: `Пользователь заглушен на ${duration_minutes} минут`
      });
    });
  });

  app.post('/admin/users/:userId/unmute', authenticateToken, (req, res) => {
    const { userId } = req.params;
    
    db.query('SELECT is_admin FROM users WHERE id = ?', [req.user.id], (err, results) => {
      if (err) return res.status(500).json({ error: 'Ошибка сервера' });
      if (results.length === 0 || !results[0].is_admin) {
        return res.status(403).json({ error: 'Доступ запрещен' });
      }
      
      if (global.mutedUsers) {
        global.mutedUsers.delete(String(userId));
      }
      
      io.to(`user_${userId}`).emit('user_unmuted', {
        message: 'Вы больше не заглушены'
      });
      
      console.log(`🔊 Пользователь ${userId} разглушен админом ${req.user.id}`);
      
      res.json({ 
        success: true, 
        message: 'Пользователь разглушен'
      });
    });
  });

  // ============================================
  // 🏘️ ЭНДПОИНТЫ ДЛЯ СООБЩЕСТВ (Communities)
  // ============================================

  // 1️⃣ ПОЛУЧИТЬ ВСЕ СООБЩЕСТВА (с пагинацией и фильтрацией)
  // 1️⃣ ПОЛУЧИТЬ ВСЕ СООБЩЕСТВА (с пагинацией и фильтрацией) - ИСПРАВЛЕНО
  app.get('/api/communities', authenticateToken, (req, res) => {
    const { limit = 20, offset = 0, category = null, search = null, sort = 'newest' } = req.query;
    const userId = req.user.id;
    
    let query = `
      SELECT 
        c.*,
        u.username as creator_username,
        u.avatar as creator_avatar,
        COALESCE(cs.members_count, 0) as members_count,
        COALESCE(cs.posts_count, 0) as posts_count,
        COALESCE(cs.followers_count, 0) as followers_count,
        COALESCE(cs.views_count, 0) as views_count,
        COALESCE(
          (SELECT 1 FROM community_members 
          WHERE community_id = c.id AND user_id = ?), 0
        ) as is_member,
        COALESCE(
          (SELECT 1 FROM community_followers 
          WHERE community_id = c.id AND user_id = ?), 0
        ) as is_following,
        COALESCE(
          (SELECT 1 FROM community_bans 
          WHERE community_id = c.id AND banned_user_id = ?), 0
        ) as is_banned
      FROM communities c
      LEFT JOIN users u ON c.creator_id = u.id
      LEFT JOIN community_stats cs ON c.id = cs.community_id
      WHERE 1=1
    `;
    
    const params = [userId, userId, userId];
    
    // ✅ ИСПРАВЛЕНО: Показываем И приватные И публичные сообщества
    // Приватные - только если пользователь уже член
    query += ` AND (c.is_private = 0 OR c.id IN (
      SELECT community_id FROM community_members WHERE user_id = ?
    ))`;
    params.push(userId);
    
    if (category) {
      query += ` AND c.category = ?`;
      params.push(category);
    }
    
    if (search) {
      query += ` AND (c.name LIKE ? OR c.description LIKE ?)`;
      params.push(`%${search}%`, `%${search}%`);
    }
    
    if (sort === 'newest') {
      query += ` ORDER BY c.created_at DESC`;
    } else if (sort === 'members') {
      query += ` ORDER BY cs.members_count DESC`;
    } else if (sort === 'trending') {
      query += ` ORDER BY cs.posts_count DESC, cs.members_count DESC`;
    }
    
    query += ` LIMIT ? OFFSET ?`;
    params.push(parseInt(limit), parseInt(offset));
    
    db.query(query, params, (err, results) => {
      if (err) {
        console.error('❌ Ошибка получения сообществ:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }
      
      // Получаем общее количество
      let countQuery = `SELECT COUNT(*) as total FROM communities WHERE 1=1`;
      const countParams = [];
      
      // ✅ То же самое для счета
      countQuery += ` AND (is_private = 0 OR id IN (
        SELECT community_id FROM community_members WHERE user_id = ?
      ))`;
      countParams.push(userId);
      
      if (category) {
        countQuery += ` AND category = ?`;
        countParams.push(category);
      }
      
      if (search) {
        countQuery += ` AND (name LIKE ? OR description LIKE ?)`;
        countParams.push(`%${search}%`, `%${search}%`);
      }
      
      db.query(countQuery, countParams, (err, countResults) => {
        res.json({
          success: true,
          data: results || [],
          total: countResults?.[0]?.total || 0,
          limit: parseInt(limit),
          offset: parseInt(offset)
        });
      });
    });
  });

  // 2️⃣ ПОЛУЧИТЬ МОИ СООБЩЕСТВА
  // 2️⃣ МОИ СООБЩЕСТВА - ИСПРАВЛЕНО (добавлен views_count)
  app.get('/api/communities/my', authenticateToken, (req, res) => {
    const userId = req.user.id;
    
    console.log(`\n${'='.repeat(70)}`);
    console.log(`📋 МОИ СООБЩЕСТВА`);
    console.log(`   Пользователь: ${userId}`);
    console.log(`${'='.repeat(70)}`);
    
    const query = `
      SELECT 
        c.*,
        u.username as creator_username,
        u.avatar as creator_avatar,
        COALESCE(cs.members_count, 0) as members_count,
        COALESCE(cs.posts_count, 0) as posts_count,
        COALESCE(cs.followers_count, 0) as followers_count,
        COALESCE(cs.views_count, 0) as views_count,
        cm.role as my_role
      FROM communities c
      JOIN community_members cm ON c.id = cm.community_id
      LEFT JOIN users u ON c.creator_id = u.id
      LEFT JOIN community_stats cs ON c.id = cs.community_id
      WHERE cm.user_id = ?
      ORDER BY cm.joined_at DESC
    `;
    
    db.query(query, [userId], (err, results) => {
      if (err) {
        console.error('❌ Ошибка получения моих сообществ:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }
      
      console.log(`✅ Получено ${results.length} сообществ для пользователя`);
      console.log(`${'='.repeat(70)}\n`);
      
      res.json({
        success: true,
        data: results || []
      });
    });
  });

  // 3️⃣ ПОЛУЧИТЬ ДЕТАЛЬ СООБЩЕСТВА - ИСПРАВЛЕНО (логирование + views_count)
  app.get('/api/communities/:communityId', authenticateToken, (req, res) => {
    const { communityId } = req.params;
    const userId = req.user.id;
    
    console.log(`\n${'='.repeat(70)}`);
    console.log(`📖 ДЕТАЛЬ СООБЩЕСТВА`);
    console.log(`   ID: ${communityId}`);
    console.log(`   Зритель: ${userId}`);
    console.log(`${'='.repeat(70)}`);
    
    const query = `
      SELECT 
        c.*,
        u.username as creator_username,
        u.avatar as creator_avatar,
        COALESCE(cs.members_count, 0) as members_count,
        COALESCE(cs.posts_count, 0) as posts_count,
        COALESCE(cs.followers_count, 0) as followers_count,
        COALESCE(cs.views_count, 0) as views_count,
        EXISTS(
          SELECT 1 FROM community_members 
          WHERE community_id = c.id AND user_id = ?
        ) as is_member,
        EXISTS(
          SELECT 1 FROM community_followers 
          WHERE community_id = c.id AND user_id = ?
        ) as is_following,
        EXISTS(
          SELECT 1 FROM community_bans 
          WHERE community_id = c.id AND banned_user_id = ?
        ) as is_banned
      FROM communities c
      LEFT JOIN users u ON c.creator_id = u.id
      LEFT JOIN community_stats cs ON c.id = cs.community_id
      WHERE c.id = ?
    `;
    
    db.query(query, [userId, userId, userId, communityId], (err, results) => {
      if (err) {
        console.error('❌ Ошибка получения деталей сообщества:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }
      
      if (!results || results.length === 0) {
        console.warn(`⚠️ Сообщество ${communityId} не найдено`);
        return res.status(404).json({ error: 'Сообщество не найдено' });
      }
      
      const community = results[0];
      
      // Увеличиваем счетчик просмотров
      db.query(
        `INSERT INTO community_stats (community_id, views_count) VALUES (?, 1)
        ON DUPLICATE KEY UPDATE views_count = views_count + 1`,
        [communityId]
      );
      
      console.log(`✅ Сообщество: ${community.name}`);
      console.log(`   Просмотров: ${community.views_count}`);
      console.log(`   Членов: ${community.members_count}`);
      console.log(`${'='.repeat(70)}\n`);
      
      res.json({
        success: true,
        data: community
      });
    });
  });

  // 4️⃣ СОЗДАТЬ СООБЩЕСТВО
  // 4️⃣ СОЗДАТЬ СООБЩЕСТВО - ПОЛНОСТЬЮ ИСПРАВЛЕНО
  // 🆕 НАСТРОЙКА MULTER ДЛЯ ЗАГРУЗКИ СООБЩЕСТВ
  const uploadsCommunitiesDir = path.join(__dirname, 'uploads/communities');
  if (!fs.existsSync(uploadsCommunitiesDir)) {
    fs.mkdirSync(uploadsCommunitiesDir, { recursive: true });
  }
  
  const communityUpload = multer({
    storage: multer.diskStorage({
      destination: (req, file, cb) => {
        cb(null, uploadsCommunitiesDir);
      },
      filename: (req, file, cb) => {
        const uniqueSuffix = Date.now() + '-' + Math.round(Math.random() * 1E9);
        cb(null, 'community-' + uniqueSuffix + path.extname(file.originalname));
      }
    }),
    limits: { fileSize: 10 * 1024 * 1024 },
    fileFilter: (req, file, cb) => {
      const allowedTypes = /jpeg|jpg|png|gif/;
      const extname = allowedTypes.test(path.extname(file.originalname).toLowerCase());
      const mimetype = allowedTypes.test(file.mimetype);
      if (mimetype && extname) {
        return cb(null, true);
      } else {
        cb(new Error('Разрешены только изображения'));
      }
    }
  });

  app.post('/api/communities', authenticateToken, communityUpload.single('image'), (req, res) => {
    const { name, description, category = 'General', banner_image, rules, is_private = false } = req.body;
    const userId = req.user.id;
    
    console.log(`\n${'='.repeat(70)}`);
    console.log(`🆕 СОЗДАНИЕ СООБЩЕСТВА`);
    console.log(`   Название: ${name}`);
    console.log(`   Категория: ${category}`);
    console.log(`   Приватное: ${is_private}`);
    console.log(`   Создатель: ${userId}`);
    console.log(`   Файл загружен: ${req.file ? 'ДА' : 'НЕТ'}`);
    if (req.file) {
      console.log(`   Имя файла: ${req.file.filename}`);
      console.log(`   Размер: ${(req.file.size / 1024).toFixed(2)} KB`);
    }
    console.log(`${'='.repeat(70)}`);
    
    if (!name || name.trim().length === 0) {
      return res.status(400).json({ error: 'Название сообщества обязательно' });
    }
    
    if (name.length > 100) {
      return res.status(400).json({ error: 'Название не должно превышать 100 символов' });
    }
    
    // ✅ Формируем URL изображения
    let imageUrl = null;
    if (req.file) {
      imageUrl = `http://151.241.228.247:3001/uploads/communities/${req.file.filename}`;
      console.log(`✅ Image URL: ${imageUrl}`);
    }
    
    // Создаем сообщество
    db.query(
      `INSERT INTO communities (name, description, category, creator_id, image, banner_image, rules, is_private) 
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [name, description || null, category, userId, imageUrl || null, banner_image || null, rules || null, is_private ? 1 : 0],
      (err, result) => {
        if (err) {
          console.error('❌ Ошибка создания сообщества:', err);
          // Удаляем загруженный файл если был
          if (req.file) {
            fs.unlink(req.file.path, (unlinkErr) => {
              if (unlinkErr) console.error('⚠️ Ошибка удаления файла:', unlinkErr);
            });
          }
          return res.status(500).json({ error: 'Ошибка сервера', details: err.message });
        }
        
        const communityId = result.insertId;
        console.log(`✅ Сообщество ${communityId} создано в БД`);
        
        // Шаг 1: Добавляем создателя как admin
        db.query(
          `INSERT INTO community_members (community_id, user_id, role) VALUES (?, ?, 'admin')`,
          [communityId, userId],
          (memberErr) => {
            if (memberErr) {
              console.error('⚠️ Ошибка добавления создателя в члены:', memberErr);
            } else {
              console.log(`✅ Создатель ${userId} добавлен как admin`);
            }
            
            // Шаг 2: Инициализируем статистику - ИСПРАВЛЕНО (полные поля)
            db.query(
              `INSERT INTO community_stats (community_id, members_count, posts_count, followers_count, views_count) 
              VALUES (?, 1, 0, 0, 0)
              ON DUPLICATE KEY UPDATE members_count = 1`,
              [communityId],
              (statsErr) => {
                if (statsErr) {
                  console.error('⚠️ Ошибка создания статистики:', statsErr);
                } else {
                  console.log(`✅ Статистика сообщества ${communityId} инициализирована`);
                }
                
                // Шаг 3: Получаем полные данные нового сообщества
                db.query(
                  `SELECT 
                    c.*,
                    u.username as creator_username,
                    u.avatar as creator_avatar,
                    COALESCE(cs.members_count, 1) as members_count,
                    COALESCE(cs.posts_count, 0) as posts_count,
                    COALESCE(cs.followers_count, 0) as followers_count,
                    COALESCE(cs.views_count, 0) as views_count,
                    1 as is_member,
                    0 as is_following,
                    0 as is_banned
                  FROM communities c
                  LEFT JOIN users u ON c.creator_id = u.id
                  LEFT JOIN community_stats cs ON c.id = cs.community_id
                  WHERE c.id = ?`,
                  [communityId],
                  (fetchErr, communities) => {
                    if (fetchErr) {
                      console.error('❌ Ошибка получения данных сообщества:', fetchErr);
                      return res.status(500).json({ error: 'Ошибка сервера' });
                    }
                    
                    if (!communities || communities.length === 0) {
                      console.error('❌ Сообщество не найдено после создания');
                      return res.status(500).json({ error: 'Ошибка: сообщество не найдено' });
                    }
                    
                    const community = communities[0];
                    
                    console.log(`✅ Полные данные сообщества ${communityId} загружены`);
                    console.log(`   Имя: ${community.name}`);
                    console.log(`   Создатель: ${community.creator_username}`);
                    console.log(`${'='.repeat(70)}\n`);
                    
                    // Отправляем обновление через Socket.io
                    io.emit('community_created', {
                      community_id: communityId,
                      name: name,
                      creator_id: userId,
                      created_at: new Date()
                    });
                    
                    // ✅ ГЛАВНОЕ: Возвращаем полные данные сообщества
                    res.json({
                      success: true,
                      message: 'Сообщество создано',
                      community_id: communityId,
                      data: community
                    });
                  }
                );
              }
            );
          }
        );
      }
    );
  });

  // 5️⃣ ОБНОВИТЬ СООБЩЕСТВО (только администратор)
  app.put('/api/communities/:communityId', authenticateToken, (req, res) => {
    const { communityId } = req.params;
    const { name, description, category, image, banner_image, rules } = req.body;
    const userId = req.user.id;
    
    // Проверяем, является ли пользователь админом сообщества
    db.query(
      `SELECT role FROM community_members WHERE community_id = ? AND user_id = ?`,
      [communityId, userId],
      (err, results) => {
        if (err) {
          console.error('❌ Ошибка проверки роли:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        
        if (!results || results.length === 0 || results[0].role === 'member') {
          return res.status(403).json({ error: 'Только администраторы могут редактировать сообщество' });
        }
        
        const updateData = {};
        if (name) updateData.name = name;
        if (description) updateData.description = description;
        if (category) updateData.category = category;
        if (image) updateData.image = image;
        if (banner_image) updateData.banner_image = banner_image;
        if (rules) updateData.rules = rules;
        
        db.query(
          `UPDATE communities SET ? WHERE id = ?`,
          [updateData, communityId],
          (err) => {
            if (err) {
              console.error('❌ Ошибка обновления сообщества:', err);
              return res.status(500).json({ error: 'Ошибка сервера' });
            }
            
            console.log(`✅ Сообщество ${communityId} обновлено админом ${userId}`);
            
            res.json({
              success: true,
              message: 'Сообщество обновлено'
            });
          }
        );
      }
    );
  });

  // 6️⃣ УДАЛИТЬ СООБЩЕСТВО (только создатель)
  app.delete('/api/communities/:communityId', authenticateToken, (req, res) => {
    const { communityId } = req.params;
    const userId = req.user.id;
    
    // Проверяем, является ли пользователь создателем
    db.query(
      `SELECT creator_id FROM communities WHERE id = ?`,
      [communityId],
      (err, results) => {
        if (err) {
          console.error('❌ Ошибка проверки создателя:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        
        if (!results || results.length === 0) {
          return res.status(404).json({ error: 'Сообщество не найдено' });
        }
        
        if (results[0].creator_id !== userId) {
          return res.status(403).json({ error: 'Только создатель может удалить сообщество' });
        }
        
        // Удаляем сообщество (каскадное удаление через FOREIGN KEY)
        db.query(
          `DELETE FROM communities WHERE id = ?`,
          [communityId],
          (err) => {
            if (err) {
              console.error('❌ Ошибка удаления сообщества:', err);
              return res.status(500).json({ error: 'Ошибка сервера' });
            }
            
            console.log(`✅ Сообщество ${communityId} удалено создателем ${userId}`);
            
            res.json({
              success: true,
              message: 'Сообщество удалено'
            });
          }
        );
      }
    );
  });

  // ============================================
  // ЧЛЕНСТВО В СООБЩЕСТВАХ
  // ============================================

  // 7️⃣ ПРИСОЕДИНИТЬСЯ К СООБЩЕСТВУ
  // 5️⃣ ПРИСОЕДИНИТЬСЯ К СООБЩЕСТВУ - ИСПРАВЛЕНО (более детальное логирование)
  app.post('/api/communities/:communityId/join', authenticateToken, (req, res) => {
    const { communityId } = req.params;
    const userId = req.user.id;
    
    console.log(`\n${'='.repeat(70)}`);
    console.log(`🔗 ПРИСОЕДИНЕНИЕ К СООБЩЕСТВУ`);
    console.log(`   Сообщество: ${communityId}`);
    console.log(`   Пользователь: ${userId}`);
    console.log(`${'='.repeat(70)}`);
    
    // Шаг 1: Проверяем, не забанен ли пользователь
    db.query(
      `SELECT * FROM community_bans WHERE community_id = ? AND banned_user_id = ?`,
      [communityId, userId],
      (err, banResults) => {
        if (err) {
          console.error('❌ Ошибка проверки бана:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        
        if (banResults && banResults.length > 0) {
          const ban = banResults[0];
          const isActive = !ban.banned_until || new Date() < new Date(ban.banned_until);
          
          if (isActive) {
            console.warn(`⚠️ Пользователь ${userId} забанен до ${ban.banned_until}`);
            return res.status(403).json({ 
              error: 'Вы заблокированы в этом сообществе',
              reason: ban.reason,
              banned_until: ban.banned_until
            });
          } else {
            console.log(`✅ Бан истёк, позволяем присоединиться`);
          }
        }
        
        // Шаг 2: Проверяем, не член ли уже
        db.query(
          `SELECT id FROM community_members WHERE community_id = ? AND user_id = ?`,
          [communityId, userId],
          (err, memberResults) => {
            if (err) {
              console.error('❌ Ошибка проверки членства:', err);
              return res.status(500).json({ error: 'Ошибка сервера' });
            }
            
            if (memberResults && memberResults.length > 0) {
              console.warn(`⚠️ Пользователь ${userId} уже состоит в сообществе ${communityId}`);
              return res.status(400).json({ error: 'Вы уже состоите в этом сообществе' });
            }
            
            // Шаг 3: Добавляем в сообщество
            db.query(
              `INSERT INTO community_members (community_id, user_id) VALUES (?, ?)`,
              [communityId, userId],
              (err) => {
                if (err) {
                  console.error('❌ Ошибка присоединения:', err);
                  return res.status(500).json({ error: 'Ошибка сервера' });
                }
                
                console.log(`✅ Пользователь ${userId} добавлен в члены`);
                
                // Шаг 4: Обновляем статистику (members_count)
                db.query(
                  `INSERT INTO community_stats (community_id, members_count) VALUES (?, 1)
                  ON DUPLICATE KEY UPDATE members_count = members_count + 1`,
                  [communityId],
                  (statErr) => {
                    if (statErr) {
                      console.error('⚠️ Ошибка обновления статистики:', statErr);
                    } else {
                      console.log(`✅ Статистика сообщества обновлена`);
                    }
                    
                    // Отправляем уведомление через Socket.io
                    io.emit('user_joined_community', {
                      community_id: communityId,
                      user_id: userId,
                      joined_at: new Date()
                    });
                    
                    console.log(`${'='.repeat(70)}\n`);
                    
                    res.json({
                      success: true,
                      message: 'Вы присоединились к сообществу'
                    });
                  }
                );
              }
            );
          }
        );
      }
    );
  });

  // 6️⃣ ВЫЙТИ ИЗ СООБЩЕСТВА - ИСПРАВЛЕНО (логирование + stats update)
  app.post('/api/communities/:communityId/leave', authenticateToken, (req, res) => {
    const { communityId } = req.params;
    const userId = req.user.id;
    
    console.log(`\n${'='.repeat(70)}`);
    console.log(`❌ ВЫХОД ИЗ СООБЩЕСТВА`);
    console.log(`   Сообщество: ${communityId}`);
    console.log(`   Пользователь: ${userId}`);
    console.log(`${'='.repeat(70)}`);
    
    // Шаг 1: Проверяем админов
    db.query(
      `SELECT COUNT(*) as admin_count FROM community_members 
      WHERE community_id = ? AND role = 'admin'`,
      [communityId],
      (err, adminResults) => {
        if (err) {
          console.error('❌ Ошибка проверки админов:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        
        const adminCount = adminResults[0].admin_count;
        console.log(`📊 Админов в сообществе: ${adminCount}`);
        
        if (adminCount === 1) {
          // Проверяем, является ли текущий пользователь единственным админом
          db.query(
            `SELECT role FROM community_members WHERE community_id = ? AND user_id = ?`,
            [communityId, userId],
            (err, roleResults) => {
              if (err) {
                console.error('❌ Ошибка проверки роли:', err);
                return res.status(500).json({ error: 'Ошибка сервера' });
              }
              
              if (roleResults && roleResults.length > 0 && roleResults[0].role === 'admin') {
                console.warn(`⚠️ Пользователь ${userId} - единственный админ`);
                return res.status(400).json({ 
                  error: 'Вы единственный администратор. Пожалуйста, назначьте другого админа перед выходом' 
                });
              }
              
              removeUserFromCommunity();
            }
          );
        } else {
          removeUserFromCommunity();
        }
        
        function removeUserFromCommunity() {
          db.query(
            `DELETE FROM community_members WHERE community_id = ? AND user_id = ?`,
            [communityId, userId],
            (err) => {
              if (err) {
                console.error('❌ Ошибка удаления из членов:', err);
                return res.status(500).json({ error: 'Ошибка сервера' });
              }
              
              console.log(`✅ Пользователь ${userId} удалён из членов`);
              
              // Обновляем статистику (members_count)
              db.query(
                `UPDATE community_stats 
                SET members_count = GREATEST(0, members_count - 1) 
                WHERE community_id = ?`,
                [communityId],
                (statErr) => {
                  if (statErr) {
                    console.error('⚠️ Ошибка обновления статистики:', statErr);
                  } else {
                    console.log(`✅ Статистика сообщества обновлена (members_count -1)`);
                  }
                  
                  io.emit('user_left_community', {
                    community_id: communityId,
                    user_id: userId,
                    left_at: new Date()
                  });
                  
                  console.log(`${'='.repeat(70)}\n`);
                  
                  res.json({
                    success: true,
                    message: 'Вы вышли из сообщества'
                  });
                }
              );
            }
          );
        }
      }
    );
  });

  // 9️⃣ ПОЛУЧИТЬ ЧЛЕНОВ СООБЩЕСТВА
  app.get('/api/communities/:communityId/members', authenticateToken, (req, res) => {
    const { communityId } = req.params;
    const { role = null } = req.query;
    
    let query = `
      SELECT 
        u.id,
        u.username,
        u.avatar,
        u.status,
        cm.role,
        cm.joined_at
      FROM community_members cm
      JOIN users u ON cm.user_id = u.id
      WHERE cm.community_id = ?
    `;
    
    const params = [communityId];
    
    if (role) {
      query += ` AND cm.role = ?`;
      params.push(role);
    }
    
    query += ` ORDER BY 
      CASE cm.role
        WHEN 'admin' THEN 1
        WHEN 'moderator' THEN 2
        ELSE 3
      END,
      cm.joined_at DESC`;
    
    db.query(query, params, (err, results) => {
      if (err) {
        console.error('❌ Ошибка получения членов:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }
      
      res.json({
        success: true,
        data: results || [],
        total: results?.length || 0
      });
    });
  });

  // 🔟 ИЗМЕНИТЬ РОЛЬ ЧЛЕНА (только администратор)
  app.put('/api/communities/:communityId/members/:userId/role', authenticateToken, (req, res) => {
    const { communityId, userId } = req.params;
    const { role } = req.body;
    const adminId = req.user.id;
    
    if (!['member', 'moderator', 'admin'].includes(role)) {
      return res.status(400).json({ error: 'Неверная роль' });
    }
    
    // Проверяем, является ли пользователь администратором
    db.query(
      `SELECT role FROM community_members WHERE community_id = ? AND user_id = ?`,
      [communityId, adminId],
      (err, adminResults) => {
        if (err || !adminResults || adminResults.length === 0 || adminResults[0].role !== 'admin') {
          return res.status(403).json({ error: 'Только администраторы могут менять роли' });
        }
        
        // Обновляем роль
        db.query(
          `UPDATE community_members SET role = ? WHERE community_id = ? AND user_id = ?`,
          [role, communityId, userId],
          (err) => {
            if (err) {
              console.error('❌ Ошибка обновления роли:', err);
              return res.status(500).json({ error: 'Ошибка сервера' });
            }
            
            console.log(`✅ Роль пользователя ${userId} обновлена на ${role} в сообществе ${communityId}`);
            
            res.json({
              success: true,
              message: `Роль изменена на ${role}`
            });
          }
        );
      }
    );
  });

  // 1️⃣1️⃣ ВЫГНАТЬ ЧЛЕНА ИЗ СООБЩЕСТВА (только администратор)
  app.delete('/api/communities/:communityId/members/:userId', authenticateToken, (req, res) => {
    const { communityId, userId } = req.params;
    const adminId = req.user.id;
    
    // Проверяем, является ли пользователь администратором
    db.query(
      `SELECT role FROM community_members WHERE community_id = ? AND user_id = ?`,
      [communityId, adminId],
      (err, adminResults) => {
        if (err || !adminResults || adminResults.length === 0 || adminResults[0].role !== 'admin') {
          return res.status(403).json({ error: 'Только администраторы могут выгонять членов' });
        }
        
        // Удаляем члена
        db.query(
          `DELETE FROM community_members WHERE community_id = ? AND user_id = ?`,
          [communityId, userId],
          (err) => {
            if (err) {
              console.error('❌ Ошибка удаления члена:', err);
              return res.status(500).json({ error: 'Ошибка сервера' });
            }
            
            console.log(`✅ Пользователь ${userId} удален из сообщества ${communityId} админом ${adminId}`);
            
            res.json({
              success: true,
              message: 'Пользователь удален из сообщества'
            });
          }
        );
      }
    );
  });

  // ============================================
  // ПОСТЫ СООБЩЕСТВ
  // ============================================

  // 1️⃣2️⃣ ПОЛУЧИТЬ ПОСТЫ СООБЩЕСТВА
  app.get('/api/communities/:communityId/posts', authenticateToken, (req, res) => {
    const { communityId } = req.params;
    const { limit = 20, offset = 0 } = req.query;
    const userId = req.user.id;
    
    const query = `
      SELECT 
        cp.*,
        u.username as author_username,
        u.avatar as author_avatar,
        COUNT(DISTINCT cpl.id) as likes_count,
        COUNT(DISTINCT cpc.id) as comments_count,
        EXISTS(
          SELECT 1 FROM community_post_likes 
          WHERE post_id = cp.id AND user_id = ?
        ) as is_liked
      FROM community_posts cp
      JOIN users u ON cp.author_id = u.id
      LEFT JOIN community_post_likes cpl ON cp.id = cpl.post_id
      LEFT JOIN community_post_comments cpc ON cp.id = cpc.post_id
      WHERE cp.community_id = ?
      GROUP BY cp.id
      ORDER BY cp.is_pinned DESC, cp.created_at DESC
      LIMIT ? OFFSET ?
    `;
    
    db.query(query, [userId, communityId, parseInt(limit), parseInt(offset)], (err, results) => {
      if (err) {
        console.error('❌ Ошибка получения постов:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }
      
      res.json({
        success: true,
        data: results || [],
        limit: parseInt(limit),
        offset: parseInt(offset)
      });
    });
  });

  // 1️⃣3️⃣ СОЗДАТЬ ПОСТ В СООБЩЕСТВЕ
  app.post('/api/communities/:communityId/posts', authenticateToken, (req, res) => {
    const { communityId } = req.params;
    const { title, content, image } = req.body;
    const userId = req.user.id;
    
    if (!content || content.trim().length === 0) {
      return res.status(400).json({ error: 'Содержание поста обязательно' });
    }
    
    // Проверяем, член ли пользователь сообщества
    db.query(
      `SELECT id FROM community_members WHERE community_id = ? AND user_id = ?`,
      [communityId, userId],
      (err, memberResults) => {
        if (err || !memberResults || memberResults.length === 0) {
          return res.status(403).json({ error: 'Вы должны быть членом сообщества чтобы постить' });
        }
        
        // Создаем пост
        db.query(
          `INSERT INTO community_posts (community_id, author_id, title, content, image) 
          VALUES (?, ?, ?, ?, ?)`,
          [communityId, userId, title || null, content, image || null],
          (err, result) => {
            if (err) {
              console.error('❌ Ошибка создания поста:', err);
              return res.status(500).json({ error: 'Ошибка сервера' });
            }
            
            const postId = result.insertId;
            
            console.log(`✅ Пост ${postId} создан пользователем ${userId} в сообществе ${communityId}`);
            
            io.emit('community_post_created', {
              community_id: communityId,
              post_id: postId,
              author_id: userId
            });
            
            res.json({
              success: true,
              message: 'Пост создан',
              post_id: postId
            });
          }
        );
      }
    );
  });

  // 1️⃣4️⃣ УДАЛИТЬ ПОСТ (автор или администратор)
  app.delete('/api/communities/:communityId/posts/:postId', authenticateToken, (req, res) => {
    const { communityId, postId } = req.params;
    const userId = req.user.id;
    
    // Проверяем права на удаление
    db.query(
      `SELECT cp.author_id, cm.role FROM community_posts cp
      LEFT JOIN community_members cm ON cp.community_id = cm.community_id AND cm.user_id = ?
      WHERE cp.id = ? AND cp.community_id = ?`,
      [userId, postId, communityId],
      (err, results) => {
        if (err || !results || results.length === 0) {
          return res.status(404).json({ error: 'Пост не найден' });
        }
        
        const { author_id, role } = results[0];
        
        if (author_id !== userId && role !== 'admin' && role !== 'moderator') {
          return res.status(403).json({ error: 'Вы не можете удалить этот пост' });
        }
        
        // Удаляем пост
        db.query(
          `DELETE FROM community_posts WHERE id = ?`,
          [postId],
          (err) => {
            if (err) {
              console.error('❌ Ошибка удаления поста:', err);
              return res.status(500).json({ error: 'Ошибка сервера' });
            }
            
            console.log(`✅ Пост ${postId} удален пользователем ${userId}`);
            
            res.json({
              success: true,
              message: 'Пост удален'
            });
          }
        );
      }
    );
  });

  // 1️⃣5️⃣ ЛАЙК НА ПОСТ
  app.post('/api/communities/:communityId/posts/:postId/like', authenticateToken, (req, res) => {
    const { postId } = req.params;
    const userId = req.user.id;
    
    db.query(
      `SELECT id FROM community_post_likes WHERE post_id = ? AND user_id = ?`,
      [postId, userId],
      (err, results) => {
        if (err) {
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        
        if (results && results.length > 0) {
          // Убираем лайк
          db.query(
            `DELETE FROM community_post_likes WHERE post_id = ? AND user_id = ?`,
            [postId, userId],
            (err) => {
              if (err) {
                return res.status(500).json({ error: 'Ошибка сервера' });
              }
              
              res.json({ success: true, liked: false });
            }
          );
        } else {
          // Добавляем лайк
          db.query(
            `INSERT INTO community_post_likes (post_id, user_id) VALUES (?, ?)`,
            [postId, userId],
            (err) => {
              if (err) {
                return res.status(500).json({ error: 'Ошибка сервера' });
              }
              
              res.json({ success: true, liked: true });
            }
          );
        }
      }
    );
  });

  // 1️⃣6️⃣ ПОЛУЧИТЬ КОММЕНТАРИИ ПОСТА
  app.get('/api/communities/:communityId/posts/:postId/comments', authenticateToken, (req, res) => {
    const { postId } = req.params;
    
    const query = `
      SELECT 
        cpc.*,
        u.username,
        u.avatar
      FROM community_post_comments cpc
      JOIN users u ON cpc.author_id = u.id
      WHERE cpc.post_id = ?
      ORDER BY cpc.created_at ASC
    `;
    
    db.query(query, [postId], (err, results) => {
      if (err) {
        return res.status(500).json({ error: 'Ошибка сервера' });
      }
      
      res.json({
        success: true,
        data: results || []
      });
    });
  });

  // 1️⃣7️⃣ ДОБАВИТЬ КОММЕНТАРИЙ
  app.post('/api/communities/:communityId/posts/:postId/comments', authenticateToken, (req, res) => {
    const { postId } = req.params;
    const { content } = req.body;
    const userId = req.user.id;
    
    if (!content || content.trim().length === 0) {
      return res.status(400).json({ error: 'Комментарий не может быть пустым' });
    }
    
    db.query(
      `INSERT INTO community_post_comments (post_id, author_id, content) VALUES (?, ?, ?)`,
      [postId, userId, content],
      (err, result) => {
        if (err) {
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        
        res.json({
          success: true,
          comment_id: result.insertId
        });
      }
    );
  });

  // ============================================
  // БЛОКИРОВКА В СООБЩЕСТВАХ
  // ============================================

  // 1️⃣8️⃣ ЗАБАНИТЬ ПОЛЬЗОВАТЕЛЯ В СООБЩЕСТВЕ
  app.post('/api/communities/:communityId/ban', authenticateToken, (req, res) => {
    const { communityId } = req.params;
    const { user_id, reason = '', ban_duration_days = null } = req.body;
    const adminId = req.user.id;
    
    // Проверяем права
    db.query(
      `SELECT role FROM community_members WHERE community_id = ? AND user_id = ?`,
      [communityId, adminId],
      (err, adminResults) => {
        if (err || !adminResults || adminResults.length === 0 || adminResults[0].role !== 'admin') {
          return res.status(403).json({ error: 'Только администраторы могут банить' });
        }
        
        let banned_until = null;
        if (ban_duration_days && ban_duration_days > 0) {
          const now = new Date();
          banned_until = new Date(now.getTime() + ban_duration_days * 24 * 60 * 60 * 1000);
        }
        
        db.query(
          `INSERT INTO community_bans (community_id, banned_user_id, banned_by, reason, ban_duration_days, banned_until) 
          VALUES (?, ?, ?, ?, ?, ?)
          ON DUPLICATE KEY UPDATE 
            reason = VALUES(reason),
            banned_until = VALUES(banned_until)`,
          [communityId, user_id, adminId, reason, ban_duration_days, banned_until],
          (err) => {
            if (err) {
              return res.status(500).json({ error: 'Ошибка сервера' });
            }
            
            // Удаляем пользователя из сообщества
            db.query(
              `DELETE FROM community_members WHERE community_id = ? AND user_id = ?`,
              [communityId, user_id]
            );
            
            console.log(`✅ Пользователь ${user_id} забанен в сообществе ${communityId}`);
            
            res.json({ success: true, message: 'Пользователь забанен' });
          }
        );
      }
    );
  });

  // 1️⃣9️⃣ РАЗБАНИТЬ ПОЛЬЗОВАТЕЛЯ
  app.post('/api/communities/:communityId/unban', authenticateToken, (req, res) => {
    const { communityId } = req.params;
    const { user_id } = req.body;
    const adminId = req.user.id;
    
    // Проверяем права
    db.query(
      `SELECT role FROM community_members WHERE community_id = ? AND user_id = ?`,
      [communityId, adminId],
      (err, adminResults) => {
        if (err || !adminResults || adminResults.length === 0 || adminResults[0].role !== 'admin') {
          return res.status(403).json({ error: 'Только администраторы могут разбанить' });
        }
        
        db.query(
          `DELETE FROM community_bans WHERE community_id = ? AND banned_user_id = ?`,
          [communityId, user_id],
          (err) => {
            if (err) {
              return res.status(500).json({ error: 'Ошибка сервера' });
            }
            
            console.log(`✅ Пользователь ${user_id} разбанен в сообществе ${communityId}`);
            
            res.json({ success: true, message: 'Пользователь разбанен' });
          }
        );
      }
    );
  });

  // ============================================
  // ПОИСК И РЕКОМЕНДАЦИИ
  // ============================================

  // 2️⃣0️⃣ ПОИСК СООБЩЕСТВ
  app.get('/api/communities/search', authenticateToken, (req, res) => {
    const { q, limit = 10 } = req.query;
    
    if (!q || q.trim().length < 2) {
      return res.status(400).json({ error: 'Запрос должен быть минимум 2 символа' });
    }
    
    const query = `
      SELECT 
        c.*,
        cs.members_count
      FROM communities c
      LEFT JOIN community_stats cs ON c.id = cs.community_id
      WHERE c.is_private = FALSE AND (c.name LIKE ? OR c.description LIKE ?)
      ORDER BY cs.members_count DESC
      LIMIT ?
    `;
    
    db.query(query, [`%${q}%`, `%${q}%`, parseInt(limit)], (err, results) => {
      if (err) {
        return res.status(500).json({ error: 'Ошибка сервера' });
      }
      
      res.json({
        success: true,
        data: results || []
      });
    });
  });

  // 2️⃣1️⃣ ПОПУЛЯРНЫЕ СООБЩЕСТВА
  app.get('/api/communities/trending', authenticateToken, (req, res) => {
    const { limit = 10 } = req.query;
    const userId = req.user.id;
    
    const query = `
      SELECT 
        c.*,
        cs.members_count,
        cs.posts_count,
        EXISTS(
          SELECT 1 FROM community_members 
          WHERE community_id = c.id AND user_id = ?
        ) as is_member
      FROM communities c
      LEFT JOIN community_stats cs ON c.id = cs.community_id
      WHERE c.is_private = FALSE
      ORDER BY cs.members_count DESC, cs.posts_count DESC
      LIMIT ?
    `;
    
    db.query(query, [userId, parseInt(limit)], (err, results) => {
      if (err) {
        return res.status(500).json({ error: 'Ошибка сервера' });
      }
      
      res.json({
        success: true,
        data: results || []
      });
    });
  });

  // 2️⃣2️⃣ ПОЛУЧИТЬ КАТЕГОРИИ
  app.get('/api/communities/categories', authenticateToken, (req, res) => {
    db.query(`SELECT * FROM community_categories ORDER BY name ASC`, (err, results) => {
      if (err) {
        return res.status(500).json({ error: 'Ошибка сервера' });
      }
      
      res.json({
        success: true,
        data: results || []
      });
    });
  });

  // ============================================
  // ПОДПИСКИ НА СООБЩЕСТВА
  // ============================================

  // 2️⃣3️⃣ ПОДПИСАТЬСЯ НА СООБЩЕСТВО
  app.post('/api/communities/:communityId/follow', authenticateToken, (req, res) => {
    const { communityId } = req.params;
    const userId = req.user.id;
    
    db.query(
      `INSERT INTO community_followers (community_id, user_id) VALUES (?, ?)
      ON DUPLICATE KEY UPDATE followed_at = NOW()`,
      [communityId, userId],
      (err) => {
        if (err) {
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        
        res.json({ success: true, message: 'Вы подписались' });
      }
    );
  });

  // 2️⃣4️⃣ ОТПИСАТЬСЯ ОТ СООБЩЕСТВА
  app.delete('/api/communities/:communityId/follow', authenticateToken, (req, res) => {
    const { communityId } = req.params;
    const userId = req.user.id;
    
    db.query(
      `DELETE FROM community_followers WHERE community_id = ? AND user_id = ?`,
      [communityId, userId],
      (err) => {
        if (err) {
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        
        res.json({ success: true, message: 'Вы отписались' });
      }
    );
  });

  // ============================================
  // 🆘 SUPPORT SYSTEM ENDPOINTS
  // ============================================

  // 1️⃣ ПОЛУЧИТЬ ВСЕ КАТЕГОРИИ ПОДДЕРЖКИ
  app.get('/api/support/categories', authenticateToken, (req, res) => {
    db.query(
      'SELECT * FROM support_categories ORDER BY name ASC',
      (err, results) => {
        if (err) {
          console.error('❌ Ошибка получения категорий:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        
        res.json({
          success: true,
          data: results || []
        });
      }
    );
  });

  // 2️⃣ СОЗДАТЬ ТИКЕТ ПОДДЕРЖКИ
  app.post('/api/support/tickets', authenticateToken, (req, res) => {
    const { subject, message, category = 'general', priority = 'medium' } = req.body;
    const userId = req.user.id;
    
    if (!subject || subject.trim().length === 0) {
      return res.status(400).json({ error: 'Тема обязательна' });
    }
    
    if (!message || message.trim().length === 0) {
      return res.status(400).json({ error: 'Сообщение обязательно' });
    }
    
    if (subject.length > 255) {
      return res.status(400).json({ error: 'Тема не должна превышать 255 символов' });
    }
    
    if (message.length > 5000) {
      return res.status(400).json({ error: 'Сообщение не должно превышать 5000 символов' });
    }
    
    db.query(
      `INSERT INTO support_tickets (user_id, subject, message, category, priority) 
      VALUES (?, ?, ?, ?, ?)`,
      [userId, subject.trim(), message.trim(), category, priority],
      (err, result) => {
        if (err) {
          console.error('❌ Ошибка создания тикета:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        
        const ticketId = result.insertId;
        
        console.log(`✅ Тикет поддержки #${ticketId} создан пользователем ${userId}`);
        console.log(`   Тема: ${subject}`);
        console.log(`   Категория: ${category}`);
        console.log(`   Приоритет: ${priority}`);
        
        // Отправляем уведомление админам
        io.emit('support_ticket_created', {
          ticket_id: ticketId,
          user_id: userId,
          subject,
          category,
          priority,
          created_at: new Date()
        });
        
        res.json({
          success: true,
          message: 'Тикет создан. Спасибо за обращение!',
          ticket_id: ticketId
        });
      }
    );
  });

  // 3️⃣ ПОЛУЧИТЬ МОИ ТИКЕТЫ
  app.get('/api/support/tickets', authenticateToken, (req, res) => {
    const userId = req.user.id;
    const { status = null, limit = 20, offset = 0 } = req.query;
    
    let query = `
      SELECT 
        st.*,
        u.username,
        u.avatar,
        (SELECT COUNT(*) FROM support_replies WHERE ticket_id = st.id) as replies_count,
        (SELECT MAX(created_at) FROM support_replies WHERE ticket_id = st.id) as last_reply_at
      FROM support_tickets st
      JOIN users u ON st.user_id = u.id
      WHERE st.user_id = ?
    `;
    
    const params = [userId];
    
    if (status) {
      query += ` AND st.status = ?`;
      params.push(status);
    }
    
    query += ` ORDER BY st.created_at DESC LIMIT ? OFFSET ?`;
    params.push(parseInt(limit), parseInt(offset));
    
    db.query(query, params, (err, results) => {
      if (err) {
        console.error('❌ Ошибка получения тикетов:', err);
        return res.status(500).json({ error: 'Ошибка сервера' });
      }
      
      res.json({
        success: true,
        data: results || [],
        limit: parseInt(limit),
        offset: parseInt(offset)
      });
    });
  });

  // 4️⃣ ПОЛУЧИТЬ ДЕТАЛЬ ТИКЕТА
  app.get('/api/support/tickets/:ticketId', authenticateToken, (req, res) => {
    const { ticketId } = req.params;
    const userId = req.user.id;
    
    // Проверяем, может ли пользователь видеть этот тикет
    db.query(
      `SELECT st.*, u.username, u.avatar, u.email
      FROM support_tickets st
      JOIN users u ON st.user_id = u.id
      WHERE st.id = ? AND (st.user_id = ? OR ? IN (SELECT id FROM users WHERE is_admin = TRUE))`,
      [ticketId, userId, userId],
      (err, ticketResults) => {
        if (err) {
          console.error('❌ Ошибка получения тикета:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        
        if (!ticketResults || ticketResults.length === 0) {
          return res.status(403).json({ error: 'Доступ запрещен' });
        }
        
        const ticket = ticketResults[0];
        
        // Получаем все ответы
        db.query(
          `SELECT 
            sr.*,
            admin.username as admin_username,
            admin.avatar as admin_avatar
          FROM support_replies sr
          JOIN users admin ON sr.admin_id = admin.id
          WHERE sr.ticket_id = ?
          ORDER BY sr.created_at ASC`,
          [ticketId],
          (err, replies) => {
            if (err) {
              console.error('❌ Ошибка получения ответов:', err);
              replies = [];
            }
            
            // Отмечаем как прочитанное
            db.query(
              `INSERT INTO support_ticket_read_status (ticket_id, user_id, is_read, read_at)
              VALUES (?, ?, TRUE, NOW())
              ON DUPLICATE KEY UPDATE is_read = TRUE, read_at = NOW()`,
              [ticketId, userId]
            );
            
            res.json({
              success: true,
              ticket: ticket,
              replies: replies || []
            });
          }
        );
      }
    );
  });

  // 5️⃣ ДОБАВИТЬ ОТВЕТ НА ТИКЕТ (ADMIN)
  app.post('/api/support/tickets/:ticketId/reply', authenticateToken, (req, res) => {
    const { ticketId } = req.params;
    const { message } = req.body;
    const adminId = req.user.id;
    
    if (!message || message.trim().length === 0) {
      return res.status(400).json({ error: 'Сообщение обязательно' });
    }
    
    if (message.length > 5000) {
      return res.status(400).json({ error: 'Сообщение не должно превышать 5000 символов' });
    }
    
    // Проверяем, является ли пользователь админом
    db.query(
      'SELECT is_admin FROM users WHERE id = ?',
      [adminId],
      (err, adminResults) => {
        if (err || !adminResults || adminResults.length === 0 || !adminResults[0].is_admin) {
          return res.status(403).json({ error: 'Доступ только для администраторов' });
        }
        
        // Проверяем существование тикета
        db.query(
          'SELECT user_id FROM support_tickets WHERE id = ?',
          [ticketId],
          (err, ticketResults) => {
            if (err || !ticketResults || ticketResults.length === 0) {
              return res.status(404).json({ error: 'Тикет не найден' });
            }
            
            const userId = ticketResults[0].user_id;
            
            // Добавляем ответ
            db.query(
              `INSERT INTO support_replies (ticket_id, admin_id, message) VALUES (?, ?, ?)`,
              [ticketId, adminId, message.trim()],
              (err, result) => {
                if (err) {
                  console.error('❌ Ошибка добавления ответа:', err);
                  return res.status(500).json({ error: 'Ошибка сервера' });
                }
                
                // Обновляем статус тикета на 'in_progress' если он был 'open'
                db.query(
                  `UPDATE support_tickets SET status = 'in_progress', updated_at = NOW() 
                  WHERE id = ? AND status = 'open'`,
                  [ticketId]
                );
                
                console.log(`✅ Ответ на тикет #${ticketId} добавлен админом ${adminId}`);
                
                // Отправляем уведомление пользователю через Socket.io
                io.to(`user_${userId}`).emit('support_reply_received', {
                  ticket_id: ticketId,
                  admin_id: adminId,
                  message: message.substring(0, 100) + '...'
                });
                
                res.json({
                  success: true,
                  message: 'Ответ добавлен',
                  reply_id: result.insertId
                });
              }
            );
          }
        );
      }
    );
  });

  // 6️⃣ ОБНОВИТЬ СТАТУС ТИКЕТА (ADMIN)
  app.patch('/api/support/tickets/:ticketId/status', authenticateToken, (req, res) => {
    const { ticketId } = req.params;
    const { status } = req.body;
    const adminId = req.user.id;
    
    if (!['open', 'in_progress', 'resolved', 'closed'].includes(status)) {
      return res.status(400).json({ error: 'Неверный статус' });
    }
    
    // Проверяем, является ли пользователь админом
    db.query(
      'SELECT is_admin FROM users WHERE id = ?',
      [adminId],
      (err, adminResults) => {
        if (err || !adminResults || adminResults.length === 0 || !adminResults[0].is_admin) {
          return res.status(403).json({ error: 'Доступ только для администраторов' });
        }
        
        // Получаем информацию о тикете
        db.query(
          'SELECT user_id FROM support_tickets WHERE id = ?',
          [ticketId],
          (err, ticketResults) => {
            if (err || !ticketResults || ticketResults.length === 0) {
              return res.status(404).json({ error: 'Тикет не найден' });
            }
            
            const userId = ticketResults[0].user_id;
            
            // Обновляем статус
            db.query(
              'UPDATE support_tickets SET status = ?, updated_at = NOW() WHERE id = ?',
              [status, ticketId],
              (err) => {
                if (err) {
                  console.error('❌ Ошибка обновления статуса:', err);
                  return res.status(500).json({ error: 'Ошибка сервера' });
                }
                
                console.log(`✅ Статус тикета #${ticketId} изменен на ${status} админом ${adminId}`);
                
                // Отправляем уведомление пользователю
                io.to(`user_${userId}`).emit('support_ticket_status_changed', {
                  ticket_id: ticketId,
                  status: status
                });
                
                res.json({
                  success: true,
                  message: `Статус изменен на ${status}`
                });
              }
            );
          }
        );
      }
    );
  });

  // 7️⃣ ИЗМЕНИТЬ ПРИОРИТЕТ ТИКЕТА (ADMIN)
  app.patch('/api/support/tickets/:ticketId/priority', authenticateToken, (req, res) => {
    const { ticketId } = req.params;
    const { priority } = req.body;
    const adminId = req.user.id;
    
    if (!['low', 'medium', 'high', 'critical'].includes(priority)) {
      return res.status(400).json({ error: 'Неверный приоритет' });
    }
    
    // Проверяем, является ли пользователь админом
    db.query(
      'SELECT is_admin FROM users WHERE id = ?',
      [adminId],
      (err, adminResults) => {
        if (err || !adminResults || adminResults.length === 0 || !adminResults[0].is_admin) {
          return res.status(403).json({ error: 'Доступ только для администраторов' });
        }
        
        db.query(
          'UPDATE support_tickets SET priority = ?, updated_at = NOW() WHERE id = ?',
          [priority, ticketId],
          (err) => {
            if (err) {
              console.error('❌ Ошибка обновления приоритета:', err);
              return res.status(500).json({ error: 'Ошибка сервера' });
            }
            
            console.log(`✅ Приоритет тикета #${ticketId} изменен на ${priority} админом ${adminId}`);
            
            res.json({
              success: true,
              message: `Приоритет изменен на ${priority}`
            });
          }
        );
      }
    );
  });

  // 8️⃣ ПОЛУЧИТЬ ВСЕ ТИКЕТЫ (ADMIN)
  app.get('/api/admin/support/tickets', authenticateToken, (req, res) => {
    const adminId = req.user.id;
    const { status = null, priority = null, limit = 20, offset = 0 } = req.query;
    
    // Проверяем, является ли пользователь админом
    db.query(
      'SELECT is_admin FROM users WHERE id = ?',
      [adminId],
      (err, adminResults) => {
        if (err || !adminResults || adminResults.length === 0 || !adminResults[0].is_admin) {
          return res.status(403).json({ error: 'Доступ только для администраторов' });
        }
        
        let query = `
          SELECT 
            st.*,
            u.username,
            u.avatar,
            u.email,
            (SELECT COUNT(*) FROM support_replies WHERE ticket_id = st.id) as replies_count,
            sc.name as category_name,
            sc.icon as category_icon
          FROM support_tickets st
          JOIN users u ON st.user_id = u.id
          LEFT JOIN support_categories sc ON st.category = sc.name
          WHERE 1=1
        `;
        
        const params = [];
        
        if (status) {
          query += ` AND st.status = ?`;
          params.push(status);
        }
        
        if (priority) {
          query += ` AND st.priority = ?`;
          params.push(priority);
        }
        
        query += ` ORDER BY 
          CASE st.priority
            WHEN 'critical' THEN 1
            WHEN 'high' THEN 2
            WHEN 'medium' THEN 3
            ELSE 4
          END,
          CASE st.status
            WHEN 'open' THEN 1
            WHEN 'in_progress' THEN 2
            WHEN 'resolved' THEN 3
            ELSE 4
          END,
          st.created_at ASC
        LIMIT ? OFFSET ?`;
        
        params.push(parseInt(limit), parseInt(offset));
        
        db.query(query, params, (err, results) => {
          if (err) {
            console.error('❌ Ошибка получения тикетов:', err);
            return res.status(500).json({ error: 'Ошибка сервера' });
          }
          
          // Получаем статистику
          db.query(
            `SELECT 
              COUNT(*) as total,
              SUM(CASE WHEN status = 'open' THEN 1 ELSE 0 END) as open_count,
              SUM(CASE WHEN status = 'in_progress' THEN 1 ELSE 0 END) as in_progress_count,
              SUM(CASE WHEN status = 'resolved' THEN 1 ELSE 0 END) as resolved_count,
              SUM(CASE WHEN status = 'closed' THEN 1 ELSE 0 END) as closed_count,
              SUM(CASE WHEN priority = 'critical' THEN 1 ELSE 0 END) as critical_count
            FROM support_tickets`,
            (err, statsResults) => {
              res.json({
                success: true,
                data: results || [],
                stats: statsResults?.[0] || {},
                limit: parseInt(limit),
                offset: parseInt(offset)
              });
            }
          );
        });
      }
    );
  });

  // 9️⃣ ПОЛУЧИТЬ СТАТИСТИКУ ПОДДЕРЖКИ (ADMIN)
  app.get('/api/admin/support/statistics', authenticateToken, (req, res) => {
    const adminId = req.user.id;
    
    // Проверяем, является ли пользователь админом
    db.query(
      'SELECT is_admin FROM users WHERE id = ?',
      [adminId],
      (err, adminResults) => {
        if (err || !adminResults || adminResults.length === 0 || !adminResults[0].is_admin) {
          return res.status(403).json({ error: 'Доступ только для администраторов' });
        }
        
        // Получаем статистику
        db.query(
          `SELECT 
            COUNT(*) as total_tickets,
            SUM(CASE WHEN status = 'open' THEN 1 ELSE 0 END) as open_tickets,
            SUM(CASE WHEN status = 'in_progress' THEN 1 ELSE 0 END) as in_progress_tickets,
            SUM(CASE WHEN status = 'resolved' THEN 1 ELSE 0 END) as resolved_tickets,
            SUM(CASE WHEN status = 'closed' THEN 1 ELSE 0 END) as closed_tickets,
            SUM(CASE WHEN priority = 'critical' THEN 1 ELSE 0 END) as critical_tickets,
            SUM(CASE WHEN priority = 'high' THEN 1 ELSE 0 END) as high_tickets,
            SUM(CASE WHEN DATE(created_at) = CURDATE() THEN 1 ELSE 0 END) as tickets_today,
            SUM(CASE WHEN DATE(created_at) = DATE_SUB(CURDATE(), INTERVAL 1 DAY) THEN 1 ELSE 0 END) as tickets_yesterday,
            SUM(CASE WHEN DATE(created_at) >= DATE_SUB(CURDATE(), INTERVAL 7 DAY) THEN 1 ELSE 0 END) as tickets_this_week
          FROM support_tickets`,
          (err, stats) => {
            if (err) {
              console.error('❌ Ошибка получения статистики:', err);
              return res.status(500).json({ error: 'Ошибка сервера' });
            }
            
            // Получаем статистику по категориям
            db.query(
              `SELECT 
                category,
                COUNT(*) as count,
                SUM(CASE WHEN status = 'open' THEN 1 ELSE 0 END) as open_count,
                SUM(CASE WHEN status = 'resolved' THEN 1 ELSE 0 END) as resolved_count
              FROM support_tickets
              GROUP BY category`,
              (err, categoryStats) => {
                res.json({
                  success: true,
                  overall: stats?.[0] || {},
                  by_category: categoryStats || []
                });
              }
            );
          }
        );
      }
    );
  });

  // 🔟 ЗАКРЫТЬ ТИКЕТ (ADMIN/USER)
  app.post('/api/support/tickets/:ticketId/close', authenticateToken, (req, res) => {
    const { ticketId } = req.params;
    const userId = req.user.id;
    
    // Проверяем, может ли пользователь закрыть этот тикет
    db.query(
      `SELECT st.user_id, st.status, u.is_admin
      FROM support_tickets st
      JOIN users u ON u.id = ?
      WHERE st.id = ?`,
      [userId, ticketId],
      (err, results) => {
        if (err) {
          console.error('❌ Ошибка проверки:', err);
          return res.status(500).json({ error: 'Ошибка сервера' });
        }
        
        if (!results || results.length === 0) {
          return res.status(403).json({ error: 'Доступ запрещен' });
        }
        
        const { user_id, status, is_admin } = results[0];
        
        if (user_id !== userId && !is_admin) {
          return res.status(403).json({ error: 'Доступ запрещен' });
        }
        
        if (status === 'closed') {
          return res.status(400).json({ error: 'Тикет уже закрыт' });
        }
        
        // Закрываем тикет
        db.query(
          'UPDATE support_tickets SET status = "closed", updated_at = NOW() WHERE id = ?',
          [ticketId],
          (err) => {
            if (err) {
              console.error('❌ Ошибка закрытия тикета:', err);
              return res.status(500).json({ error: 'Ошибка сервера' });
            }
            
            console.log(`✅ Тикет #${ticketId} закрыт`);
            
            res.json({
              success: true,
              message: 'Тикет закрыт'
            });
          }
        );
      }
    );
  });

  // 1️⃣1️⃣ УДАЛИТЬ ТИКЕТ (ADMIN)
  app.delete('/api/admin/support/tickets/:ticketId', authenticateToken, (req, res) => {
    const { ticketId } = req.params;
    const adminId = req.user.id;
    
    // Проверяем, является ли пользователь админом
    db.query(
      'SELECT is_admin FROM users WHERE id = ?',
      [adminId],
      (err, adminResults) => {
        if (err || !adminResults || adminResults.length === 0 || !adminResults[0].is_admin) {
          return res.status(403).json({ error: 'Доступ только для администраторов' });
        }
        
        db.query(
          'DELETE FROM support_tickets WHERE id = ?',
          [ticketId],
          (err) => {
            if (err) {
              console.error('❌ Ошибка удаления тикета:', err);
              return res.status(500).json({ error: 'Ошибка сервера' });
            }
            
            console.log(`✅ Тикет #${ticketId} удален админом ${adminId}`);
            
            res.json({
              success: true,
              message: 'Тикет удален'
            });
          }
        );
      }
    );
  });

  const PORT = process.env.PORT || 3001;
  server.listen(PORT, '0.0.0.0', () => {
    addLog(`✅ Сервер запущен на порту ${PORT}`, 'success');
    addLog('✅ Push-уведомления АКТИВИРОВАНЫ', 'success');
    addLog('✅ Мониторинг сервера АКТИВИРОВАН', 'success');
    addLog('✅ Socket.io room-based messaging готов', 'success');
    
    console.log(`
  ╔═══════════════════════════════════════════════════════════╗
  ║  ✅ Сервер запущен на порту ${PORT}                       ║
  ║  ✅ Push-уведомления АКТИВИРОВАНЫ                         ║
  ║  ✅ Мониторинг сервера АКТИВИРОВАН                        ║
  ║  ✅ Socket.io room-based messaging готов                  ║
  ║  ⏳ Дозвон: 15 секунд                                     ║
  ╚═══════════════════════════════════════════════════════════╝
    `);
  });